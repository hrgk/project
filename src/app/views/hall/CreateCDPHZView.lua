...
local CreateCDPHZView = class("CreateCDPHZView", BaseCreateView)

function CreateCDPHZView:ctor(showZFType)
    CreateCDPHZView.super.ctor(self)
    self.jushuConf = {8,16}
    self.renShuConf = {3,2}
    self.juShuList_ = {}
    ...
    self.radioInfo = {"dzh","hhx","jhh","47h","dty","3twk"}
    self:initShow()
    createRoomData:setGameIndex(GAME_CDPHZ)
    self:setShowZFType(showZFType)
end

function CreateCDPHZView:setData(data)
    ...
    local conf = {"duiDuiHu","hangHangXi","jiaHangHang","siQiHong","daTuanYuan","santi5kan"}
    for i = 1, #conf do
...
function CreateCDPHZView:initShow()
    local guiZe = xtcdphzData:getRuleInfo()
...
function CreateCDPHZView:getElement(aimNode,aimType)
...
function CreateCDPHZView:initElement_()
    CreateCDPHZView.super.initElement_(self)
...
function CreateCDPHZView:ren3Handler_()
    self:updateRSList_(1)
end

function CreateCDPHZView:ren2Handler_()
    self:updateRSList_(2)
end

function CreateCDPHZView:zhifu1Handler_()
    self:updateZFList_(1)
end

function CreateCDPHZView:zhifu2Handler_()
    self:updateZFList_(2)
end

function CreateCDPHZView:zhifu3Handler_()
    self:updateZFList_(3)
end

function CreateCDPHZView:updateRSList_(index)
...
function CreateCDPHZView:huXiTypeHandler_(item)
...
function CreateCDPHZView:tiPaiHandler_(item)
    if item:isSelected() then
        self.quPai_ = 1
    else
        self.quPai_ = 0
...
function CreateCDPHZView:qiShouTiHandler_(item)
    if item:isSelected() then
        self.qiShouTiPai_ = 1
    else
        self.qiShouTiPai_ = 0
...
function CreateCDPHZView:juShu1Handler_()
    self:updateJuShuList_(1)
end

function CreateCDPHZView:juShu2Handler_()
    self:updateJuShuList_(2)
end

function CreateCDPHZView:updateJuShuList_(index)
...
function CreateCDPHZView:fd1Handler_()
    self:updateFDList_(1)
end

function CreateCDPHZView:fd2Handler_()
    self:updateFDList_(2)
end

function CreateCDPHZView:fd3Handler_()
    self:updateFDList_(3)
end

function CreateCDPHZView:updateFDList_(index)
...
function CreateCDPHZView:ct1Handler_()
    self:updateCTList_(1)
end

function CreateCDPHZView:ct2Handler_()
    self:updateCTList_(2)
end

function CreateCDPHZView:ct3Handler_()
    self:updateCTList_(3)
end

function CreateCDPHZView:updateCTList_(index)
...
function CreateCDPHZView:updateZFList_(index)
...
function CreateCDPHZView:qmtHandler_()
...
function CreateCDPHZView:hhdHandler_()
    self.node_subQMT:setVisible(false)
...
function CreateCDPHZView:updateWFList_(index)
...
function CreateCDPHZView:loaderCsb()
    self.csbNode_ = cc.uiloader:load("views/createRoom/createCDPHZ.csb"):addTo(self)
end

function CreateCDPHZView:getRuleInfo()
    local guiZe = {}
    local strRule = ""
...
function CreateCDPHZView:getAimGuiZe(guiZe)
    local aimGuiZe = {}
    aimGuiZe.ruleType = guiZe["wf"] == 1 and 3 or 1
    local score = {{0,100,200},{0,20,40}}
    aimGuiZe.limitScore = score[guiZe["wf"]][guiZe["fd"]]
    aimGuiZe.baseTun  = guiZe["ct"]
    aimGuiZe.santi5kan  = guiZe["3twk"]
    aimGuiZe.hangHangXi  = guiZe["hhx"]
    aimGuiZe.jiaHangHang  = guiZe["jhh"]
    aimGuiZe.siQiHong  = guiZe["47h"]
...
function CreateCDPHZView:calcCreateRoomParams(daiKai)
    local guiZe = self:getRuleInfo()
    local str = json.encode(guiZe)
    xtcdphzData:setRuleInfo(str)
