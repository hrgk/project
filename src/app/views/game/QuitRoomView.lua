local QuitRoomView = class("QuitRoomView", gailun.BaseView)
...
local data = {
    ...
    children = {
        ...
        {type = TYPES.SPRITE, filename = "res/images/sz_bg.png", scale9 = true, size = {740, 500}, capInsets = cc.rect(340, 150, 1, 1), x = display.cx, y = display.cy, children = {
            ...
            {type = TYPES.SPRITE, var = "tc_word_", filename = "res/images/common/tc_word.png", ppx = 0.5, ppy = 0.95, ap = {0.5, 0.5}},
...
function QuitRoomView:ctor(message, callback, title)
    self.title_ = title
...
function QuitRoomView:callHandler_(isOK)
...
function QuitRoomView:onClose_(event)
    self.callback_ = nil
    if tolua.isnull(self) then
        return
    end
