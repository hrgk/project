local MessageRecordList = class("MessageRecordList", function()
    return display.newSprite()
end)

function MessageRecordList:ctor(data)
    self.data_ = data
    self:initListView_()
end

function MessageRecordList:initListView_()
    self.tableView_ = cc.TableView:create(cc.size(1100, 450))      --列表的显示区域的大小
    self.tableView_:setDirection(1)         --设置列表是竖直方向
    self.tableView_:setPosition(-550, -220+20)
...
function MessageRecordList:cellSizeForTable_()
    return 90, 1068
end

function MessageRecordList:tableCellAtIndex_(table,idx)
    local cell = table:dequeueCell() 
    local index = idx + 1 
    if nil == cell then  
        cell = cc.TableViewCell:new() 
        local item = app:createView("chaguan.message.MessageRecordItem")
...
function MessageRecordList:update(data)
    self.data_ = data
    self.tableView_:reloadData()
end

function MessageRecordList:tableCellTouched_(table,cell)
    local index = cell:getIdx() + 1
end

function MessageRecordList:numberOfCellsInTableView_()
    return #self.data_
end
