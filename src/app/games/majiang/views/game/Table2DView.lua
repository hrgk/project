...
tableNodes.normal = {
    ...
    children = {
        {type = TYPES.NODE, var = "nodeTableInfo_", children = {  -- 桌子信息容器
            ...
            {type = TYPES.CUSTOM, var = "directorController_", class = "app.games.majiang.controllers.DirectorController", px = 0.5, py = display.cy + 30 * display.width / DESIGN_WIDTH},
        ...
        {type = TYPES.LABEL, var = "viewProgress_", type = TYPES.CUSTOM, class = "app.games.majiang.views.game.ProgressView", x = display.cx, y = display.cy + 35},
        {type = TYPES.CUSTOM, var = "focusArrow_", class = "app.games.majiang.views.game.FocusArrowView", px = 0.2, py = 0.5, visible = false},
        ...
        {type = TYPES.CUSTOM, var = "tingView_", class = "app.games.csmj.views.game.Ting2DView", visible = true,x = display.cx-600,y = -120},
...
function Table2DView:ctor(table)
    ...
    local debugView = DebugView.new(COMMANDS.MJ_DEBUG_CONFIG_CARD, testData)
...
function Table2DView:initTGAnim()
    ...
    self.buttonTG_:setPosition(display.left + 280, display.bottom + 170)
...
function Table2DView:calcPokerPos_(total, index)
...
function Table2DView:onChuiClicked_(event)
    local data = {chui = 2}
    dataCenter:sendOverSocket(COMMANDS.MJ_PLAYER_CHUI, data)
end
