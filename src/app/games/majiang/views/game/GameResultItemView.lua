...
function GameResultItemView:showBirds_(params, showData, isWin)
    ...
    if configdata and configdata.maJiangType == 4 then
        ...
        self.spriteZhongNiao_:setTexture("res/images/round_over/zhongniao1.png")
    ...
    --      local x = 0.83 * size.width + (index - 1) * space
    --      index = index + 1
    --      local maJiang = app:createConcreteView("MaJiangView", v, MJ_TABLE_DIRECTION.BOTTOM, true):addTo(self.bg_):pos(x, y)
    --      maJiang:scale(0.6)
    --      table.insert(maJiangs, maJiang)
    --      local ret, birdShunXu = BaseAlgorithm.isBird(v, isZiMo, isSortBird, isTongPao, jiePaoIndex, fangPaoIndex)
    --      if ret then
    ...
    --  end
    -- else
    --  self.spriteZhongNiao_:setTexture("res/images/majiang/round_over/zhongniao2.png")
    -- end
end
