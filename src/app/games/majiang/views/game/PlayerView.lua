local BaseAlgorithm = require("app.games.majiang.utils.BaseAlgorithm")
local FaceAnimationsData = require("app.data.FaceAnimationsData")
local ZZAlgorithm = require("app.games.majiang.utils.ZZAlgorithm")

local TingView = require("app.games.csmj.views.game.TingViewNew")
...
ready_pos[1][1] = display.cx
ready_pos[2][1] = OFFSET_OF_SEATS_4[2][1] - adjust_ready_x-250
ready_pos[3][1] = display.cx
ready_pos[4][1] = OFFSET_OF_SEATS_4[4][1] + adjust_ready_x+250
ready_pos[1][2] = ready_pos[1][2] + 100
ready_pos[3][2] = ready_pos[3][2] - 100
...
function PlayerView:onHidePlayerInfo_()
...
function PlayerView:onRestCardPos_()
...
function PlayerView:calcWaiPaiPosBottom_(index, offset)
    ...
    local y = bottom_margin + majiang_height / 2 + self:calcWaiPaiOffsetHeight_(majiang_height, offset) - 25
    return x, y
end
...
function PlayerView:calcWaiPaiPosLeft_(index, offset)
    ...
    local x = display.width * (166 / DESIGN_WIDTH) - offset*4 - index *15 + 80
    if offset == 4 then
        x = x + 4
    end
...
function PlayerView:showReayAnim()
...
function PlayerView:calcChuPaiPosBottom_(total, index)
    ...
    local isAdd = false
    if index > self.cpLian*3 and self.cpLian == 12 then
...
function PlayerView:calcMaJiangPosLeft_(total, index, isDown)
    ...
    local x = display.width * (166 / DESIGN_WIDTH) - index*6.5 + dist+20
...
function PlayerView:onMaJiangTouchdBegin_(maJiang, event)
    ...
    maJiang:hide()
    self.tMJ_ = app:createConcreteView("MaJiangView", card, self.index_, false, true):addTo(maJiang:getParent())
...
function PlayerView:updateTingPaiTag(card)
    ...
    if ZZAlgorithm.canZiMoHu(handCards, true) then
        self.tingPaiMap = {}
    else
