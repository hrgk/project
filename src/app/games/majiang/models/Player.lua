...
Player.RESET_CARD_POS = "RESET_CARD_POS" 
...
Player.schema['lockGold'] = {"number", 0} -- 是否已举手
...
function Player:setScore(score)
    ...
    local before = self.score_ + lockGold
    local rawScore = before
    self.score_ = score
    self:dispatchEvent({name = Player.SCORE_CHANGED, score = score + lockGold, from = rawScore})
end

function Player:getScore()
    return self.lockGold_ + self.score_
end
...
function Player:reSetCardPos()
    self:dispatchEvent({name = Player.RESET_CARD_POS})
end
...
function Player:checkTingPaiByRemoveCard(card)
    ...
    local cardNum = {}
    local canGangValue = {}
    if self.waiPai_ then
        for i = 1,#self.waiPai_ do
            if self.waiPai_[i].action == MJ_ACTIONS.PENG and self.waiPai_[i].cards[1] == card then
                table.insert(canGangValue,card)
            end
        end
    end
    for i = 1,#temp do
    ...
    return ZZAlgorithm.isTingPai(result,display.getRunningScene():getTable():getAllowSevenPairs()),canGangValue
end
