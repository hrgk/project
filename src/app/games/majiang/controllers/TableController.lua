...
local BaseAlgorithm = import("app.games.majiang.utils.BaseAlgorithm")
local TaskQueue = require("app.controllers.TaskQueue")
local MaJiangTable = import("app.games.majiang.models.MaJiangTable")
...
function TableController:hideReady()
...
function TableController:setPlayerCard(seatID,cards)
    ...
    table.insert(self.tempPalyer,player)
end
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames { 
        {COMMANDS.MJ_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.MJ_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.MJ_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.MJ_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.MJ_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.MJ_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播
        {COMMANDS.MJ_PUBLIC_TIME, handler(self, self.onPublicTime_)},  -- 公共牌时间

        {COMMANDS.MJ_DEAL_CARD, handler(self, self.onDealCards_)},  -- 发牌
        {COMMANDS.MJ_USER_GANG, handler(self, self.onUserGang_)},  -- 玩家提 
        {COMMANDS.MJ_USER_HU, handler(self, self.onUserHu_)},  -- 胡牌
        {COMMANDS.MJ_USER_CHI, handler(self, self.onUserChi_)},  -- 吃牌
        {COMMANDS.MJ_USER_PENG, handler(self, self.onUserPeng_)},  -- 碰牌
        {COMMANDS.MJ_USER_MO_PAI, handler(self, self.onPlayerMoPai_)},  
        {COMMANDS.MJ_ALL_PASS, handler(self, self.onAllPass_)},
        {COMMANDS.MJ_NOTIFY_HU, handler(self, self.onNotifyHu_)},  -- 通知胡牌请求
        {COMMANDS.MJ_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
        {COMMANDS.MJ_PLAY_ACTION, handler(self, self.onPlayAction_)},
        {COMMANDS.MJ_UPDATE_SCORE, handler(self, self.onUpdateScore_)},

        {COMMANDS.MJ_USER_AFTER_GANG, handler(self, self.onCSMJUserGang_)},
        {COMMANDS.MJ_USER_BU_CARD, handler(self, self.onUserGang_)},

        {COMMANDS.MJ_BEGIN_CHUI, handler(self, self.onsBeginChui_)},
        {COMMANDS.MJ_PLAYER_CHUI, handler(self, self.onPlayerChui_)},

        {COMMANDS.MJ_GET_ALL_PLAYER_CARDS, handler(self, self.onGetAllPlayerCards_)},
    }
...
function TableController:setScore(score)
    for i,v in ipairs(self.seats_) do
        v.view_:setScore(v.player_:getScore() + score)
    end
end
...
function TableController:onChuPaiEvent_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.MJ_CHU_PAI, data)
end
    ...
    if data.seatID == self.hostSeatID_ then
        ...
        if player:isLocked() and self.currentMoCard_ > 0 then
            --self.tableView_:performWithDelay(function()
                dataCenter:sendOverSocket(COMMANDS.MJ_CHU_PAI, {
...
function TableController:resumePlayer_(player, data)
    ...
    if data.judgeInfo and data.judgeInfo.naiZi then
        ...
        print("resumeplayer setlaizi ------------")
    -- else
    --     dataCenter:setIsLaiZi(false)
    end
    -- if data.isReview and data.isPrepare then
    --     player:showReady_(false)
    -- end
...
function TableController:onHostPlayerTingEvent_(event)
    dump(event.isTing,"event.isTingevent.isTingevent.isTing")
...
function TableController:onRoundStart_(event)
    TaskQueue.continue()
    if self.tempPalyer then
        for i,v in ipairs(self.tempPalyer) do
            v:onStandUp_()
        end
        self.tempPalyer = nil
    end
    if event.data.isReview then
        ...
        TaskQueue.add(handler(self, self.doRoundStart_), TABLE_TIME.onRoundStart_[1], 2, event.data)
    end
end 
...
function TableController:doTurnTo_(data)
    if data.outCards ~= nil then
        ...
        if not isRight then
            dataCenter:sendOverSocket(COMMANDS.MJ_GET_ALL_PLAYER_CARDS)
        end
    end
...
function TableController:getMatchType()
    self.table_:getMatchType()
end
...
function TableController:resetHostPlayerCardPos()
    local palyer = self:getPlayerBySeatID(self.hostSeatID_)
    if palyer then
        palyer:reSetCardPos()
    end
end
...
function TableController:doTG()
    ...
    if player then
        player:onGangLock_(self.swithTG)
    end
end

function TableController:changeSit(data)
    if data.fromSeatId == self.hostPlayer_:getSeatID() then
        self.hostSeatID_ = data.toSeatId 
    end
    local fromPlayer = self:getPlayerBySeatID(data.fromSeatId)
    fromPlayer:setSeatID(data.toSeatId)
    self.seats_[data.toSeatId],self.seats_[data.fromSeatId] = self.seats_[data.fromSeatId],self.seats_[data.toSeatId]
    for k, player in pairs(self.seats_) do
        player:setIndex(self:calcPlayerIndex(k), true)
    end
end

function TableController:cheakCanChangeSit_()
    local rule = self:getRoomConfig()
    if rule and rule.ruleDetails and rule.ruleDetails.totalSeat then
        if rule.ruleDetails.totalSeat == 2 or rule.ruleDetails.totalSeat ==  self.table_:getCurrPlayerCount() then
            display.getRunningScene():hideChangeSit()
        end
    end
end
