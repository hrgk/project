...
-- require ("app.games.majiang.utils.test")
...
function GameScene:ctor()
    ...
    self.gameType_ = GAME_MJZHUANZHUAN
...
function GameScene:onEnterTransitionFinish()
    ...
    self:onBgLoaded_()
    self:onAllLoaded_()
    local GameSceneTest = require("app.games.majiang.scenes.GameSceneTest")
...
function GameScene:onEnter_()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.MJ_ROUND_OVER, handler(self, self.onRoundOver_)},
        {COMMANDS.MJ_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.LOGIN, handler(self, self.onLoginReturn_)},
        {COMMANDS.MJ_ENTER_ROOM, handler(self, self.onMJEnterRoom_)},
        {COMMANDS.MJ_READY, handler(self, self.onReady_)},
        {COMMANDS.MJ_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.MJ_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.MJ_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.MJ_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.CHANGE_SERVER, handler(self, self.onChangeServer_)},
        {COMMANDS.MJ_DEBUG_CONFIG_CARD, handler(self, self.onConfigCard_)},
        {COMMANDS.MJ_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.MJ_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},
        {COMMANDS.MJ_SHOW_BIRDS, handler(self,self.onShowBirds_)},
        {COMMANDS.MJ_GAME_START, handler(self, self.onGameStart_)},
        {COMMANDS.MJ_CHANGE_SIT, handler(self, self.onChangeSit_)},
    }
...
function GameScene:onWanFa_(event)
    ...
    display.getRunningScene():initWanFa(GAME_MJZHUANZHUAN,date)
end
...
function GameScene:onDingWeiClicked_(event)
    dataCenter:sendOverSocket(COMMANDS.MJ_REQUEST_LOCATION)
end
...
function GameScene:onNotifyLocation_(event)
    ...
    if event.data.code == 0 then
        ...
        if 4 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SiRenDingWei.new(params,event.data.distances,true,GAME_MJZHUANZHUAN):addTo(self.layerWindows_)
        elseif 3 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,true,GAME_MJZHUANZHUAN):addTo(self.layerWindows_)
        end
...
function GameScene:onRequestLocation_(event)
    ...
    if event.data.code == 0 then
        ...
        if 4 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SiRenDingWei.new(params,event.data.distances,false,GAME_MJZHUANZHUAN):addTo(self.layerWindows_)
        elseif 3 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,false,GAME_MJZHUANZHUAN):addTo(self.layerWindows_)
        end
        if self:getTable():getState() ~= "idle" then
                self.dingWeiView_:showBg(true)
                self.dingWeiView_:setPosition(0, -100)
...
function GameScene:onMJEnterRoom_(event)
    if (display.getRunningScene().name == "GAME_MJZHUANZHUAN2D" and setData:getMJHMTYPE()+0  == 2) or (display.getRunningScene().name == "GAME_MJZHUANZHUAN3D" and setData:getMJHMTYPE()+0  == 1) then
        dataCenter:pauseSocketMessage()
        local roomID = event.data.roomID
        dataCenter:setRoomID(roomID)
        gailun.EventUtils.clear(self)
...
function GameScene:onConfigClicked_(event)
    display.getRunningScene():showGameSheZhi(true,GAME_MJZHUANZHUAN)
end

function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.MJ_REQUEST_DISMISS, {agree = isAgree})
end
...
function GameScene:onExitClicked_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.MJ_QUIT_TABLE)
end
