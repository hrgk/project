...
local GameOverView = require("app.games.datongzi.views.game.GameOverView")
...
function GameScene:bindSocketListeners()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        ...
        {COMMANDS.DTZ_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.DTZ_READY, handler(self, self.onReady_)},
        {COMMANDS.DTZ_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.DTZ_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.DTZ_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.DTZ_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.DTZ_CHECK_CHOU_JIANG, handler(self, self.onCheckChouJiang_)},
        {COMMANDS.DTZ_CHOU_JIANG, handler(self, self.onChouJiang_)},
        {COMMANDS.DTZ_GAME_BROADCAST, handler(self, self.onGameBroadcast_)},
        {COMMANDS.DTZ_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.DTZ_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},
    }
...
function GameScene:onEnterTransitionFinish_()
    ...
    self.canSort_ = true
    if 0 < DEBUG then
        local GameSceneTest = require("app.games.datongzi.scenes.GameSceneTest")
...
function GameScene:onWanFa_(event)
    ...
    display.getRunningScene():initWanFa(GAME_DA_TONG_ZI,date)
end
...
function GameScene:onLocationClicked_(event)
    dataCenter:sendOverSocket(COMMANDS.DTZ_REQUEST_LOCATION)
end
...
function GameScene:onQuitRoomClicked_(event)
    ...
    local function callback(bool)
        if bool then
            ...
            dataCenter:sendOverSocket(COMMANDS.DTZ_LEAVE_ROOM)
        end
    end
...
function GameScene:onRoomInfo_(event)
    ...
    if event.data.clubID and event.data.clubID > 0 then
        self.buttonZl_:show()
        self.buttonDismiss_:hide()
...
function GameScene:onInviteClicked_(event)
    ...
    local msg =  paiZhangInfo .. overScoreInfo .. overBonusInfo .. paiZhangInfo .. randomDealerInfo
    local queRenMsg = ""
    ...
    local title = "打筒子 房间号【" .. roomId .. "】" .. queRenMsg
    local description = "<" .. playerCountInfo.. "> " .. msg .. "速度来玩！"
...
function GameScene:showOnStateIdle_()
    ...
    self:showInvite_()
    if pokerTable:isOwner(selfData:getUid()) then  -- 房主才显示此按钮
        self.buttonDismiss_:show()
        self.buttonQuitRomm_:hide()
    else
        self.buttonDismiss_:hide()
    ...
    if pokerTable:getClubID() and pokerTable:getClubID() > 0 then
        self.buttonQuitRomm_:show()
        self.buttonDismiss_:hide()
    end
    self.buttonOK_:hide()
    ...
    if data.code == 0 then
        ...
        if date.totalSeat == 2 then
            ...
            if 4 == self:getTable():getMaxPlayer() then
                self.dingWeiView_ = SiRenDingWei.new(params, data.distances,true,GAME_DA_TONG_ZI):addTo(self.layerWindows_)
            elseif 3 == self:getTable():getMaxPlayer() then
                self.dingWeiView_ = SanRenDingWei.new(params, data.distances,true,GAME_DA_TONG_ZI):addTo(self.layerWindows_)
            end
...
function GameScene:onDismissClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.DTZ_OWNER_DISMISS)
        end
    end
    app:confirm(msg, callback)
end
...
function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.DTZ_REQUEST_DISMISS, {agree = isAgree})
end
