...
local BaseAlgorithm = import("app.games.datongzi.utils.BaseAlgorithm")
local DtzAlgorithm = import("app.games.datongzi.utils.DtzAlgorithm")
local TaskQueue = require("app.controllers.TaskQueue")
local PokerTable = import("app.games.datongzi.models.PokerTable")
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.DTZ_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.DTZ_ROUND_OVER, handler(self, self.onRoundOver_)},
        {COMMANDS.DTZ_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.DTZ_DEAL_CARD, handler(self, self.onDealCards_)},
        {COMMANDS.DTZ_TURN_START, handler(self, self.onTurnStart_)},
        {COMMANDS.DTZ_TURN_END, handler(self, self.onTurnEnd_)},
        {COMMANDS.DTZ_ZHA_DAN_DE_FEN, handler(self, self.onZhaDanDeFen_)},
        {COMMANDS.DTZ_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.DTZ_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.DTZ_GAME_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.DTZ_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播
        {COMMANDS.DTZ_ROUND_FLOW, handler(self, self.onRoundFlow_)},
        {COMMANDS.DTZ_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
    }
...
function TableController:doZhaDanDeFen_(data)
    ...
    player:setTongScore(data.tongScore)
    -- for i,v in ipairs(data.data) do
    --     local player = self:getPlayerBySeatID(v.seatID)
    --     player:setScore(v.totalScore)
    --     player:showZhaDanDeFen(v.score)
...
function TableController:onChuPaiEvent_(event)
    ...
    if DtzAlgorithm.isBigger(cards, self.table_:getCurCards(), config) then
        if DtzAlgorithm.isChaiZha(cards, self:getHostPlayer():getCards(), config) then
            app:confirm("您确定要拆掉炸弹吗？", function (isOk)
                if isOk then
                    dataCenter:sendOverSocket(COMMANDS.DTZ_CHU_PAI, data)
                end
            end)
            return
        end
...
function TableController:onPlayerPass_(event)
    if event.data.isReview then
        ...
        TaskQueue.add(handler(self, self.doPlayerPass_), 0.4, 0.4, event.data)
    end
end
...
function TableController:doDealCards_(data)
    ...
    if self.table_:getConfigData().showCard == 1 then
        self.hostPlayer_:setLeftCards(#data.handCards)
    else
        self.hostPlayer_:setLeftCards(-1)
    end
    ...
    for _, player in pairs(self.seats_) do
        if player:getSeatID() ~= self.hostPlayer_:getSeatID() then
            print(player:getSeatID(), leftCards)
...
function TableController:doRoomInfo(data)
    self.table_:setMaxPlayer(data.config.totalSeat)
...
function TableController:onPlayerChuPai_(event)
    ...
    if event.data.isReview then
        ...
        TaskQueue.add(handler(self, self.doPlayerChuPai_), 0, 0.5, event.data)
    end
end
...
function TableController:createSeats_(number)
    self.seats_ = {}
    local number = number or 3 -- 默认4人桌
    for i=1, number do
...
function TableController:onRoundOver_()
    for i,v in ipairs(self.seats_) do
        v:doRoundOver()
    end
end
...
function TableController:setTurnTo_(seatID, seconds)
    self.table_:onTurnTo(seatID, seconds)
