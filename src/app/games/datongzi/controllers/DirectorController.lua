...
function DirectorController:onTickEvent_(event)
    ...
    if event and checkint(event.leftSeconds) == 3 then
        ...
        if display:getRunningScene():getHostPlayer():getIndex() == event.lightIndex then
            playNotice = true
        end
...
function DirectorController:startTimeCount(seatId, seconds)
    self:show()
    self.view_:start(seatId, seconds)
end
