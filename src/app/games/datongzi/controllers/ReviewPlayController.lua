...
function ReviewPlayController:ctor(data, hostPlayer)
    ...
    self.deleCommondList_ = {
        COMMANDS.DTZ_PLAYER_ENTER_ROOM,
        COMMANDS.DTZ_ROUND_FLOW,
        COMMANDS.DTZ_DEAL_CARD,
        COMMANDS.DTZ_ROUND_OVER,
        COMMANDS.DTZ_REQUEST_DISMISS,
        COMMANDS.DTZ_ROUND_FLOW,
    }
end
...
function ReviewPlayController:filterCmd_()
    ...
    for i,k in ipairs(self.deleCommondList_) do
        for i,v in ipairs(self.gameData_) do
            if v.cmd == COMMANDS.DTZ_PLAYER_ENTER_ROOM then
                self.playerList_[v.msg.seatID] = v
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.DTZ_DEAL_CARD then
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.DTZ_REQUEST_DISMISS then
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.DTZ_ROUND_FLOW then
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.DTZ_ROOM_INFO then
                table.removebyvalue(self.gameData_, v)
