...
function PokerListView:ctor()
    self.margin_ = 40 * display.width / DESIGN_WIDTH * 0.95
...
function PokerListView:setChuPaiBtnStatus()
    ...
    local isBigger = DtzAlgorithm.isBigger(self:getSelectedCard_(), tableData:getCurCards(), self.table_:getConfigData()) --被选起来的牌、打出来的牌
...
function PokerListView:fileterSelectedPokers_(cards, pokers)
    ...
    --         self.selectedCards_ = DtzAlgorithm.getSameValue_(pokers, BaseAlgorithm.getValue(poker))
...
function PokerListView:showPokers(cards, isAnima, callback)
    ...
    local cards1 = {}
    local cards2 = {}
    cards1, cards2 = DtzAlgorithm.rank(self.sortIndex_, cards, self.table_:getConfigData())
...
function PokerListView:updatePink(pokerList)
    ...
    for k, card in ipairs(DtzAlgorithm.getPinkCards(cards, self.table_:getConfigData())) do
        for _,v in ipairs(self.pokerList_) do
...
function PokerListView:calcPokerPos_(total, index, ceng, diTotal)
    ...
    local x = 0
    offset = (1 - (14 - 1) / 2 - 1) * self.margin_ + display.cx
...
function PokerListView:removePokers(cards)
    ...
    local cards1, cards2 = DtzAlgorithm.rank(self.sortIndex_, self:getCards_(), self.table_:getConfigData())
    if cards2 then
        self.downCount_ = #cards2
    end
    self.upCount_ = #cards1
    if self.isInReView_ then
        ...
        self:upDateHanderPokerPos_(cards1,cards2)
    end
    if self.isInReView_ then
        self:show()
    end
...
function PokerListView:initTishi(clickType_)
    ...
    local tishiCards = DtzAlgorithm.tishi(curCards, cards, self.table_:getConfigData())
    if not clickType_ then
        if self.yaobuqiSprite_ then
            self.yaobuqiSprite_:removeSelf()
        end
        self.yaobuqiSprite_ = nil
        if #tishiCards == 0 then
            print("================没有比对方大的牌")
            self.yaobuqiSprite_ = display.newSprite("res/images/datongzi/game/pztsz.png"):addTo(self:getParent(),"11",1):pos(display.cx, 40)
        end
    end
...
function PokerListView:sort()
    ...
    local cards1, cards2 = DtzAlgorithm.rank(self.sortIndex_, cards, self.table_:getConfigData())
    if cards2 then
        self.downCount_ = #cards2
    end
    self.upCount_ = #cards1
...
function PokerListView:upDateHanderPokerPos_(cards1, cards2)
    self.interval_ = 0
    local timers = 0
    local setup = 0
    local diTotal = 0
