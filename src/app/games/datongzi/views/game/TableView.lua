...
local DtzCardType = import("app.games.datongzi.utils.DtzCardType")
...
tableNodes.normal = {
    ...
    children = {
        {type = TYPES.NODE, var = "nodeTableInfo_", children = {  -- 桌子信息容器
            {type = TYPES.LABEL, var = "viewProgress_", type = TYPES.CUSTOM, class = "app.games.datongzi.views.game.ProgressView"},
        ...
        {type = TYPES.CUSTOM, var = "scoreNode_", class = "app.games.datongzi.views.game.CountingScoreView"}, --计分面板
        ...
        {type = TYPES.CUSTOM, var = "directorController_", class = "app.games.datongzi.controllers.DirectorController"},
...
function TableView:updateRemainCards()
    local table = display:getRunningScene():getTable()
    if table:getCardCount() == 3 then
        if table:getPlayerCount() == 2 then
            self.viewProgress_:setRemainCards(66)
        else
            self.viewProgress_:setRemainCards(9)
        end
    elseif table:getCardCount() == 4 then
        if table:getPlayerCount() == 3 then
            self.viewProgress_:setRemainCards(88)
        else
            self.viewProgress_:setRemainCards(44)
        end
    end
end
...
local POKER_ANIMS = {
    [DtzCardType.DI_ZHA] = {{"coscosdizha"}},
    [DtzCardType.XI_ZHA] = {{"xizha"}},
    [DtzCardType.TONG_ZI] = {{"coscostongzi"}},
}
...
local POKER_ANIMS_FRAMES = {
    [DtzCardType.ZHA_DAN] = {{"sizha", "sizha%02d.png", 1, 18, 1.5, false}, {"sounds/zhadanbaozha.mp3", nil, nil, 0.7}},
}
...
function TableView:playFeiJiAnim_(index)
    ...
    local flipX = false
    -- if index == DTZ_TABLE_DIRECTION.RIGHT and 3 == display:getRunningScene():getTable():getMaxPlayer() then
    if index == DTZ_TABLE_DIRECTION.RIGHT then
        fromX, toX = display.right, display.left
        ap = display.RIGHT_CENTER
        fromY = 544
        flipX = true
    -- elseif index == DTZ_TABLE_DIRECTION.RIGHT and 2 == display:getRunningScene():getTable():getMaxPlayer() then
    --     fromY = 544
    elseif index == DTZ_TABLE_DIRECTION.LEFT then
        fromY = 544
    end
...
function TableView:onRoomConfigChanged_(event)
    ...
    self.nodeHostMaJiang_:setScale(1.2)
    self.nodeHostMaJiang_:setPositionX(-120)
    self:updateRemainCards()
end
...
function TableView:shouPaiAnim_(index)  -- 收牌
    ...
    for i=1,#nodes do
        if DTZ_TABLE_DIRECTION.BOTTOM ~= index then
...
function TableView:faceIconFly_(faceID, fromPlayer, toPlayer)
    ...
    if toIndex == DTZ_TABLE_DIRECTION.RIGHT then
