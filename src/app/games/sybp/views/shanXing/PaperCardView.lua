...
-- 原始的牌宽
local CARD_RAW_WIDTH = 94
local CARD_RAW_HEIGHT = 272
local FIXED_OPACITY = 0.2 * 255

local aCardPath = "res/images/paohuzi/a_cards/"
local bCardPath = "res/images/paohuzi/b_cards/"
local paths = {aCardPath, bCardPath}
function PaperCardView:ctor(card, cardSize, showBack, func)
    print("========setData:getCDPHZCardType()==============",setData:getCDPHZCardType())
    self.cardPath_ = paths[setData:getCDPHZCardType()]
    self.isHide = false
    self.card = card
    self.card_ = card
...
function PaperCardView:showTingTag(isShow)
	...
	if not sep then 
		sep =  display.newSprite("res/images/game/tingTag.png"):pos(-25, 125):addTo(self)
		sep:setName("tingTag")
	end
...
function PaperCardView:addFaGuang()
    ...
    self.emitterHand_:setPosVar(cc.p(42,158))
...
function PaperCardView:setChou(flag)
    ...
    if flag then
        self.chouSprite = display.newSprite(self.cardPath_.."cz.png"):addTo(self)
...
function PaperCardView:initCard_(showBack) 
    local cardname = BaseAlgorithm.getCardName(self.card_, self.cardSize_)
