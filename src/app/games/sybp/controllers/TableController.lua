...
local BaseAlgorithm = import("app.games.sybp.utils.BaseAlgorithm")
local TaskQueue = require("app.controllers.TaskQueue")
local TableModel = import("app.games.sybp.models.PokerTable")
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames { 
        {COMMANDS.SYBP_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.SYBP_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.SYBP_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.SYBP_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.SYBP_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.SYBP_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播

        {COMMANDS.SYBP_DEAL_CARD, handler(self, self.onDealCards_)},  -- 发牌
        {COMMANDS.SYBP_TIAN_HU_START, handler(self, self.onTianHuStart_)},  -- 开始天胡 
        {COMMANDS.SYBP_TIAN_HU_END, handler(self, self.onTianHuEnd_)},  -- 开始结束 
        {COMMANDS.SYBP_USER_TI, handler(self, self.onUserTi_)},  -- 玩家提 
        {COMMANDS.SYBP_USER_PAO, handler(self, self.onUserPao_)},  -- 玩家跑
        {COMMANDS.SYBP_USER_WEI, handler(self, self.onUserWei_)},  -- 玩家偎牌 
        {COMMANDS.SYBP_USER_HU, handler(self, self.onUserHu_)},  -- 胡牌
        {COMMANDS.SYBP_USER_CHI, handler(self, self.onUserChi_)},  -- 吃牌
        {COMMANDS.SYBP_USER_PENG, handler(self, self.onUserPeng_)},  -- 碰牌
        {COMMANDS.SYBP_USER_MO_PAI, handler(self, self.onPlayerMoPai_)},  
        {COMMANDS.SYBP_ALL_PASS, handler(self, self.onAllPass_)},
        {COMMANDS.SYBP_NOTIFY_HU, handler(self, self.onNotifyHu_)},  -- 通知胡牌请求
        {COMMANDS.SYBP_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
        {COMMANDS.SYBP_FIRST_HAND_TI, handler(self, self.onFirstHandTi_)},
        {COMMANDS.SYBP_HU_PASS, handler(self, self.onHuPass_)} 
    }
