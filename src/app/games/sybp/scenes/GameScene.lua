...
-- 算法测试用例
-- require ("app.games.sybp.utils.PaoHuZiTest")
...
function GameScene:onEnter_()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.SYBP_ROUND_OVER, handler(self, self.onRoundOver_)},
        {COMMANDS.SYBP_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.SYBP_GAME_START, handler(self, self.onGameStart_)},
        {COMMANDS.SYBP_READY, handler(self, self.onReady_)},
        {COMMANDS.SYBP_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.SYBP_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.SYBP_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.SYBP_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.SYBP_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.SYBP_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},
        {COMMANDS.SYBP_DEBUG_CONFIG_CARD, handler(self, self.onConfigCard_)},
    }
...
function GameScene:onEnterTransitionFinish_()
    ...
    if DEBUG > 0 then
        local GameSceneTest = require("app.games.sybp.scenes.GameSceneTest")
...
function GameScene:onWanFa_(event)
    ...
    display.getRunningScene():initWanFa(GAME_SYBP,date)
end
...
function GameScene:onDingWeiClicked_(event)
    dataCenter:sendOverSocket(COMMANDS.SYBP_REQUEST_LOCATION)
end
...
function GameScene:showLocationTips_(data,iskaiju)
    ...
    if data.code == 0 then
        ...
        if date.rules.totalSeat == 2 then
            ...
            self.dingWeiView_ = SanRenDingWei.new(params,data.distances,iskaiju,GAME_SYBP):addTo(self.layerWindows_)
        end 
    end
end
...
function GameScene:onQuitRoomClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.SYBP_LEAVE_ROOM)
        end
    end
...
function GameScene:onDismissClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.SYBP_OWNER_DISMISS)
        end
    end
    app:confirm(msg, callback)
end
...
function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.SYBP_REQUEST_DISMISS, {agree = isAgree})
end
