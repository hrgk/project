...
local data1 = {
    type = TYPES.ROOT,
    children = {
        {type = TYPES.SPRITE, scale9 = true, filename = "#review_bg.png", size = bgSize, var = "spriteBg_", children = {
...
function ReviewActionView:lightByActions_(actions)
    ...
    if table.indexof(actions, CSMJ_ACTIONS.PENG) then
        self.spritePeng_:show()
    end
    if table.indexof(actions, CSMJ_ACTIONS.CHI) then
        self.spriteChi_:show()
    end
    if table.indexof(actions, CSMJ_ACTIONS.AN_GANG) or 
        table.indexof(actions, CSMJ_ACTIONS.CHI_GANG) or 
        table.indexof(actions, CSMJ_ACTIONS.BU_GANG) then
        self.spriteGang_:show()
    end
    if table.indexof(actions, CSMJ_ACTIONS.CHI_HU) or 
        table.indexof(actions, CSMJ_ACTIONS.ZI_MO) then
        self.spriteHu_:show()
    end
end
...
function ReviewActionView:adjustShape_(index)
    if index == MJ_TABLE_DIRECTION.BOTTOM then
...
function ReviewActionView:setHandIndex_(direction, index)
    local nx, ny, ox, oy
    if direction == MJ_TABLE_DIRECTION.LEFT or direction == MJ_TABLE_DIRECTION.RIGHT then
...
function ReviewActionView:showHand_(direction, data)
    local index = 6
    if CSMJ_ACTIONS.CHI == data.actType then
        index = 1
    elseif CSMJ_ACTIONS.PENG == data.actType then
        index = 2
    elseif table.indexof({CSMJ_ACTIONS.CHI_GANG, CSMJ_ACTIONS.AN_GANG, CSMJ_ACTIONS.BU_GANG}, data.actType) then
        index = 3
    elseif table.indexof({CSMJ_ACTIONS.CHI_HU, CSMJ_ACTIONS.ZI_MO}, data.actType) then
        index = 5
    end
