...
local nodes = {
    ...
    children = {
        ...
        {type = TYPES.LABEL, var  = "infoLabel_", ap = {0, 0.5}, x = 2, y = 50, options={text="", 
        size = 16, font = DEFAULT_FONT, color = cc.c3b(255, 255, 255)}},
...
function GameOverView:ctor(data, closeHandler)
    ...
    dump(data.ruleString,"data.ruleString")
    ...
    local ItemView = require("app.games.csmj.views.game.GameOVerItemView")
    for _,v in pairs(checktable(data.seats)) do
        local x = display.cx + (v.seatID - 2) * space - space / 2
        ItemView.new(v, data.roomInfo):addTo(self):pos(x, display.cy - 40)
    end
...
function GameOverView:onShareClicked_(event)
    ...
    display.getRunningScene():gameShareWeiXin("长沙麻将","",callback,display.getRunningScene():getTable():getTid(),selfData:getUid(), self.recordId)
end
...
function GameOverView:onShareImageClicked_()
    ...
    display.captureScreen(function (bSuc, filePath)
        ...
        display.getRunningScene():shareImage(2,"长沙麻将","",0, callback, filePath)
