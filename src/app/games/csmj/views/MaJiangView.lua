...
function MaJiangView:ctor(card, direction, isDown, isMax)
    ...
    self.isLaizi_ = dataCenter:getIsLaiZi()
...
function MaJiangView:setMaJiang(card, isDown)
    ...
    self:changeFixed_(isDown)
    return self:create_()
end

function MaJiangView:setFlag(flag)
    self.flag_ = flag
end

function MaJiangView:getFlag()
    return self.flag_
end
...
function MaJiangView:create_()
    ...
    if not self.isDown_ and self.direction_ ~= CSMJ_TABLE_DIRECTION.BOTTOM then
        return self:createBack_()
    end
...
function MaJiangView:createFront_()
    local path = "#res/images/majiang/majiang/%s/%s_%s_%d.png"
    ...
    path = string.format(path, col1, col2, col3, value)
...
function MaJiangView:createBack_()
    local path = "#res/images/majiang/majiang/mjbg%s_%s.png"
    local col1 = 'd' -- 倒着的牌
    local col2 = self:getDirectionName_()
    if not self.isDown_ then
        col1 = "s"  -- 竖着的牌
        -- assert(1 ~= self.direction_)
        if CSMJ_TABLE_DIRECTION.BOTTOM == self.direction_ then   -- 竖着的牌没有背面的
    ...
    path = string.format(path, col1, col2)
    -- print("=======path========")
    self.spriteContent_ = display.newFilteredSprite(path):addTo(self)
    return self
end
...
function MaJiangView:changeFixed_(isDown)
    isDown = isDown or false
    if self.fixedSprite_ then
        self.fixedSprite_:setVisible(not isDown)
    end
    if self.fixedSprite1_ then
        self.fixedSprite1_:setVisible(isDown)
    end
end
...
function MaJiangView:showMajiang(card, delay)
