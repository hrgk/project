...
local ZZAlgorithm = require("app.games.csmj.utils.ZZAlgorithm")
...
Player.AFTER_GANG_PAI_ADDED = "AFTER_GANG_PAI_ADDED"
...
Player.TAN_PAI_ONLY_DATA = "TAN_PAI_ONLY_DATA"
Player.GANG_LOCK = "GANG_LOCK"
Player.ON_MING_BU_EVENT = "ON_MING_BU_EVENT"
Player.CHU_PAI_COUNT = "CHU_PAI_COUNT"
Player.HIDE_PLAYER_INFO = "HIDE_PLAYER_INFO" 
-- 定义属性
...
Player.schema['isTanPai'] = {'boolean', false}  -- 是否锤
...
Player.schema['tingPai'] = {"table", {}} -- 听牌
...
function Player:setStates_()
    local defaultEvents = {
        ...
        {name = "anBu", from = {"public_thinking", "thinking"}, to = "wait_call"},
        {name = "gongBu", from = {"public_thinking", "thinking"}, to = "wait_call"},
        {name = "mingBu", from = {"public_thinking", "thinking"}, to = "wait_call"},
...
function Player:checkTingPai()
    local isTingPai = CSAlgorithm.isTingPai(self.waiPai_, self.cards_)
...
function Player:doPlayAction(data)
    if table.indexof({CSMJ_ACTIONS.CHI, CSMJ_ACTIONS.PENG, CSMJ_ACTIONS.CHI_GANG}, data.actType) ~= false then
...
function Player:onActionChi_(data)
    ...
    self:addWaiPai({action = CSMJ_ACTIONS.CHI, dennyAnim = data.dennyAnim, cards = data.cards})
...
function Player:onActionPeng_(data)
    ...
    self:addWaiPai({action = CSMJ_ACTIONS.PENG, dennyAnim = data.dennyAnim, cards = {card, card, card}})
    -- if self.isTanPai_ then
    --     self:showTanPaiData()
    -- end
end
...
function Player:onActionGang_(data)
    ...
    if action == CSMJ_ACTIONS.AN_GANG then
        ...
        elseif  action == CSMJ_ACTIONS.BU_GANG then
        self:onActionBuGang_(data)
        elseif  action == CSMJ_ACTIONS.CHI_GANG then
        self:onActionChiGang_(data)
    end
    self:onGangLock_(true)
    
end

function Player:onGangLock_(isLock)
    self.isLocked_ = isLock
    self:dispatchEvent({name = Player.GANG_LOCK, isLock = isLock})
end
...
function Player:onActionBuGang_(data)
    ...
    for i,v in ipairs(checktable(self.waiPai_)) do
        if v.action == CSMJ_ACTIONS.PENG and v.cards[1] == card then
            v.action = CSMJ_ACTIONS.BU_GANG
...
function Player:onActionAnGang_(data)
    ...
    local card = data.card
    if not self:isHostPlayer() and self:isInGangState() then
        for i=1,3 do  -- 删除对应的牌张
            self:removeCardFromHand_(card)
        end
    else
    ...
    self:setInGangState(false)
end

function Player:onActionTanPai(data, isReview, noShowAni)
    if data.cards then
        ...
        dump(self.tanPai_,"self.tanPai_self.tanPai_")
        self:dispatchEvent({name = Player.TAN_PAI_ADDED, data = data, isReview = isReview, index = 1, huNameList = huNameList,noShowAni = noShowAni})
    end
end

function Player:showTanPaiData()
    if self.isTanPai_ then
        self:dispatchEvent({name = Player.TAN_PAI_ONLY_DATA, cards = self.tanPai_[1].cards})
    end
end
...
function Player:resumeTanPai(isReview)
    print("isReview")
    print("self.isTanPai_",self.isTanPai_)
    if not self.isTanPai_ then
        return
    end
...
function Player:onActionHu_(data)
    ...
    data.action = action
    if action == CSMJ_ACTIONS.CHI_HU then
        self:onActionChiHu_(data)
        elseif  action == CSMJ_ACTIONS.ZI_MO then
        self:onActionZiMo_(data)
        elseif  action == CSMJ_ACTIONS.QIANG_GANG_HU then
...
function Player:beQiangGang(card)
    for _,v in ipairs(self.waiPai_) do
        if v.action == CSMJ_ACTIONS.BU_GANG and v.cards and v.cards[1] == card then
            v.action = CSMJ_ACTIONS.PENG
...
function Player:beTurnTo(seconds, isReview)
...
function Player:onRoundStart_(event)
    ...
    self.isTanPai_ = false
    self.cardsBeforeTanPai_ = {}
    local isReConnect = false
    self:onGangLock_(false)
    self.tanPai_ = {}
    if event.args then
    ...
    if #fuTb > 0 then
        ...
        for i=1, #fuTb do
            if fuTb[i][1] == self.setOffline then
                offlineIndex = i
            elseif fuTb[i][1] == self.setScore then
                fuTb[i][1](self, fuTb[i][2],true)
            else
...
function Player:setScore(score, isReConnect)
    local rawScore = self.score_
    self.score_ = score
    self:dispatchEvent({name = Player.SCORE_CHANGED, score = score, from = rawScore, isReConnect = isReConnect})
end
...
function Player:calcAnGang()
    ...
    for _,v in pairs(cards) do
        if v ~= BaseAlgorithm.NAI_ZI and 4 == table.removebyvalue(testCards, v, true) then
            table.insert(result, {action = CSMJ_ACTIONS.AN_GANG, cards = {v, v, v, v}})
        end
    end
    return result
end
...
function Player:calcBuGang()
    ...
    for _,v in ipairs(list) do
        if CSMJ_ACTIONS.PENG == v.action and v.cards[1] == self.lastCard_ then
            local cards = {self.lastCard_, self.lastCard_, self.lastCard_, self.lastCard_}
            table.insert(result, {action = CSMJ_ACTIONS.BU_GANG, cards = cards, highLight = 1})
        end
    end
    return result
end
...
-- 设置出牌
function Player:setChuPai(cards)
    self.chuPai_ = cards or {}
    self:dispatchEvent({name = Player.CHU_PAI_CHANGED, cards = cards})
end
...
-- 添加出牌
function Player:addAfterGangPai(cards, dennyAnim)
    assert(cards and #cards > 0)
    for k,card in pairs(cards) do
        table.insert(self.chuPai_, card) 
    end
    self:dispatchEvent({name = Player.AFTER_GANG_PAI_ADDED, cards = cards, dennyAnim = dennyAnim})
end

function Player:setInGangState(bool)
    self.inGang_ = bool
end

function Player:isInGangState()
    return self.inGang_
end

function Player:onActionCSMJBu_(data)
    print("Player onActionBU")
    local action = data.act
    data.action = action
    if action == CSMJ_ACTIONS.AN_BU then
        self:onActionAnBu_(data)
        elseif  action == CSMJ_ACTIONS.MING_BU then
        self:onActionMingBu_(data)
        elseif  action == CSMJ_ACTIONS.GONG_BU then
        self:onActionGongBu_(data)
    end
    -- if self.isTanPai_ then
    --     self:showTanPaiData()
    -- end
...
function Player:onActionMingBu_(data)
    self:tryDoEvent_("mingBu")
    local card = data.card
    self:removeCardFromHand_(card)
    for i,v in ipairs(checktable(self.waiPai_)) do
        if v.action == CSMJ_ACTIONS.PENG and v.cards[1] == card then
            ...
            table.insert(v.cards, card)
            self:setWaiPai(self.waiPai_)
            self:dispatchEvent({name = Player.ON_MING_BU_EVENT})
            break
        end
    end
end

function Player:onActionGongBu_(data)
    self:tryDoEvent_("gongBu")
    local card = data.card
    for i=1,3 do  -- 删除对应的牌张
        self:removeCardFromHand_(card)
    end
...
function Player:onActionAnBu_(data)
    self:tryDoEvent_("anBu")
    local card = data.card
    for i=1,4 do  -- 删除对应的牌张
        self:removeCardFromHand_(card)
    end
...
function Player:removeCardFromHand_(card)
    ...
    if self.cardsBeforeTanPai_ and 0 >= table.removebyvalue(self.cardsBeforeTanPai_, card, false) then
        table.removebyvalue(self.cardsBeforeTanPai_, 0, false)
    end
    -- if self.tanPai_ and self.tanPai_[1] and self.tanPai_[1].cards then
    --     if 0 >= table.removebyvalue(self.tanPai_[1].cards, card, false) then
    --         table.removebyvalue(self.tanPai_[1].cards, 0, false)
    --     end
    -- end
...
function Player:addMaJiang(card, isDown)
    ...
    if self.isTanPai_ and self.cardsBeforeTanPai_ then
        table.insert(self.cardsBeforeTanPai_, card)
    end
    local event = {
...
function Player:doCardsEvent_(eventName, cards, isDown, action)
    ...
    local card
    -- if action == CSMJ_ACTIONS.CHI_HU then
    --     local lastCards = display.getRunningScene():getTable():getLastCard()
    --     card = lastCards[1]
    -- end
...
function Player:hidePlayerInfo_()
    self:dispatchEvent({name = Player.HIDE_PLAYER_INFO})
end
