...
local MaJiangTable = import("app.games.csmj.models.MaJiangTable")
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames { 
        {COMMANDS.CS_MJ_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.CS_MJ_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.CS_MJ_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.CS_MJ_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.CS_MJ_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.CS_MJ_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播
        {COMMANDS.CS_MJ_PUBLIC_TIME, handler(self, self.onPublicTime_)},  -- 公共牌时间

        {COMMANDS.CS_MJ_DEAL_CARD, handler(self, self.onDealCards_)},  -- 发牌
        -- {COMMANDS.CS_MJ_USER_GANG, handler(self, self.onUserGang_)},  -- 玩家提 
        {COMMANDS.CS_MJ_USER_HU, handler(self, self.onUserHu_)},  -- 胡牌
        {COMMANDS.CS_MJ_USER_CHI, handler(self, self.onUserChi_)},  -- 吃牌
        {COMMANDS.CS_MJ_USER_PENG, handler(self, self.onUserPeng_)},  -- 碰牌
        {COMMANDS.CS_MJ_USER_MO_PAI, handler(self, self.onPlayerMoPai_)},  
        {COMMANDS.CS_MJ_NOTIFY_HU, handler(self, self.onNotifyHu_)},  -- 通知胡牌请求
        {COMMANDS.CS_MJ_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
        -- -- {COMMANDS.CS_MJ_PLAY_ACTION, handler(self, self.onPlayAction_)},
        {COMMANDS.CS_MJ_UPDATE_SCORE, handler(self, self.onUpdateScore_)},

        {COMMANDS.CS_MJ_USER_AFTER_GANG, handler(self, self.onCSMJUserAfterGang_)},
        {COMMANDS.CS_MJ_PLAYER_SHOW_CARDS, handler(self, self.onCSTanPai_)},
        {COMMANDS.CS_MJ_USER_AN_GANG, handler(self, self.onUserGang_)},
        {COMMANDS.CS_MJ_USER_MING_GANG, handler(self, self.onUserGang_)},
        {COMMANDS.CS_MJ_USER_GONG_GANG, handler(self, self.onUserGang_)},
        {COMMANDS.CS_MJ_USER_BU_CARD, handler(self, self.onUserBu_)},
        {COMMANDS.CS_MJ_USER_MING_BU, handler(self, self.onUserBu_)},
        {COMMANDS.CS_MJ_USER_GONG_BU, handler(self, self.onUserBu_)},
        {COMMANDS.CS_MJ_HAI_DI, handler(self, self.onMJHaiDi_)},
        {COMMANDS.CS_MJ_PLAYER_OPERATES, handler(self, self.onMJPlayerOperate_)},
        {COMMANDS.CS_MJ_GET_ALL_PLAYER_CARDS, handler(self, self.onGetAllPlayerCards_)},

    }
    gailun.EventUtils.create(self, dataCenter, self, handlers)
    
    self.doFlowList_ = {
        [CS_MJ_T_IN_IDLE] = handler(self, self.doFlowInIDLE_),
        [CS_MJ_T_IN_CHU_PAI] = handler(self, self.doFlowInChuPai_),
        [CS_MJ_T_IN_PUBLIC_OPRATE] = handler(self, self.doFlowInChuPaiCall_),
        [CS_MJ_T_IN_MO_PAI] = handler(self, self.doFlowInMoPai_),
        [CS_MJ_T_IN_MO_PAI_CALL] = handler(self, self.doFlowInMoPaiCall_),
        [CS_MJ_T_IN_MING_GANG_PAI_CALL] = handler(self, self.doFlowInQiangGangHuCall_),
        [CS_MJ_T_IN_GONG_GANG_PAI_CALL] = handler(self, self.doFlowInQiangGangHuCall_),
        [CS_MJ_T_IN_AN_GANG_PAI_CALL] = handler(self, self.doFlowInQiangGangHuCall_),
        -- [CS_CS_MJ_T_IN_GANG_PAI_CALL] = handler(self, self.doFlowInQiangGangHuCall_),
        [CS_MJ_T_IN_OTHER_GANG_PAI_CALL] = handler(self, self.doFlowInQiangGangHuCall_),
        [CS_MJ_T_IN_WILL_BEGIN_OPTION] = handler(self, self.doFlowInBeginOptionCall_),
        [CS_MJ_T_IN_HAI_DI] = handler(self, self.doFlowInHaidiChoosCall_),
        [CS_MJ_T_IN_HAI_DI_CALL] = handler(self, self.doFlowInHaiDiCall_),
...
function TableController:setPlayerCard(seatID,cards)
    ...
    player:setChuPai(cards)
    player:hidePlayerInfo_()
end
...
function TableController:onChuPaiEvent_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.CS_MJ_CHU_PAI, data)
end
    ...
    if data.seatID == self.hostSeatID_ then
        ...
        if player:isLocked() and self.currentMoCard_ > 0 then
            self.tableView_:performWithDelay(function()
                dataCenter:sendOverSocket(COMMANDS.CS_MJ_CHU_PAI, {
...
function TableController:getHuInfo()
    ...
    local configdata = self:getTable():getConfigData()
    return {isTongPao = self.isTongPao_, isSortBird = configdata.ruleDetails.birdType, huInfo = self.huInfo_}
end
...
function TableController:doTurnTo_(data)
    if data.outCards ~= nil then
        ...
        if not isRight then
            dataCenter:sendOverSocket(COMMANDS.CS_MJ_GET_ALL_PLAYER_CARDS)
        end
    end
    -- dump(data, "doTurnTo_")
    local seatID = data.seatID
