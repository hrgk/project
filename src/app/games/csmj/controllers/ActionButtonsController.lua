...
function ActionButtonsController:onEnter()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.CS_MJ_CHU_PAI, handler(self, self.onPlayerChuPai_)},  -- 某人出牌
...
function ActionButtonsController:onChiClicked_(event)
    ...
    if #chidata == 1 then
        ...
        if #operateCards == 1 then
            ...
            print("chi send card:")
            dump(data)
            dataCenter:sendOverSocket(COMMANDS.CS_MJ_USER_CHI, data)
...
function ActionButtonsController:onPengClicked_(event)
    ...
    if #pengdata == 1 then
        ...
        dataCenter:sendOverSocket(COMMANDS.CS_MJ_USER_PENG, data)
        self.player_:stopZhuanQuanAction()
        return
    end
    ...
    dump(self.data,7,7)
     for k,v in pairs(self.data) do
      ...
      if #cards == 1 then
        ...
        local command = COMMANDS.CS_MJ_USER_AN_GANG
        if types[1] == CSMJ_ACTIONS.CHI_GANG then
            command = COMMANDS.CS_MJ_USER_GONG_GANG
        end
        if types[1] == CSMJ_ACTIONS.BU_GANG then
            command = COMMANDS.CS_MJ_USER_MING_GANG
        end
...
function ActionButtonsController:onBuClicked_(event)
    ...
    if #cards == 1 then
        local data = {card = cards[1], operateCards = {cards[1], cards[1]}}
        local command = COMMANDS.CS_MJ_USER_BU_CARD
        if types[1] == CSMJ_ACTIONS.GONG_BU then
            command = COMMANDS.CS_MJ_USER_GONG_BU
        end
        if types[1] == CSMJ_ACTIONS.MING_BU then
            command = COMMANDS.CS_MJ_USER_MING_BU
        end
        dataCenter:sendOverSocket(command, data)
        self.player_:stopZhuanQuanAction()
        return
...
function ActionButtonsController:onZiMouClicked_(event)
    print("onHU8Clicked_:")
    local data = {}
    dataCenter:sendOverSocket(COMMANDS.CS_MJ_USER_HU, data)
    self.player_:stopZhuanQuanAction()
end
...
function ActionButtonsController:showOperates(operates, gameType)
    ...
    self.gameType_ = gameType
    self:show()
    self.view_:showOperates(self.operates, self.data)
end
...
function ActionButtonsController:sendPlayAction_(action, params)
    ...
    dataCenter:sendOverSocket(COMMANDS.CS_MJ_PLAY_ACTION, data)
    self.player_:stopZhuanQuanAction()
end
...
function ActionButtonsController:onPassClicked_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.CS_MJ_PLAYER_PASS, data)
    self.player_:stopZhuanQuanAction()
