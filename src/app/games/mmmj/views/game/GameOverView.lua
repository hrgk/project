...
function GameOverView:ctor(data, closeHandler)
    ...
    local ItemView = require("app.games.mmmj.views.game.GameOVerItemView")
...
function GameOverView:onShareClicked_(event)
    ...
    display.getRunningScene():gameShareWeiXin("摸摸麻将","",callback,display.getRunningScene():getTable():getTid(),selfData:getUid(), self.recordId)
end
...
function GameOverView:onShareImageClicked_()
    ...
    display.captureScreen(function (bSuc, filePath)
        ...
        display.getRunningScene():shareImage(2,"摸摸麻将","",0, callback, filePath)
