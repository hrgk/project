...
-- require ("app.games.mmmj.utils.testCsmj")
...
function GameScene:onEnter_()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        ...
        {COMMANDS.MMMJ_ENTER_ROOM, handler(self, self.onCSMJEnterRoom_)},
        {COMMANDS.MMMJ_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.MMMJ_READY, handler(self, self.onReady_)},
        {COMMANDS.MMMJ_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.MMMJ_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.MMMJ_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.MMMJ_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.CHANGE_SERVER, handler(self, self.onChangeServer_)},
        {COMMANDS.MMMJ_DEBUG_CONFIG_CARD, handler(self, self.onConfigCard_)},
        {COMMANDS.MMMJ_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.MMMJ_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},
        {COMMANDS.MMMJ_SHOW_BIRDS, handler(self,self.onShowBirds_)},
        {COMMANDS.MMMJ_SHOW_BANBANHU, handler(self,self.onShowBanBanHu_)},
        {COMMANDS.MMMJ_PLAYER_PIAO_FEN, handler(self,self.onPiaoFenHandler_)},
        {COMMANDS.MMMJ_PIAO_FEN_VALUE, handler(self,self.showPiaoResult_)},
    ...
    for i = 1,#fenConf do
        ...
        btn:addTouchEventListener(function(sender, event) 
            if event == ccui.TouchEventType.ended then
                dataCenter:sendOverSocket(COMMANDS.MMMJ_PIAO_FEN_VALUE, {
                    score = fenConf[i]
                })
                self:piaoFenSelectHide_() 
            end
...
function GameScene:onEnterTransitionFinish_()
    ...
    if DEBUG > 0 then
        local GameSceneTest = require("app.games.mmmj.scenes.GameSceneTest")
...
function GameScene:onWanFa_(event)
    ...
    display.getRunningScene():initWanFa(GAME_MMMJ,date)
end
...
function GameScene:onInviteClicked_(event)
    ...
    local title = "摸摸麻将" .. " 房间号【" .. roomId .. "】" .. queRenMsg
...
function GameScene:onNotifyLocation_(event)
    ...
    if event.data.code == 0 then
        ...
        elseif 4 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SiRenDingWei.new(params,event.data.distances,true,GAME_MMMJ):addTo(self.layerWindows_)
        elseif 3 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,true,GAME_MMMJ):addTo(self.layerWindows_)
        end
...
function GameScene:onRequestLocation_(event)
    ...
    if event.data.code == 0 then
        ...
        if self:getTable():getRuleDetails().totalSeat == 2 then
            self.buttonStart_:show()
        elseif 4 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SiRenDingWei.new(params,event.data.distances,false,GAME_MMMJ):addTo(self.layerWindows_)
        elseif 3 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,false,GAME_MMMJ):addTo(self.layerWindows_)
        end
...
function GameScene:onDismissClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.MMMJ_OWNER_DISMISS)
        end
    end
    app:confirm(msg, callback)
end
...
function GameScene:onDingWeiClicked_(event)
    dataCenter:sendOverSocket(COMMANDS.MMMJ_REQUEST_LOCATION)
end
...
function GameScene:onExitClicked_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.MMMJ_QUIT_TABLE)
end

function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.MMMJ_REQUEST_DISMISS, {agree = isAgree})
end
