...
function ZZAlgorithm.canChiHu(cards, addCards, allowSevenPairs)
    ...
    for i,v in _ipairs(addCards) do
        table.insert(cards, v)
    end
    local hzCount = BaseAlgorithm.calcValueCount_(cards, BaseAlgorithm.NAI_ZI)
    if allowSevenPairs and ZZAlgorithm.isSevenPairs_(cards, hzCount) then
        return true
    end
    return ZZAlgorithm.canHu_(cards, hzCount)
end
...
-- 判断胡牌的总循环
function ZZAlgorithm.canHu_(cards, hzCount, naiZi)
    naiZi = naiZi or BaseAlgorithm.NAI_ZI
...
function ZZAlgorithm.isSevenPairs_(cards, nai_zi)
    -- """判断是否7小对"""
