...
local PlayAnim = require("app.games.hhqmt.views.PlayAnim")
...
local player_pos = {
    ...
    {display.fixLeft+player_pos_adjustx, display.height - player_pos_adjusty+85},
}
...
local HUXI_POS = {
    {player_pos[1][1] + 65, player_pos[1][2]-70},
    {player_pos[2][1] - 220+5, player_pos[2][2] -90+80},
    {player_pos[3][1] + 62,  player_pos[3][2] -90+80},
}
...
local nodeData = {
    ...
    children = {
        {type = TYPES.NODE, var = "nodePlayer_", children = {  -- 玩家基本数据的对象
            ...
            {type = TYPES.LABEL_ATLAS, var = "labelScore_", filename = "fonts/game_score.png", options = {text="", w = 14, h = 35, startChar = "-"}, y = - 65, x = 30, ap = {0, 0.5}},
            -- }},
            {type = TYPES.LABEL, var = "scoreName_", options = {text="积分:", size=26, font = DEFAULT_FONT, align=cc.TEXT_ALIGNMENT_CENTER, color=cc.c3b(255, 255, 255)}, y = -60, x = 0, ap = {1, 0.5}},
        ...
        {type = TYPES.SPRITE, var = "nodehuxi",children = { 
            {type = TYPES.NODE, var = "huxiBg_", filename = "res/images/game/tablehuxibg.png", scale9 = true, size = {190, 50}, children = {
                {type = TYPES.LABEL, var = "labelzhuomianhuxi_", options = {text="胡息:", size=26, font = DEFAULT_FONT, color=cc.c4b(255, 255, 255, 128) },ap = {0.5, 0.5}, ppx = 0.25, ppy = 0.5},
...
function PlayerView:initPosByIndex_()
    ...
    local huxiX = HUXI_X
    if self.index_ == TABLE_HHQMT_DIRECTION.BOTTOM or self.index_ == TABLE_HHQMT_DIRECTION.LEFT then
    ...
    elseif self.index_ == TABLE_HHQMT_DIRECTION.RIGHT then
        zhuangX = - DEALER_X
        self.spriteZhuang_:setPositionX(zhuangX)
    ...
    self.labelScore_:setPosition(scorePosInfo[self.index_][1], scorePosInfo[self.index_][2]-5)
end
...
function PlayerView:calcWaiPaiPos_(index, offset)
    if TABLE_HHQMT_DIRECTION.BOTTOM == self.index_ then
...
function PlayerView:doWeiPai(card,isChou,isReConnect,inFastMode)
    ...
    transition.execute(tmpCard, cc.FadeOut:create(0), {
        ...
        onComplete = function()
            ...
            local mtime = 0.5 
            local anix,aniy = self:calcZhuomianPaiAniPos_()
            local actionDelay = cc.DelayTime:create(0.1)
            local action1 = cc.MoveTo:create(mtime, cc.p(anix,aniy))
            local action2 = cc.ScaleTo:create(mtime, 0.1)    
            local action3 = cc.FadeOut:create(mtime)
            local action4 =   cc.CallFunc:create(function ()
