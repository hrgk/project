...
local testData = require("app.games.hhqmt.data.test")

local HuangAnim = require("app.games.hhqmt.views.HuangAnim")
...
tableNodes.normal = {
    ...
    children = {
        ...
        {type = TYPES.CUSTOM, var = "directorController_", class = "app.games.hhqmt.controllers.DirectorController"},
        ...
        {type = TYPES.CUSTOM, var = "progressView_", class = "app.games.hhqmt.views.game.ProgressView"}, -- 主玩家的手牌容器，需要最高等级的显示
...
local PuPingCardList = import("app.games.hhqmt.views.pingPu.PaperCardList")
local ShanXingCardList = import("app.games.hhqmt.views.shanXing.PaperCardList")
-- "app.games.hhqmt.views.pingPu.PaperCardList"
local paiXingList = {}
...
function TableView:ctor(table)
    ...
    local debugView = DebugView.new(COMMANDS.HHQMT_DEBUG_CONFIG_CARD, testData)
...
function TableView:onCurrCardChange_(event)
    ...
    if not event.card  or event.card == 0 then
        ...
        self:clearNodeMoPaiDaPai_()
        if TABLE_HHQMT_DIRECTION.BOTTOM == event.index then
        ...
        elseif TABLE_HHQMT_DIRECTION.RIGHT == event.index then   
...
function TableView:calcRoundOverShowPokerPos_(total, i, j, postion, playerX, playerY,perkan)
    if TABLE_HHQMT_DIRECTION.LEFT == postion then
        return self:calcRoundOverPokerPosLeft_(total, i, j, playerX, playerY,perkan)
    elseif TABLE_HHQMT_DIRECTION.RIGHT == postion then
...
function TableView:calcRoundOverHulistPos_(index,total,hutotal,winner)
    if winner then
        if TABLE_HHQMT_DIRECTION.LEFT == winner:getIndex() then
            return self:calcRoundOverHulistPosLeft_(index,total,hutotal,winner)
        elseif TABLE_HHQMT_DIRECTION.RIGHT == winner:getIndex() then
...
function TableView:playBomb_(cardType, index, scale)
    ...
    if index == TABLE_HHQMT_DIRECTION.RIGHT then
        x, y = display.width * 0.78, 500
    elseif index == TABLE_HHQMT_DIRECTION.LEFT then
...
function TableView:faceIconFly_(faceID, fromPlayer, toPlayer)
    ...
    if toIndex == TABLE_HHQMT_DIRECTION.LEFT then
...
function TableView:makeRuleString(spliter)
    if self.configData_ then
        ...
        if data.ruleType == 3 then
            table.insert(list, "全名堂")
        elseif data.ruleType == 1 then
            table.insert(list, "红黑点")
        end
        table.insert(list, (data.rules.totalSeat or 3) .. "人")
        if data.rules.limitScore == 0 then
            table.insert(list, "不封顶")
        else
            table.insert(list, "封顶" .. checknumber(data.rules.limitScore))
        end
        -- if data.rules.baseTun == 1 then
        --     table.insert(list, "充屯无")
        -- else
        --     table.insert(list, "充屯" .. checknumber(data.rules.baseTun - 1))
        -- end
...
function TableView:makeAddRuleString(spliter)
    if self.configData_ then
        ...
        local list = {}
        if data.ruleType == 3 then
            if data.rules.santi5kan == 1 then
                table.insert(list, "3提5坎")
            end
            if data.rules.hangHangXi == 1 then
                table.insert(list, "行行息")
            end
            if data.rules.jiaHangHang == 1 then
                table.insert(list, "假行行")
            end
            if data.rules.siQiHong == 1 then
                table.insert(list, "四七红")
            end
            if data.rules.daTuanYuan == 1 then
                table.insert(list, "大团圆")
            end
        elseif data.ruleType == 1 then
            if data.rules.duiDuiHu == 1 then
                table.insert(list, "对子胡")
            end
        end
        return table.concat(list, spliter)
    end
...
function TableView:onFaPaiAction_(index,seatID)
    ...
    for i=1,count do
        ...
        self:performWithDelay(function ()
            if TABLE_HHQMT_DIRECTION.BOTTOM == index then
...
function TableView:calcFaPaiPokerPos_(total, index)
    if TABLE_HHQMT_DIRECTION.LEFT == self.index_ then
        return self:calcFaPaiPokerPosLeft_(total, index)
    elseif TABLE_HHQMT_DIRECTION.RIGHT == self.index_ then
