...
local RoundOverMingTangItem = import("app.games.hhqmt.views.game.RoundOverMingTangItem")
local RoundOverKan = import("app.games.hhqmt.views.game.RoundOverKan")
...
function CDPHZRoundOver:initLabelInfo_(data)
    ...
    if data.winInfo.fanList and #data.winInfo.fanList > 0 then
        ...
        fan = fan + data.winInfo.huangZhuangCount
    else
        fan = 1 + data.winInfo.huangZhuangCount
    end
    self.fan_:setAnchorPoint(0.5,0.5)
    self.fan_:pos(495,124.22)
    ...
    self.jiFen_:pos(495,-53.45)
    if data.winInfo.huangZhuangCount > 0 then
        self.wzcsNum_ = gailun.uihelper.createBMFontLabel(fanFnt):addTo(self.csbNode_)
        self.wzcsNum_:setString(data.winInfo.huangZhuangCount)
        self.wzcsNum_:setAnchorPoint(0.5,0.5)
        self.wzcsNum_:pos(495,-144.78)
        self.wzcs_:show()
    end
end
...
function CDPHZRoundOver:initMingTang_(info, winType)
    ...
    if info.huangZhuangCount > 0 then
        local item = RoundOverMingTangItem.new(53, winType):addTo(self.fanContent_)
        item:haungZhang()
        table.insert(items, item)
    end
...
function CDPHZRoundOver:loaderCsb()
    self.csbNode_ = cc.uiloader:load("views/games/hhqmt/RoundOver.csb"):addTo(self)
