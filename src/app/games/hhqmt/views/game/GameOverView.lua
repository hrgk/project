...
function GameOverView:loaderCsb()
    self.csbNode_ = cc.uiloader:load("views/games/hhqmt/GameOver.csb"):addTo(self)
...
function GameOverView:shareHandler_()
    ...
    display.getRunningScene():gameShareWeiXin("怀化全名堂","",callback,display.getRunningScene():getTable():getTid(),selfData:getUid(), self.recordId)
end
...
function GameOverView:share2Handler_()
    ...
    display.captureScreen(function (bSuc, filePath)
        ...
        display.getRunningScene():shareImage(2,"怀化全名堂","",0, callback, filePath)
