...
function ActionButtonsController:sendPlayAction_(action, params)
    ...
    dataCenter:sendOverSocket(COMMANDS.HHQMT_PLAY_ACTION, data)
end

function ActionButtonsController:onGuoClicked_(event)
    local data = {}
    dataCenter:sendOverSocket(COMMANDS.HHQMT_PLAYER_PASS, data)
end
...
function ActionButtonsController:doHuPass(data)
    local function doPass(isOK)    
        if isOK then
            local data = {}
            dataCenter:sendOverSocket(COMMANDS.HHQMT_HU_PASS, data)
        end
    end
...
function ActionButtonsController:onChiClose(chi_path)
    if chi_path then
        ...
        dataCenter:sendOverSocket(COMMANDS.HHQMT_USER_CHI, data)
    end 
    self:onActionEnd()
end
...
function ActionButtonsController:onPengClicked_(event)
    local function doPeng()
        local data = {}
        dataCenter:sendOverSocket(COMMANDS.HHQMT_USER_PENG, data)
    end
    local function doPass(isOK)    
        if isOK then
            doPeng()
...
function ActionButtonsController:onHuClicked_(event)
    local data = {}
    dataCenter:sendOverSocket(COMMANDS.HHQMT_USER_HU, data)
end
