...
function Player:doTianHuStart(card)
    ...
    gameAudio.playHHPHZHumanSound(card..".mp3", self.sex_)
end
...
function Player:setPaoPai(card, index,isReConnect, inFastMode)
    ...
    if not inFastMode then    
        gameAudio.playHHPHZHumanSound("pao.mp3", self.sex_)
...
function Player:onPengPaiExcecute_(event)
    ...
    if not inFastMode then
        gameAudio.playHHPHZHumanSound("peng.mp3", self.sex_)
    end
...
function Player:onChiPaiExcecute_(event)
    ...
    if not inFastMode then
        gameAudio.playHHPHZHumanSound("chi.mp3", self.sex_)  
        --将吃牌放第一张
...
function Player:setWeiPai(isChou, card,isReConnect, inFastMode)
    --臭偎没有
    if not inFastMode then
        gameAudio.playHHPHZHumanSound("wei.mp3", self.sex_)    
...
function Player:setTiPai(card, index,isReConnect, inFastMode)
     if not inFastMode then
        gameAudio.playHHPHZHumanSound("ti.mp3", self.sex_)
...
function Player:setHuPai(typeid)
    if not inFastMode then
        gameAudio.playHHPHZHumanSound("hu.mp3", self.sex_)
    end
