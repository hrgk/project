...
MaJiangTable.PLAY_SHAIZI_ANIMA = "PLAY_SHAIZI_ANIMA"
MaJiangTable.INIT_TABLE_CARDS = "INIT_TABLE_CARDS"
MaJiangTable.TABLE_CARD_CHANGE = "TABLE_CARD_CHANGE"
...
MaJiangTable.schema["matchType"] = {"number", 1}  -- 是否有同IP玩家在游戏
...
function MaJiangTable:setTableCount(count)
    self:dispatchEvent({name = MaJiangTable.TABLE_CARD_CHANGE, count = count})
end
...
function MaJiangTable:setConfigData(data)
    -- print(dataCenter:getBool(data.rules.wanFa.qxd), data.rules.wanFa.qxd)
    self.configData_ = {
        ...
        allowSevenPairs = data.rules.wanFa.qxd == 1,
...
function MaJiangTable:playShaiZi(data)
    self:dispatchEvent({name = MaJiangTable.PLAY_SHAIZI_ANIMA, data = data})
end

function MaJiangTable:initTableCards(zhuangIndex,dice1,dice2,leftCount)
    self:dispatchEvent({name = MaJiangTable.INIT_TABLE_CARDS, zhuangIndex = zhuangIndex,dice1 = dice1,dice2 = dice2,leftCount = leftCount})
end
