...
local BaseAlgorithm = import("app.games.hzmajiang.utils.BaseAlgorithm")
local TaskQueue = require("app.controllers.TaskQueue")
local MaJiangTable = import("app.games.hzmajiang.models.MaJiangTable")
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames { 
        ...
        {COMMANDS.HZMJ_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.HZMJ_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.HZMJ_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.HZMJ_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.HZMJ_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播
        {COMMANDS.HZMJ_PUBLIC_TIME, handler(self, self.onPublicTime_)},  -- 公共牌时间

        {COMMANDS.HZMJ_DEAL_CARD, handler(self, self.onDealCards_)},  -- 发牌
        {COMMANDS.HZMJ_USER_GANG, handler(self, self.onUserGang_)},  -- 玩家提 
        {COMMANDS.HZMJ_USER_HU, handler(self, self.onUserHu_)},  -- 胡牌
        {COMMANDS.HZMJ_USER_CHI, handler(self, self.onUserChi_)},  -- 吃牌
        {COMMANDS.HZMJ_USER_PENG, handler(self, self.onUserPeng_)},  -- 碰牌
        {COMMANDS.HZMJ_USER_MO_PAI, handler(self, self.onPlayerMoPai_)},  
        {COMMANDS.HZMJ_ALL_PASS, handler(self, self.onAllPass_)},
        {COMMANDS.HZMJ_NOTIFY_HU, handler(self, self.onNotifyHu_)},  -- 通知胡牌请求
        {COMMANDS.HZMJ_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
        {COMMANDS.HZMJ_PLAY_ACTION, handler(self, self.onPlayAction_)},
        {COMMANDS.HZMJ_UPDATE_SCORE, handler(self, self.onUpdateScore_)},

        {COMMANDS.HZMJ_USER_AFTER_GANG, handler(self, self.onCSMJUserGang_)},
        {COMMANDS.HZMJ_USER_BU_CARD, handler(self, self.onUserGang_)},

        {COMMANDS.HZMJ_BEGIN_CHUI, handler(self, self.onsBeginChui_)},
        {COMMANDS.HZMJ_PLAYER_CHUI, handler(self, self.onPlayerChui_)},

        {COMMANDS.HZMJ_GET_ALL_PLAYER_CARDS, handler(self, self.onGetAllPlayerCards_)},
    }
...
function TableController:onChuPaiEvent_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.HZMJ_CHU_PAI, data)
end
    ...
    if data.operates and #data.operates >0 then
        ...
        if needAuto then
            ...
            or table.indexof(data.operates, MJ_ACTIONS.QIANG_GANG_HU) then
                ...
                print('doPlayerMoPai_',isCanTing,#canGange,table.indexof(canGange,data.card))
...
function TableController:doRoundStart_(data)
    ...
    self:playersRoundStart_()
    self:hideReady({})
    dump(data,"TableController:doRoundStart_")
...
function TableController:doTurnTo_(data)
    if data.outCards ~= nil then
        ...
        if not isRight then
            dataCenter:sendOverSocket(COMMANDS.HZMJ_GET_ALL_PLAYER_CARDS)
        end
    end
