...
function ReviewPlayController:ctor(data, hostPlayer)
    ...
    for i,v in ipairs(data) do
        if (not v.msg.type or v.msg.type ~= "recv") and (v.cmd ~= COMMANDS.HZMJ_PLAYER_PASS) then
    ...
    self.deleCommondList_ = {
        COMMANDS.HZMJ_PLAYER_ENTER_ROOM,
        COMMANDS.HZMJ_PLAYER_ENTER_ROOM,
        COMMANDS.HZMJ_PLAYER_ENTER_ROOM,
        COMMANDS.HZMJ_PLAYER_ENTER_ROOM,
        COMMANDS.HZMJ_ROOM_INFO,
        COMMANDS.HZMJ_DEAL_CARD,
        COMMANDS.HZMJ_DEAL_CARD,
        COMMANDS.HZMJ_DEAL_CARD,
        COMMANDS.HZMJ_DEAL_CARD,
    }
...
function ReviewPlayController:filterCmd_()
    ...
    for i,v in ipairs(self.deleCommondList_) do
        for i,v in ipairs(self.gameData_) do
            if v.cmd == COMMANDS.HZMJ_PLAYER_ENTER_ROOM then
                table.insert(self.playerList_, v)
                table.removebyvalue(self.gameData_, v)
                break
            elseif v.cmd == COMMANDS.HZMJ_ROOM_INFO then
                table.insert(self.commondList_, clone(v))
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.HZMJ_DEAL_CARD then
                table.insert(self.commondList_, clone(v))
                table.removebyvalue(self.gameData_, v)
                break
            end
