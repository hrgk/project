...
function ReviewPlayController:ctor(data, hostPlayer)
	...
	for i,v in ipairs(data) do
		if (not v.msg.type or v.msg.type ~= "recv") and (v.cmd ~= COMMANDS.CDPHZ_HU_PASS and v.cmd ~= COMMANDS.CDPHZ_PLAYER_PASS) then
    ...
    self.deleCommondList_ = {
    	COMMANDS.CDPHZ_PLAYER_ENTER_ROOM,
    	COMMANDS.CDPHZ_PLAYER_ENTER_ROOM,
    	COMMANDS.CDPHZ_PLAYER_ENTER_ROOM,
    	COMMANDS.CDPHZ_ROUND_FLOW,
    	COMMANDS.CDPHZ_DEAL_CARD,
	}
end
...
function ReviewPlayController:filterCmd_()
	...
	for i,v in ipairs(self.deleCommondList_) do
		for i,v in ipairs(self.gameData_) do
			if v.cmd == COMMANDS.CDPHZ_PLAYER_ENTER_ROOM then
				table.insert(self.playerList_, v)
				table.removebyvalue(self.gameData_, v)
			elseif v.cmd == COMMANDS.CDPHZ_DEAL_CARD then
				table.removebyvalue(self.gameData_, v)
			end
		end
	end
