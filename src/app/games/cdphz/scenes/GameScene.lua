...
-- 算法测试用例
-- require ("app.games.cdphz.utils.PaoHuZiTest")
...
function GameScene:onEnter_()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.CDPHZ_ROUND_OVER, handler(self, self.onRoundOver_)},
        {COMMANDS.CDPHZ_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.CDPHZ_GAME_START, handler(self, self.onGameStart_)},
        {COMMANDS.CDPHZ_READY, handler(self, self.onReady_)},
        {COMMANDS.CDPHZ_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.CDPHZ_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.CDPHZ_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.CDPHZ_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.CDPHZ_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.CDPHZ_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},
        {COMMANDS.CDPHZ_DEBUG_CONFIG_CARD, handler(self, self.onConfigCard_)},
    }
...
function GameScene:onEnterTransitionFinish_()
    ...
    if DEBUG > 0 then
        local GameSceneTest = require("app.games.cdphz.scenes.GameSceneTest")
...
function GameScene:onWanFa_(event)
    ...
    display.getRunningScene():initWanFa(GAME_CDPHZ,date)
end

function GameScene:onStartClicked_()
    self.buttonStart_:hide()
    dataCenter:sendOverSocket(COMMANDS.CDPHZ_READY)
end

function GameScene:onDingWeiClicked_(event)
    dataCenter:sendOverSocket(COMMANDS.CDPHZ_REQUEST_LOCATION)
end
...
function GameScene:onInviteClicked_(event)
    ...
    local title = "常德跑胡 房间号【" .. roomId .. "】" .. queRenMsg
...
function GameScene:showLocationTips_(data,iskaiju)
    ...
    if data.code == 0 then
        ...
        if date.rules.totalSeat == 2 then
            ...
            self.dingWeiView_ = SanRenDingWei.new(params,data.distances,iskaiju,GAME_CDPHZ):addTo(self.layerWindows_)
        end 
    end
end
...
function GameScene:onQuitRoomClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.CDPHZ_LEAVE_ROOM)
        end
    end
...
function GameScene:onDismissClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.CDPHZ_OWNER_DISMISS)
        end
    end
    app:confirm(msg, callback)
end
...
function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.CDPHZ_REQUEST_DISMISS, {agree = isAgree})
end
