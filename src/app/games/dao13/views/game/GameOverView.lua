...
function GameOverView:initGameItem_(seats)
    local length = #checktable(seats)
    for i=1,4 do
        if seats[i] then
            local item = GameOVerItemView.new(seats[i])
            item:setNode(self.players_[i])
            if length < 4 then
                local x = self:clacItemPos_(length, i)
                self.players_[i]:setPositionX(x)  
            end
        else
...
function GameOverView:clacItemPos_(total, index)
    return (index - (total - 1) / 2 - 1) * 400
end

function GameOverView:loaderCsb()
    self.csbNode_ = cc.uiloader:load("views/games/d13/GameOver.csb"):addTo(self)
...
function GameOverView:fenxiangHandler_()
    ...
    display.getRunningScene():gameShareWeiXin("十三道","",callback,display.getRunningScene():getTable():getTid(),selfData:getUid(),self.recordId)
end
...
function GameOverView:fenxiangImageHandler_()
    ...
    display.captureScreen(function (bSuc, filePath)
        ...
        display.getRunningScene():shareImage(2,"十三道","",0, callback, filePath)
