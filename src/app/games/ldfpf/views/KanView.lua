...
local PaoHuZiAlgorithm = import("app.games.ldfpf.utils.PaoHuZiAlgorithm")

local KanView = class("KanView", function()
    return display.newNode()
end)

local max_height = 90 -- 最大行高
local da_xiao_space = 156 -- 大小牌之间的间隔

local KAN_WIDTH = 90
local KAN_HEIGHT = 123   
-- 340, 305
...
function KanView:isInKanCard(x, y)
    ...
    for i,v in ipairs(array) do
        ...
        local rotate = v:getRotation()
        local width, height = rect.width, rect.height
        local in_it = BaseAlgorithm.inRectWithAngle(width, height, x, y, rect, rotate)
        if in_it and v.card then
            return true,v
        end
    end 
    return false
...
function KanView:showAddCard(origPos)
    local cards = self:getSprites()
    if not cards or #cards ~= 1 then
        return
    end
    local x, y = self:getCardPos(1, 1)
    local aimCard = cards[1]
    local pos = self:convertToNodeSpace(cc.p(origPos.x, origPos.y))
    aimCard:setPosition(pos.x,pos.y)
    self:show()
    transition.moveTo(aimCard, {x = x, y = y, time = 0.15})
end
...
function KanView:addCard(card, x1, y1)
    local tmp = app:createConcreteView("PaperCardView", card, 2, false, self.func):addTo(self):pos(x1,y1)
    tmp:setName("PaperCardView"..x1..y1..card)

    self.cardsNodeList_["PaperCardView"..x1..y1..card] = tmp
...
function KanView:getCardPos(index, totalCount)
    ...
    local y = self.height - KAN_HEIGHT / 2 - (4-index) * (width)
...
function KanView:draw(cards, needFlip,flipBaseTime,mingpai)
    ...
    for k, v in pairs(cards) do
        count = count + 1
        local x, y = self:getCardPos(count, #cards) 
        local fanpai = mingpai or false
        local tmp = app:createConcreteView("PaperCardView", v, 2, mingpai, self.func):addTo(self):pos(x, y) 
...
function KanView:clacSelfHuXi(cards)
    ...
    self.huXiBg_:pos(45,53)
    self.huXiBg_:setOpacity(100)
    self.huXilabel_:setAnchorPoint(0.5,0.5)
    self.huXilabel_:pos(45,15)
    if setData:getCDPHZCardType() == 1 then
        self.huXiBg_:pos(45,110)
    end
    
end

function KanView:showHuXi(bool)
    if self.huXiBg_ then
        self.huXiBg_:setVisible(bool)
    end
end
