local PaperCardGroup = require("app.games.ldfpf.utils.PaperCardGroup")
local PaperCardList = class("PaperCardList", gailun.BaseView)
local KanView = import(".KanView")
...
local KAN_Y = -2.8 * 305 -- 坎的坐标位置
local THREWLINE_Y = 600
...
function PaperCardList:dealCards(cards, callfunc)
    ...
    self:showPaperCardsWithoutAnim_(cards, callfunc)
end
...
function PaperCardList:showPaperCardsWithAnim_(cards, callfunc)
    ...
    for k = 1, total_count do
        ...
        local tmp = KanView.new(kanList[k], handler(self, self.onTouchCards), true,k * 0.08,true):addTo(self.nodePaperCards_):pos(p.x, p2.y)
...
function PaperCardList:showPaperCardsWithoutAnim_(cards, callfunc, isRconnect)
    ...
    for k = 1, total_count do
        local x, y = self:getKanPos(total_count, k, #kanList[k])
        local p2 = self:convertToNodeSpace(cc.p(x, y))
        local tmp = KanView.new(kanList[k], handler(self, self.onTouchCards), false,0,false):addTo(self.nodePaperCards_):pos(p.x, p2.y)
        tmp:setAnchorPoint(cc.p(KAN_AP_X, KAN_AP_Y))
        table.insert(self.kans_, tmp)
    end
    self:pauseTouchOfKans() --先暂停坎的触摸 
...
function PaperCardList:showRawCards(cards, index, callback)
    ...
    local tmp = app:createConcreteView("PaperCardView", cards[index], 1, false, nil):addTo(self):pos(p.x, p.y, 100)
...
function PaperCardList:showKanCards(cards)
    ...
    for k = 1, total_count do
        ...
        local p2 = self:convertToNodeSpace(cc.p(x, y))
        local tmp = KanView.new(cards[k], handler(self, self.onTouchCards), true):addTo(self.nodePaperCards_):pos(p.x, p2.y)
        tmp:setAnchorPoint(cc.p(KAN_AP_X, KAN_AP_Y))
        table.insert(self.kans_, tmp)
    end
...
function PaperCardList:flipAllCards(cards)
    ...
    local total_count = table.nums(cards)
    for k = 1, total_count do
        local x, y, rotateValue = self:getKanPos(total_count, k, #cards[k])
        local p = self:convertToNodeSpace(cc.p(x, y))
        local tmp = KanView.new(cards[k], handler(self, self.onTouchCards)):addTo(self):pos(p.x, p.y)
        table.insert(self.kans_, tmp)
        tmp:setAnchorPoint(cc.p(KAN_AP_X, KAN_AP_Y))
        transition.rotateTo(tmp, {rotate = rotateValue, time = 0.5})
    end
...
function PaperCardList:reOrderAllKan(seconds, oneByOne)
    ...
    for i, v in ipairs(self.kans_) do
        ...
        local x, y, rotateValue = self:getKanPos(total_count, i, count)
        local p = self:convertToNodeSpace(cc.p(x, y))
        ...
        transition.rotateTo(v, {rotate = rotateValue, time = rotate_seconds})
...
function PaperCardList:onTouchCards(obj, event, x, y)
    ...
    if event.name == 'began' then 
        ...
        self.tipCard_ = app:createConcreteView("PaperCardView", obj.card, 1, false, nil):addTo(self.nodeOperate_):pos(p.x, p.y, 100)
            ...
            for i,v in ipairs(self.kans_) do
                if not v:isFixed() then
                    if v:isInKan(x, y) then 
                        ...
                        self.playerView_:checkQuicklyTing()
                        return
                    end
                end 
            end 
...
function PaperCardList:onTouchInSelfKan(kan, obj, deltaY)
    local x1, y1 = obj:getPosition()
    obj:pos(x1, y1 + deltaY)
    kan:sortCardsByY()
end
...
function PaperCardList:onTouchBetweenKans(to_kan, obj, x, y, tipRawY)
    ...
    if obj:getParent() == to_kan then
        self:onTouchInSelfKan(to_kan, obj, deltaY) 
        return
    end
    ...
    if obj:getParent():delTarget(obj) then 
        to_kan:addCard(card, x1, y1 + deltaY)
    end
    self:reOrderAllKan()
    self:updateTotalHuXi()
end
...
function PaperCardList:onTouchOutAllKans(obj, x, y)
    ...
    if count < max_kan then
        local p = self:convertToNodeSpace(cc.p(x, y))
        local x2, y2 = self:getKanPos(count + 1, count + 1, 1)
        local p2 = self:convertToNodeSpace(cc.p(x2, y2))
        kan = KanView.new({}, handler(self, self.onTouchCards), true):addTo(self.nodePaperCards_):pos(p.x, p2.y) 
        kan:setAnchorPoint(cc.p(KAN_AP_X, KAN_AP_Y))
        table.insert(self.kans_, kan)
    end
    local card = obj.card
    if obj:getParent():delTarget(obj) then
        kan:addCard(card)
...
function PaperCardList:onCardThrow(obj, x, y)
    ...
    -- if self:isOtherWei_(obj, true) then
    --     return
    -- end
...
function PaperCardList:isOtherWei_(obj, isShowAlert)
    local scene = display.getRunningScene()
    -- dump(scene.__cname, "GameScene __cname")
    local cards = scene:getOthersWeiPai(self.player_:getSeatID())
    dump(cards, "getOthersWeiPai")
    if table.indexof(cards, obj.card) then
        if isShowAlert then
            local tips = "您确定要放明跑(打出后此局不能吃碰)?"
            local function callback(isOK)
                if isOK then
                    local data = {cards = obj.card}
                    dataCenter:sendOverSocket(COMMANDS.BP_CHU_PAI, data)
                end
            end
            app:confirm(tips, callback)
        end
        return true
    end
    return false
