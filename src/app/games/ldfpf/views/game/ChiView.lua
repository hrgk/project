...
function ChiView:onGroupChoosed(index)
    ...
    transition.moveTo(self.selectFaGuang1, {x = self.width/ 10 * index - self.width/ 30 - self.offX[#self.curr_level_data], y = 90, time = 0.2, onComplete = function()
...
function ChiView:showChoise(card, chiList, func)
    ...
    if #self.curr_level_data == 1 then
        self:onGroupChoosed(1)
    else
        self:showLevel()
    end
	return self
...
function ChiView:showLevel()
	...
	if not self.haveNext then
		self.chiBg = display.newScale9Sprite('res/images/game/chiview_bg.png', display.cx , chi_pos_y+185, cc.size(self.width / 9 * #cards ,180)):addTo(self) 
		self.chiBg:setAnchorPoint(cc.p(1, 0.5))
	else
		self.chiBg = display.newScale9Sprite('res/images/game/chiview_bg.png',  display.cx, chi_pos_y+185, cc.size(self.width / 9 * #cards ,180)):addTo(self) 
		self.chiBg:setAnchorPoint(cc.p(1, 0.5))
	end
...
function ChiView:showLevel2()
	...
	local aimX = display.cx - self.chiBg:getContentSize().width 
	self.chiBg2 = display.newScale9Sprite('res/images/game/chiview_bg.png', aimX , chi_pos_y+185, cc.size(self.width / 9 * #cards,180)):addTo(self) 
	self.chiBg2:setAnchorPoint(cc.p(1, 0.5))
...
function ChiView:showLevel3()
	...
	local aimX = display.cx - self.chiBg:getContentSize().width - self.chiBg2:getContentSize().width
	self.chiBg3 = display.newScale9Sprite('res/images/game/chiview_bg.png', aimX , chi_pos_y+185, cc.size(self.width / 9 * #cards,180)):addTo(self) 
	self.chiBg3:setAnchorPoint(cc.p(1, 0.5))
