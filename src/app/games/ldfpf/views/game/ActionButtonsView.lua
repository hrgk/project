...
function ActionButtonsView:onEnter()
    ...
    local functions = {
                        ...
                        function(sender) dataCenter:sendOverSocket(COMMANDS.LDFPF_PLAYER_PASS, {}) end,
                      }

    for i=1, #buttons do
        self.bg_:addChild(buttons[i])
        local x = posxs[i]
        buttons[i]:setPosition(cc.p(0 , -100))
...
function ActionButtonsView:showActions(chi,peng,hu)
    ...
    for k, v in ipairs(btnList) do
        v:setPositionX(interval * (k - 1) - width / 2 + 300)
    end
end
