...
local testData = require("app.games.ldfpf.data.test")

local HuangAnim = require("app.games.ldfpf.views.HuangAnim")
...
tableNodes.normal = {
    ...
    children = {
        ...
        {type = TYPES.CUSTOM, var = "directorController_", class = "app.games.ldfpf.controllers.DirectorController"},
        ...
        {type = TYPES.CUSTOM, var = "progressView_", class = "app.games.ldfpf.views.game.ProgressView"}, -- 主玩家的手牌容器，需要最高等级的显示
...
local cur_card_pos = 
{
    {display.cx, display.cy+165},
    {player_pos[2][1] - mopai_pos_adjustx, display.height - mopai_pos_adjusty },
    {player_pos[3][1] + mopai_pos_adjustx - 230, display.height - mopai_pos_adjusty + 20 }
}

local TableView = class("TableView", gailun.BaseView)

local PuPingCardList = import("app.games.ldfpf.views.pingPu.PaperCardList")
local ShanXingCardList = import("app.games.ldfpf.views.shanXing.PaperCardList")
-- "app.games.ldfpf.views.pingPu.PaperCardList"
local paiXingList = {}
paiXingList[1] = PuPingCardList
paiXingList[2] = ShanXingCardList
function TableView:ctor(table,nowSeats)
    -- display.addSpriteFrames("textures/hall.plist", "textures/hall.png")
    -- :setOpacity(opacity)
    self.table_ = table
    self.nowSeats_ = nowSeats
    ...
    local debugView = DebugView.new(COMMANDS.LDFPF_DEBUG_CONFIG_CARD, testData)
...
function TableView:onCurrCardChange_(event)
    ...
    if not event.card  or event.card == 0 then
        ...
        if direction.BOTTOM == event.index then
        ...
        elseif direction.RIGHT == event.index then   
            local orgX,orgY = display.cx + 200,cur_card_pos[2][2]
            if not isMo then
...
function TableView:calcRoundOverPokerPosRight_(total, i, j, playerX, playerY,perkan)
    ...
    if self.nowSeats_ == 2 then 
        return playerX - x - 300 + 500 , y + playerY - roundOver_AdjustY - 130
    else
...
function TableView:calcReviewPokerPosBotton_(total, i, j, playerX, playerY,perkan)
    local majiang_width = 100
    local per_height = 110
   
    local x = 200 +  i * majiang_width - 150
