...
function GameOverView:ctor(data)
    ...
    self.daniaoDifen_:setString("打鸟底分："..tostring(data.niaoFen))
	...
	for i,v in pairs(checktable(data.seats)) do
		...
		item:update(v,data.dismissSeat,matchDifen,gameDifen )
...
function GameOverView:loaderCsb()
    self.csbNode_ = cc.uiloader:load("views/games/sybp/GameOver.csb"):addTo(self)
