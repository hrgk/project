...
function PlayerController:ctor(seatID,totaSeats)
...
--    if totaSeats ~= 2 then
--        self.view_ = app:createConcreteView("game.PlayerView", self.index_,self.nowSeats_):addTo(self)
--    else
--        if self.index_ == 2 then
--            self.view_ = app:createConcreteView("game.PlayerView", self.index_ + 1,self.nowSeats_):addTo(self)
--        else
--            self.view_ = app:createConcreteView("game.PlayerView", self.index_,self.nowSeats_):addTo(self)
--        end
--    end
...
function PlayerController.getScoreInfoSuc_(self,event)
    local data = json.decode(event)
    local score = data.data.dou + data.data.lock_score
    self:setClubScore(score)
end

function PlayerController:getScoreInfoFail_(event)
    print(event)
end
...
function PlayerController:setClubScore(...)
    if not self.player_ then
        return
    end
    return self.player_:setClubScore(...)
end

function PlayerController:getClubScore(...)
    if not self.player_ then
        return
    end
    return self.player_:getClubScore(...)
end
...
function PlayerController:setQiPai(...)
    if not self.player_ then
        return
    end
    self.player_:showQiPai(...)
end

function PlayerController:setDaNiao(...)
    if not self.player_ then
        return
    end
    self.player_:showDaNiao(...)
end
...
function PlayerController:doQiPai(...)
    if not self.player_ then
        return
    end
    self.player_:doQiPai(...)
end
...
function PlayerController:setScore(score,isLdfpf)
    if not self.player_ then
        return
    end
    return self.player_:setScore(score,isLdfpf)
end
