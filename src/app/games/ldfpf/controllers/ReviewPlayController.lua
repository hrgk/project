...
function ReviewPlayController:ctor(data, hostPlayer)
	...
	for i,v in ipairs(data) do
		if (not v.msg.type or v.msg.type ~= "recv") and (v.cmd ~= COMMANDS.LDFPF_HU_PASS and v.cmd ~= COMMANDS.LDFPF_PLAYER_PASS) then
    ...
    self.deleCommondList_ = {
    	COMMANDS.LDFPF_PLAYER_ENTER_ROOM,
    	COMMANDS.LDFPF_PLAYER_ENTER_ROOM,
    	COMMANDS.LDFPF_PLAYER_ENTER_ROOM,
    	COMMANDS.LDFPF_ROUND_FLOW,
    	COMMANDS.LDFPF_DEAL_CARD,
	}
end
...
function ReviewPlayController:filterCmd_()
	...
	for i,v in ipairs(self.deleCommondList_) do
		for i,v in ipairs(self.gameData_) do
			if v.cmd == COMMANDS.LDFPF_PLAYER_ENTER_ROOM then
				table.insert(self.playerList_, v)
				table.removebyvalue(self.gameData_, v)
			elseif v.cmd == COMMANDS.LDFPF_DEAL_CARD then
				table.removebyvalue(self.gameData_, v)
			end
		end
	end
