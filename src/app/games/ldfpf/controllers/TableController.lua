...
local BaseAlgorithm = import("app.games.ldfpf.utils.BaseAlgorithm")
local TaskQueue = require("app.controllers.TaskQueue")
local TableModel = import("app.games.ldfpf.models.PokerTable")
...
function TableController:ctor(totaSeats)
...
function TableController:init_()
    self.tableView_ = app:createConcreteView("game.TableView", self.table_,self.nowSeats_):addTo(self)
...
function TableController:getDaNiaoBtn_(confirm_btn,cancel_btn)
    self.buttonConfirmDaNiao_ = confirm_btn
    self.buttonCancelDaNiao_ = cancel_btn
end
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames { 
        {COMMANDS.LDFPF_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.LDFPF_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.LDFPF_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.LDFPF_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.LDFPF_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.LDFPF_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播

        {COMMANDS.LDFPF_DEAL_CARD, handler(self, self.onDealCards_)},  -- 发牌
        {COMMANDS.LDFPF_TIAN_HU_START, handler(self, self.onTianHuStart_)},  -- 开始天胡 
        {COMMANDS.LDFPF_TIAN_HU_END, handler(self, self.onTianHuEnd_)},  -- 开始结束 
        {COMMANDS.LDFPF_USER_TI, handler(self, self.onUserTi_)},  -- 玩家提 
        {COMMANDS.LDFPF_USER_PAO, handler(self, self.onUserPao_)},  -- 玩家跑
        {COMMANDS.LDFPF_USER_WEI, handler(self, self.onUserWei_)},  -- 玩家偎牌 
        {COMMANDS.LDFPF_USER_HU, handler(self, self.onUserHu_)},  -- 胡牌
        {COMMANDS.LDFPF_USER_CHI, handler(self, self.onUserChi_)},  -- 吃牌
        {COMMANDS.LDFPF_USER_PENG, handler(self, self.onUserPeng_)},  -- 碰牌
        {COMMANDS.LDFPF_USER_MO_PAI, handler(self, self.onPlayerMoPai_)},  
        {COMMANDS.LDFPF_ALL_PASS, handler(self, self.onAllPass_)},
        {COMMANDS.LDFPF_NOTIFY_HU, handler(self, self.onNotifyHu_)},  -- 通知胡牌请求
        {COMMANDS.LDFPF_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
        {COMMANDS.LDFPF_FIRST_HAND_TI, handler(self, self.onFirstHandTi_)},
        {COMMANDS.LDFPF_HU_PASS, handler(self, self.onHuPass_)},
        {COMMANDS.LDFPF_QI_PAI, handler(self, self.onQiPai_)},
        {COMMANDS.LDFPF_CONFIRM_DA_NIAO, handler(self, self.onConfirmDaNiao_)},
    }
...
function TableController:onChuPaiEvent_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.LDFPF_CHU_PAI, data)
end
...
function TableController:doRoomInfo(data)
    ...
    for _,player in ipairs(self.seats_) do
        player.view_:SetScoreInfo(data.config.matchType,data.clubID,player:getPlayer():getUid(),player.getScoreInfoSuc_,player.getScoreInfoFail_,player)
    end
...
function TableController:resumePlayer_(player, data, isReview)
    ...
    -- 恢复打鸟
    if data.daNiao ~= nil and  data.isPrepare then
        player:setDaNiao(data.daNiao)
--        if data.uid == selfData:getUid() and data.daNiao == -1 then
--            self.buttonConfirmDaNiao_:show()
--            self.buttonCancelDaNiao_:show()
--        end
    end
    ...
    player:setScore(data.score,true)
    -- 恢复手牌
    ...
    -- 恢复弃牌
    if data.qipai ~= nil then
        player:setQiPai(data.qipai)
        if data.uid == selfData:getUid() then
            self.buttonQiPai_:setVisible(not data.qipai)
        end
    end
...
function TableController:toQiPai(id)
    local player = self:getPlayerBySeatID(id)
    player.view_:isShowQiPai(true)
end

--玩家弃牌
function TableController:doQiPao_(event)
   if not event or not event.seatID then
       return
   end
   local player = self:getPlayerBySeatID(event.seatID)
   local uid = player.player_:getUid()
   if uid ~= selfData:getUid() then
       self:toQiPai(event.seatID)
       return
   end
   if player then
      player:doQiPai()
...
--玩家弃牌
function TableController:onQiPai_(event)
    if event.data.isReview then
        self:doQiPao_(event.data)
    else
        TaskQueue.add(handler(self, self.doQiPao_), 0, 0, event.data)
    end
end

--玩家确定打鸟
function TableController:onConfirmDaNiao_(event)
    if event.data.isReview then
        self:doConfirmDaNiao_(event.data)
    else
        TaskQueue.add(handler(self, self.doConfirmDaNiao_), 0, 0, event.data)
    end
end

function TableController:doConfirmDaNiao_(event)
    for _, v in ipairs(event.data) do
        while true do
            local player = self:getPlayerBySeatID(v.seatID)
            local uid = player.player_:getUid()
            if uid ~= selfData:getUid() then
                self:toConfirmDaNiao_(v)
                break
            end
            if v.daNiao == -1 then
                self.buttonConfirmDaNiao_:show()
            ...
            if v.daNiao == 1 then
                player.view_:isShowDaNiao(true)
                break
            end
            break
        end
...
function TableController:toConfirmDaNiao_(data)
    local player = self:getPlayerBySeatID(data.seatID)
    player.view_:isShowDaNiao(data.daNiao == 1)
end
...
function TableController:doQiPai_(data)
end
...
--     else
--         -- self:doUserPeng_(event.data)
--         TaskQueue.add(handler(self, self.doQiPai_), TABLE_TIME.onUserPeng_[1], TABLE_TIME.onUserPeng_[2], event.data)
--     end
-- end
...
function TableController:createSeats_(number)
    ...
    for i=1, number do
        self.seats_[i] = PlayerController.new(i,self.nowSeats_):addTo(self.nodePlayers_)
    end
end
