...
function GameScene:onAllLoaded_()
    self.tableController_ = app:createConcreteController("TableController",self.nowSeats_):addTo(self.layerTable_)
    self.tableController_:setScene(self)
    gailun.uihelper.render(self, Nodes.menuLayerTree, self.layerMenu_)
    self.tableController_.buttonQiPai_ = self.buttonQiPai_
    self.tableController_:getDaNiaoBtn_(self.buttonConfirmDaNiao_,self.buttonCancelDaNiao_)
    self:onEnter_()
end
...
function GameScene:onEnter_()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.LDFPF_ROUND_OVER, handler(self, self.onRoundOver_)},
        {COMMANDS.LDFPF_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.LDFPF_GAME_START, handler(self, self.onGameStart_)},
        {COMMANDS.LDFPF_READY, handler(self, self.onReady_)},
        {COMMANDS.LDFPF_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.LDFPF_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.LDFPF_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.LDFPF_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.LDFPF_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.LDFPF_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},
        {COMMANDS.LDFPF_DEBUG_CONFIG_CARD, handler(self, self.onConfigCard_)},
    }
...
function GameScene:onEnterTransitionFinish_()
    ...
    local texture = cc.Director:getInstance():getTextureCache():addImage("res/images/game/bg" .. setData:getCDPHZBgIndex() .. ".jpg")
    self.bgSprite_:setTexture(texture)
    ...
    self.buttonQiPai_:onButtonClicked(handler(self,self.OnQiPaiClicked_))
    self.buttonQiPai_:hide()
    self.buttonConfirmDaNiao_:onButtonClicked(handler(self,self.OnConfirmDaNiaoClicked_))
    self.buttonConfirmDaNiao_:hide()
    self.buttonCancelDaNiao_:onButtonClicked(handler(self,self.OnCancelDaNiaoClicked_))
    self.buttonCancelDaNiao_:hide()
    ...
    if DEBUG > 0 then
        local GameSceneTest = require("app.games.ldfpf.scenes.GameSceneTest")
...
function GameScene:onWanFa_(event)
    ...
    display.getRunningScene():initWanFa(GAME_LDFPF,date)
end

function GameScene:onStartClicked_()
    self.buttonStart_:hide()
    self.buttonInvite_:hide()
    dataCenter:sendOverSocket(COMMANDS.LDFPF_READY)
end

function GameScene:onDingWeiClicked_(event)
    dataCenter:sendOverSocket(COMMANDS.LDFPF_REQUEST_LOCATION)
end
...
function GameScene:onInviteClicked_(event)
    ...
    local title = "娄底放炮罚 房间号【" .. roomId .. "】" .. queRenMsg
    local descriptionString = self.tableController_:makeShareRuleString(",")
    local description = (descriptionString or "") .. "速度来玩！"
...
function GameScene:showLocationTips_(data,iskaiju)
    ...
    if data.code == 0 then
        ...
        if date.rules.totalSeat == 2 then
            ...
            self.dingWeiView_ = SanRenDingWei.new(params,data.distances,iskaiju,GAME_LDFPF):addTo(self.layerWindows_)
        end 
    end
end
...
function GameScene:doRoundOver_(data)
    ...
    self.buttonQiPai_:setVisible(true)
    for _,v in pairs(self.tableController_.seats_) do
        v.view_:isShowQiPai(false)
    end
...
function GameScene:doRoundReview_(data)
    ...
    if data.isHuangZhuang == 1 then
        ...
        pokerTable:showDiPai(data.leftCards,data.winInfo.huList, p, data.winInfo.huCard)
        delay = 3
    end
...
function GameScene:onQuitRoomClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.LDFPF_LEAVE_ROOM)
        end
    end
...
function GameScene:onDismissClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
            dataCenter:sendOverSocket(COMMANDS.LDFPF_OWNER_DISMISS)
        end
    end
    app:confirm(msg, callback)
end
...
function GameScene:OnQiPaiClicked_(event)
    self.buttonQiPai_:hide()
    dataCenter:sendOverSocket(COMMANDS.LDFPF_QI_PAI)
    print("GameScene:OnQiPaiClicked_  1019")
end

function GameScene:OnConfirmDaNiaoClicked_(event)
    self.buttonConfirmDaNiao_:hide()
    self.buttonCancelDaNiao_:hide()
    local data = {daNiao = 1}
    dataCenter:sendOverSocket(COMMANDS.LDFPF_CONFIRM_DA_NIAO,data)
end

function GameScene:OnCancelDaNiaoClicked_(event)
    self.buttonConfirmDaNiao_:hide()
    self.buttonCancelDaNiao_:hide()
    local data = {daNiao = 0}
    dataCenter:sendOverSocket(COMMANDS.LDFPF_CONFIRM_DA_NIAO,data)
...
function GameScene:doCardSize_(event)
    self.tableController_:getHostPlayer():doTiPai()
end
...
function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.LDFPF_REQUEST_DISMISS, {agree = isAgree})
end
