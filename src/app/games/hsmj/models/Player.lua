...
function Player:checkTingPai()
    local isTingPai = HSAlgorithm.isTingPai(self.cards_,display.getRunningScene():getTable().getRuleDetails().fengPai == 1)
...
function Player:checkTingPaiByRemoveCard(card)
    ...
    return HSAlgorithm.isTingPai(result,display.getRunningScene():getTable().getRuleDetails().fengPai == 1),canGangValue
end
...
function Player:canChiHu(card)
    ...
    return HSAlgorithm.canChiHu(self:getCards(), card)
end

function Player:canZiMoHu()
    return HSAlgorithm.canZiMoHu(self:getCards(),self:getIsFirstHand())
end
...
function Player:calcAnGang()
    ...
    for _,v in pairs(cards) do
        if 4 == table.removebyvalue(testCards, v, true) then
