...
function GameSceneTest.test(self)
    ...
    -- GameSceneTest.testUpdateScore(self,1,11,1)
    ...
    -- GameSceneTest.testHupai(self,2,11,2)
    -- GameSceneTest.testbird(self,2,11,2)
    -- GameSceneTest.testChiPath(self)

    -- GameSceneTest.testRoundOver(self, 2)
    -- GameSceneTest.testGameOver(self, 2)
    -- GameSceneTest.testDingWei(self, 1)
end

function GameSceneTest.testDingWei(self, seconds)
    self:performWithDelay(function()
        local data = {}
        data.distances = {-1,-1,-1,-1,-1,-1}
        data.code = 0
        self:onNotifyLocation_({data = data})
        end, seconds)
end
...
function GameSceneTest.testDebugSetPoker(self, cardId)
    ...
    dump(data)
    dataCenter:sendOverSocket(COMMANDS.HS_MJ_DEBUG_CONFIG_CARD, data)
end
...
function GameSceneTest.testHupai(self,seatID,finish,seconds)
    ...
    -- ["recv",{"msg":{"huInfo":[{"isZiMo":false,"shouPai":[21,29,29,21,17,21,18],
    -- "isQiangGangHu":false,"flow":2,"preSeatID":2,"huCards":[16],"isFinish":1,"seatID":1}],
    -- "code":0},"cmd":926}]
    self:performWithDelay(function()
        local cards = {38, 27, 32, 23, 29, 22, 33, 36, 24, 22, 25, 37, 31}
        local data = {
            cards = {11,12,13,14,15,16},
            code = 0,
            huInfo = {
            {isFinish = 1,isQiangGangHu = false, flow = 2,
            shouPai = cards, huCards = {18}, isZiMo = false, seatID = 3, preSeatID = 1}
        }}
...
function GameSceneTest.testDealCards(self,seconds)
    local  cards ={16,36,43,12,12,13,11,11,29,19,11,18,15}
    local data = {}
    data.dealerSeatID = 1
    data.handCards = cards
    data.seq = 3
...
function GameSceneTest.testRoundOver(self,seconds)
    ...
    local data = {
        ...
        seats ={
            ...
            {
                seatID = 4,
                tableCards= {{5, 11, 11, 11, 11}, {12,12,12,12,12}},  --// 桌面牌公示
                handCards = {21,23,11,12,14,15}, --// 剩余没出的牌
                score =-48, --// 当局得分
                totalScore = 30, --// 总得分
                scoreFrom = {
                    qiangGangHu=0,
