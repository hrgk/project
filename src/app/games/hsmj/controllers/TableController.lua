...
local BaseAlgorithm = import("app.games.hsmj.utils.BaseAlgorithm")
local TaskQueue = require("app.controllers.TaskQueue")
local MaJiangTable = import("app.games.hsmj.models.MaJiangTable")
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames { 
        {COMMANDS.HS_MJ_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.HS_MJ_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.HS_MJ_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.HS_MJ_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.HS_MJ_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.HS_MJ_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播
        {COMMANDS.HS_MJ_PUBLIC_TIME, handler(self, self.onPublicTime_)},  -- 公共牌时间

        {COMMANDS.HS_MJ_DEAL_CARD, handler(self, self.onDealCards_)},  -- 发牌
        {COMMANDS.HS_MJ_USER_GANG, handler(self, self.onUserGang_)},  -- 玩家提 
        {COMMANDS.HS_MJ_USER_HU, handler(self, self.onUserHu_)},  -- 胡牌
        {COMMANDS.HS_MJ_USER_CHI, handler(self, self.onUserChi_)},  -- 吃牌
        {COMMANDS.HS_MJ_USER_PENG, handler(self, self.onUserPeng_)},  -- 碰牌
        {COMMANDS.HS_MJ_USER_MO_PAI, handler(self, self.onPlayerMoPai_)},  
        {COMMANDS.HS_MJ_ALL_PASS, handler(self, self.onAllPass_)},
        {COMMANDS.HS_MJ_NOTIFY_HU, handler(self, self.onNotifyHu_)},  -- 通知胡牌请求
        {COMMANDS.HS_MJ_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
        {COMMANDS.HS_MJ_PLAY_ACTION, handler(self, self.onPlayAction_)},
        {COMMANDS.HS_MJ_UPDATE_SCORE, handler(self, self.onUpdateScore_)},

        {COMMANDS.HS_MJ_USER_AFTER_GANG, handler(self, self.onCSMJUserGang_)},
        {COMMANDS.HS_MJ_USER_BU_CARD, handler(self, self.onUserGang_)},

        {COMMANDS.HS_MJ_BEGIN_CHUI, handler(self, self.onsBeginChui_)},
        {COMMANDS.HS_MJ_PLAYER_CHUI, handler(self, self.onPlayerChui_)},

        {COMMANDS.HS_MJ_GET_ALL_PLAYER_CARDS, handler(self, self.onGetAllPlayerCards_)},
    }
...
function TableController:onChuPaiEvent_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.HS_MJ_CHU_PAI, data)
end
...
function TableController:doPlayerPass_(data)
    ...
    if data.seatID == self.hostSeatID_ then
        ...
        dump(self.currentMoCard_,"self.currentMoCard_")
...
function TableController:changeViewByRule(rules)
    local texture = nil
    if rules.paiType == 1 then
        texture = cc.Director:getInstance():getTextureCache():addImage("res/images/majiang/logo_hs_ks.png")
    else
        texture = cc.Director:getInstance():getTextureCache():addImage("res/images/majiang/logo_hs_pt.png")
    end
    display.getRunningScene().title_:setTexture(texture)
end
...
function TableController:doTurnTo_(data)
    if data.outCards ~= nil then
        ...
        if not isRight then
            dataCenter:sendOverSocket(COMMANDS.HS_MJ_GET_ALL_PLAYER_CARDS)
        end
    end
...
function TableController:changeUserChuPaiCount_()
    local rule = self:getRoomConfig()
    dump(rule,"rulerulerulerulerulerulerule")
