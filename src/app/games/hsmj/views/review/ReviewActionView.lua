...
function ReviewActionView:lightByActions_(actions)
    ...
    if table.indexof(actions, MJ_ACTIONS.PENG) then
        self.spritePeng_:show()
    end
    if table.indexof(actions, MJ_ACTIONS.CHI) then
        self.spriteChi_:show()
    end
    if table.indexof(actions, MJ_ACTIONS.AN_GANG) or 
        table.indexof(actions, MJ_ACTIONS.CHI_GANG) or 
        table.indexof(actions, MJ_ACTIONS.BU_GANG) then
        self.spriteGang_:show()
    end
    if table.indexof(actions, MJ_ACTIONS.CHI_HU) or 
        table.indexof(actions, MJ_ACTIONS.ZI_MO) then
        self.spriteHu_:show()
    end
end
...
function ReviewActionView:showHand_(direction, data)
    local index = 6
    if MJ_ACTIONS.CHI == data.actType then
        index = 1
    elseif MJ_ACTIONS.PENG == data.actType then
        index = 2
    elseif table.indexof({MJ_ACTIONS.CHI_GANG, MJ_ACTIONS.AN_GANG, MJ_ACTIONS.BU_GANG}, data.actType) then
        index = 3
    elseif table.indexof({MJ_ACTIONS.CHI_HU, MJ_ACTIONS.ZI_MO}, data.actType) then
        index = 5
    end
