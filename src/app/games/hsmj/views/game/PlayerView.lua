...
local HSAlgorithm = require("app.games.hsmj.utils.HSAlgorithm")

local TingView = require("app.games.csmj.views.game.TingView")
...
local nodeData = {
    ...
    children = {
        ...
        {type = TYPES.NODE, var = "nodePlayer_", children = {  -- 玩家基本数据的对象
            ...
            {type = TYPES.SPRITE, var = "spriteBg_", filename = "res/res/images/common/avatar_border.png",},
...
function PlayerView:onChuiEvent_(event)
...
function PlayerView:onSitDown(player)
    ...
    local events = {
        ...
        {cls.GAME_TG, handler(self, self.onGameTG_)},
        {cls.AUTO_OUT_CARD, handler(self, self.onAutoOutCard_)},
    }
  
    gailun.EventUtils.create(self, player, self, events)

    local clsTable = self.table_
    local eventsTable = {
        {clsTable.MA_JIANG_HIGH_LIGHT, handler(self, self.onHighLight)},
    }
    gailun.EventUtils.create(self, self.table_, self, eventsTable)
...
function PlayerView:onAutoOutCard_()
    if not self.isGameTg_ then
        return
    end
    if not self:canChuPai_() then
        return
    end
    local canTing,maJiang = self:getMoPaiCard_()
    if canTing then  
        return self:onMaJiangChuAction_(maJiang)
    end
end

function PlayerView:onGameTG_(event)
    self.isGameTg_ = event.isLock or false
    if self.index_ == 1 then
        if self.isGameTg_ then
...
function PlayerView:createTanPai_(data, index, isReview)
    assert(data.cards)
...
function PlayerView:showHuType(huList,cards,winner, isReview)
    local actions = {}
    local hutypes = {}
    for i = 1, #huList do
        ...
        if not hutype then
            ...
            if not png then
                ...
                local action =   cc.CallFunc:create(function ()
                    ...
                    transition.scaleTo(hutype, {scale = 0.8, time = 0.2, easing = "bounceOut"})
...
function PlayerView:calcWaiPaiPosRight_(index, offset)
    print("============calcWaiPaiPosRight_============",index, offset)
    local majiang_width = MA_JIANG_DAO_WIDTH[self.index_]
    local bottom_margin, wai_pai_space = 180, 20
    local x = display.width * (1114 / DESIGN_WIDTH)- offset*4 - index *15 + 60
    if offset == 4 then
        x = x + 11
    end
...
-- 390 890
function PlayerView:calcWaiPaiPosTop_(index, offset)
...
function PlayerView:calcWaiPaiPosLeft_(index, offset)
    ...
    local x = display.width * (166 / DESIGN_WIDTH) - offset*4 - index *15 + 60
    if offset == 4 then
        x = x + 4
    end
...
function PlayerView:onHandCardRemoved_(event)
    print("PlayerView onHandCardRemoved_"..event.card)
...
function PlayerView:calcChuPaiPosBottom_(total, index)
    ...
    local isAdd = false
    if index > self.cpLian*3 then
        index = index - self.cpLian*3
        isAdd = true
    end
...
function PlayerView:calcChuPaiPosTop_(total, index)
    ...
    local y = display.height * (display.height - 250) / DESIGN_HEIGHT + math.floor((index - 1) / self.cpLian) * 40 +30
    if isAdd then
        y = y + 15
    end
    return x, y, -index
...
function PlayerView:calcChuPaiPosLeft_(total, index)
    ...
    local x = display.width * 340 / DESIGN_WIDTH - math.floor((index - 1) / self.cpLian) * 18 - index * 4 +70
...
function PlayerView:createMaJiangWithTouch_(card, index, x, y, z, isDown, isMoPai)
    ...
    maJiang:scale(MA_JIANG_BOTTOM_SCALE)
    if self.isGameTg_ and not maJiang:isMoPai() then
...
function PlayerView:onMaJiangChuAction_(maJiang)
    ...
    dataCenter:sendOverSocket(COMMANDS.HS_MJ_CHU_PAI, {
        cards = card
    })
    if self.player_:canZiMoHu() then
...
function PlayerView:onMaJiangTouchEnded_(maJiang, event)
    ...
    if maJiang.clickTime_ then
        if maJiang.clickTime_ > 0 and clickTime - maJiang.clickTime_ < 0.6 then
            return self:onMaJiangChuAction_(maJiang)
        end
    end
...
function PlayerView:onMaJiangTouchdBegin_(maJiang, event)
    ...
    local remainCards = HSAlgorithm.getRemainCards(handCards, zhuoCards, waiCards,display.getRunningScene():getTable().getRuleDetails().fengPai == 1)
    local tingPai = HSAlgorithm.getTingPai(remainCards, 
        nowCards,display.getRunningScene():getTable().getRuleDetails().fengPai == 1)
...
function PlayerView:onMaJiangFound_(event)
    print("===========onMaJiangFound_onMaJiangFound_===========",#self.player_:getCards())
...
function PlayerView:updateTingPaiTag(card)
    ...
    if HSAlgorithm.canZiMoHu(handCards, true) then
        self.tingPaiMap = {}
    else
        self.tingPaiMap = gailun.utils.invert(HSAlgorithm.getTingOperate(handCards,display.getRunningScene():getTable().getRuleDetails().fengPai == 1))
    end
    ...
    local canTing,card = self:getMoPaiCard_()
    if canTing then
        display.getRunningScene():getTable():showTG()
    end
end

function PlayerView:getMoPaiCard_()
    if not self:isHost() then
        return
    end
    local nodes = self:getMaJiangNode_():getChildren()
    ...
    return self.tingPaiMap[moPaiCard:getCard()] ~= nil,moPaiCard
end
...
function PlayerView:initPosByIndex_()
    ...
    elseif self.index_ == MJ_TABLE_DIRECTION.LEFT then
        self.nodeWaiPai_:setSkewX(4)
        self.voiceQiPao_:setFlipX(false)
        self.voiceQiPao_:setPositionX(qiPaoX) 
        self.tanFenAdd_:setPositionX(tanFenX)
        self.tanFenDel_:setPositionX(tanFenX)
    elseif self.index_ == MJ_TABLE_DIRECTION.RIGHT then
        self.nodeWaiPai_:setSkewX(-3)
        qiPaoX = - QI_PAO_X
        self.voiceQiPao_:setPositionX(qiPaoX)
