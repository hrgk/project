...
function ActionButtonsView:getMultiChoose_(action, publicCard)
    ...
    if MJ_ACTIONS.AN_GANG == action then
        return player:calcAnGang()
    elseif MJ_ACTIONS.BU_GANG == action then
        return player:calcBuGang()
    elseif MJ_ACTIONS.CHI_GANG == action then
...
function ActionButtonsView:onPassClicked_(event)
    ...
    -- if table.indexof(actions, MJ_ACTIONS.ZI_MO) ~= false then
...
function ActionButtonsView:showActions(gameType)
    ...
    if table.indexof(actions, MJ_ACTIONS.CHI) == false then
        table.removebyvalue(buttons, self.buttonChi_)
        table.insert(hides, self.buttonChi_)
    end

    if table.indexof(actions, MJ_ACTIONS.PENG) == false then
        table.removebyvalue(buttons, self.buttonPeng_)
        table.insert(hides, self.buttonPeng_)
    end

    if table.indexof(actions, MJ_ACTIONS.ZI_MO) == false and
        table.indexof(actions, MJ_ACTIONS.CHI_HU) == false and
        table.indexof(actions, MJ_ACTIONS.QIANG_GANG_HU) == false then
        table.removebyvalue(buttons, self.buttonHu_)
        table.insert(hides, self.buttonHu_)
    end

    if table.indexof(actions, MJ_ACTIONS.CHI_GANG) == false and
        table.indexof(actions, MJ_ACTIONS.AN_GANG) == false and
        table.indexof(actions, MJ_ACTIONS.BU_GANG) == false then
        table.removebyvalue(buttons, self.buttonGang_)
        table.insert(hides, self.buttonGang_)
    end

    if table.indexof(actions, MJ_ACTIONS.BU_ZHANG) == false then
    ...
    self:showCardAfterButtons_(actions)
    local mjTable = display.getRunningScene():getTable()
    -- if mjTable:getRuleDetails().qiangZhiHu == 1 then  
    --     if table.indexof(actions, MJ_ACTIONS.CHI_HU) then
    --         self.buttonPass_:hide()
    --     end
    --     if table.indexof(actions, MJ_ACTIONS.CHI_HU) and table.indexof(actions, MJ_ACTIONS.PENG) then
    --         self.buttonPeng_:hide()
    --     end
    -- end
end
function ActionButtonsView:showOperates(operates,gameType,needAuto,isCanTing)
    dump(operates, "showOperates")
    print("xxxxxxxxxxxxx",needAuto,isCanTing)
    self.nodeTmpMaJiangs_:removeAllChildren()
    self:hideMultiChoose_()
    ...
    local mjTable = display.getRunningScene():getTable()
    if needAuto then
        if table.indexof(actions, MJ_ACTIONS.ZI_MO) 
            or table.indexof(actions, MJ_ACTIONS.CHI_HU) 
            or table.indexof(actions, MJ_ACTIONS.QIANG_GANG_HU)  then
                self:onHuClicked_()
                return
        end
        if table.indexof(actions, MJ_ACTIONS.CHI_GANG) or
            table.indexof(actions, MJ_ACTIONS.AN_GANG)  or
            table.indexof(actions, MJ_ACTIONS.BU_GANG)  then
                if isCanTing then
                    self:onGangClicked_()
                    return
                end
...
-- 计算公共时间的可以进行的动作
function ActionButtonsView:calcActionsInPublicTime_(player, allowChiHu)
    ...
    if isBuGang then
        if allowChiHu and player:canChiHu(publicCard) then
            table.insert(actions, MJ_ACTIONS.QIANG_GANG_HU)
        end
        return actions
    end
    ...
    if allowChiHu and player:canChiHu(publicCard) then
        table.insert(actions, MJ_ACTIONS.CHI_HU)
    end
    if player:canPeng(publicCard) then
        table.insert(actions, MJ_ACTIONS.PENG)
    end
    if player:canChiGang(publicCard) then
        table.insert(actions, MJ_ACTIONS.CHI_GANG)
    end
    return actions
end

-- 计算摸牌时间可以进行的动作
function ActionButtonsView:calcActionsInMoPaiTime_(player, allowChiHu)
    local actions = {}
    if player:canZiMoHu() then
        table.insert(actions, MJ_ACTIONS.ZI_MO)
    end
    if player:canAnGang() then
        table.insert(actions, MJ_ACTIONS.AN_GANG)
    end
    if player:canBuGang() then
        table.insert(actions, MJ_ACTIONS.BU_GANG)
    end
    return actions
end
...
function ActionButtonsView:calcCanDoActions(player)
    ...
    local allowChiHu = display.getRunningScene():getTable():getHuType() == 1
    print("=======allowChiHu===========",allowChiHu, display.getRunningScene():getTable():getHuType())
    local actions
    local inPublicTime = display.getRunningScene():getTable():getInPublicTime()
    print("inPublicTime = ", inPublicTime, allowChiHu)
    if inPublicTime then
        actions = self:calcActionsInPublicTime_(player, allowChiHu)
    else
        actions = self:calcActionsInMoPaiTime_(player, allowChiHu)
    end
    dump(actions)
    return actions
end

function ActionButtonsView:showCardAfterButtons_(actions)
    local card = display.getRunningScene():getTable():getLastCard()
    print("=========card============", card)
    if self.buttonChi_:isVisible() then
        local x1, y1 = self.buttonChi_:getPosition()
        app:createConcreteView("MaJiangView", card, MJ_TABLE_DIRECTION.BOTTOM, true):addTo(self.nodeTmpMaJiangs_):pos(x1 + 90, y1)
    end
    if self.buttonPeng_:isVisible() then
        local x2, y2 = self.buttonPeng_:getPosition()
        app:createConcreteView("MaJiangView", card, MJ_TABLE_DIRECTION.BOTTOM, true):addTo(self.nodeTmpMaJiangs_):pos(x2 + 90, y2)
    end
    ...
    local withChiGang = table.indexof(actions, MJ_ACTIONS.CHI_GANG) ~= false
    local withAnGang = table.indexof(actions, MJ_ACTIONS.AN_GANG) ~= false
    local withBuGang = table.indexof(actions, MJ_ACTIONS.BU_GANG) ~= false
    ...
    elseif withBuGang then  -- 补杠
        card = display.getRunningScene():getHostPlayer():getLastCard()
    elseif withAnGang then  -- 暗杠
        local cards = display.getRunningScene():getHostPlayer():getAllAnGang()
        if not cards or #cards > 1 or #cards == 0 then
            return
        end
        card = cards[1]
    end
    local x3, y3 = self.buttonGang_:getPosition()
    -- app:createConcreteView("MaJiangView", card, MJ_TABLE_DIRECTION.BOTTOM, true):addTo(self.nodeTmpMaJiangs_):pos(x3 + 90, y3)
end
