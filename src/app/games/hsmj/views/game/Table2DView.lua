...
local testData = require("app.games.hsmj.data.test")
...
tableNodes.normal = {
    ...
    children = {
        {type = TYPES.NODE, var = "nodeTableInfo_", children = {  -- 桌子信息容器
            ...
            {type = TYPES.CUSTOM, var = "directorController_", class = "app.games.hsmj.controllers.DirectorController", px = 0.5, py = display.cy + 30 * display.width / DESIGN_WIDTH},
        ...
        {type = TYPES.LABEL, var = "viewProgress_", type = TYPES.CUSTOM, class = "app.games.hsmj.views.game.ProgressView", x = display.cx, y = display.cy + 35},
        {type = TYPES.CUSTOM, var = "focusArrow_", class = "app.games.hsmj.views.game.FocusArrowView", px = 0.2, py = 0.5, visible = false},
        ...
        {type = TYPES.CUSTOM, var = "tingView_", class = "app.games.hsmj.views.game.Ting2DView", visible = true,x = display.left-80,y = display.bottom + 300},
...
function Table2DView:ctor(table)
    ...
    local debugView = DebugView.new(COMMANDS.HS_MJ_DEBUG_CONFIG_CARD, testData)
...
function Table2DView:onTingTouch_(sender, eventType)
    ...
    if eventType == 0 then
        ...
        local tingPai = HSAlgorithm.getTingPai(remainCards, 
            hostPlayer:getCards(),display.getRunningScene():getTable().getRuleDetails().fengPai == 1)
...
function Table2DView:onChuiClicked_(event)
    local data = {chui = 2}
    dataCenter:sendOverSocket(COMMANDS.HS_MJ_PLAYER_CHUI, data)
end
...
function Table2DView:makeRuleString(spliter)
    ...
    if data then
        dump(data,"datadatadatadatadatadata")
        if data.ruleDetails.paiType == 1 then
            table.insert(list, "快速场")
        end
        ...
        if data.ruleDetails.fengPai == 1 then
            table.insert(list, "带风牌")
        else
            table.insert(list, "不带风牌")
        end
