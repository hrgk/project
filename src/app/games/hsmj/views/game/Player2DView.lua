...
local TingView = require("app.games.hsmj.views.game.Ting2DView")
...
local MA_JIANG_CHU_WIDTH = {40, 27, 40, 27}
...
function Player2DView:onFlwoEvent_(event)
    print("Player2DView onflow event "..event.flag)
...
function Player2DView:onAutoOutCard_()
...
function Player2DView:onGameTG_(event)
...
function Player2DView:calcChuPaiPosBottom_(total, index)
    ...
    local y = display.height * 300 / DESIGN_HEIGHT + math.floor((index - 1) / self.cpLian) * 48 - 130
    return x, y, -index
end

function Player2DView:calcChuPaiPosRight_(total, index)
    local majiang_width = MA_JIANG_CHU_WIDTH[self.index_]
    local x = display.width * 935 / DESIGN_WIDTH - math.floor((index - 1) / self.cpLian) * 47+120
...
function Player2DView:calcChuPaiPosTop_(total, index)
    ...
    local y = display.height * (display.height - 250) / DESIGN_HEIGHT - math.floor((index - 1) / self.cpLian) * 48 +120
    return x, y, index
end
...
function Player2DView:calcChuPaiPosLeft_(total, index)
    local majiang_width = MA_JIANG_CHU_WIDTH[self.index_]
    local isAdd = false
    local x = display.width * 340 / DESIGN_WIDTH + math.floor((index - 1) / self.cpLian) * 48  -120
...
function Player2DView:calcMaJiangPosBottom_(total, index, isDown)
    ...
    local x = wai_width + wai_pai_space + (index - 1) * majiang_width + majiang_width / 2 -30
...
function Player2DView:createMaJiangWithTouch_(card, index, x, y, z, isDown, isMoPai)
    ...
    print("XXXXXXXXXXXXXXXMA_JIANG_BOTTOM_SCALE",isDown)
...
function Player2DView:onMaJiangChuAction_(maJiang)
    ...
    local tingView = self.runScene:getChildByName("tingView")
...
function Player2DView:onMaJiangTouchdBegin_(maJiang, event)
    ...
    if #table.values(tingPai) ~= 0 then
        ...
        tingView:setPosition(display.left-80,display.bottom + 300)
...
function Player2DView:getMoPaiCard_()
    if not self:isHost() then
        return
    end
    local nodes = self:getMaJiangNode_():getChildren()
