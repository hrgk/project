...
local XuanPaiView = import("app.games.nxlmz.views.game.XuanPaiView")
...
local TableController = import("app.games.nxlmz.controllers.TableController")
...
function GameScene:onEnterTransitionFinish()
    ...
    local GameSceneTest = require("app.games.nxlmz.scenes.GameSceneTest")
...
function GameScene:bindSocketListeners()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.ZMZ_GAME_START,handler(self, self.onGameStart_)},
        {COMMANDS.ZMZ_ROUND_OVER, handler(self, self.onRoundOver_)},
        {COMMANDS.ZMZ_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.ZMZ_READY, handler(self, self.onReady_)},
        {COMMANDS.ZMZ_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.ZMZ_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.ZMZ_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.ZMZ_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.ZMZ_ROUND_FLOW, handler(self, self.onRoundFlow_)},
        {COMMANDS.ZMZ_TUO_GUAN, handler(self, self.onTuoGuanHandler_)},
        -- {COMMANDS.ZMZ_CHECK_CHOU_JIANG, handler(self, self.onCheckChouJiang_)},
        -- {COMMANDS.ZMZ_CHOU_JIANG, handler(self, self.onChouJiang_)},
        {COMMANDS.ZMZ_XUAN_PAI, handler(self, self.onXuanPaiHandler_)},
        {COMMANDS.ZMZ_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.ZMZ_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},

        {COMMANDS.ZMZ_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.ZMZ_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.ZMZ_DEAL_CARD, handler(self, self.onDealCards_)},
        {COMMANDS.ZMZ_TURN_START, handler(self, self.onTurnStart_)},
        {COMMANDS.ZMZ_TURN_END, handler(self, self.onTurnEnd_)},
        {COMMANDS.ZMZ_ZHA_DAN_DE_FEN, handler(self, self.onZhaDanDeFen_)},
        {COMMANDS.ZMZ_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.ZMZ_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.ZMZ_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.ZMZ_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播
        {COMMANDS.ZMZ_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
    }
...
function GameScene:onTuoGuanHandler_(event)
    dump(event.data)
    if event.data.code == -1 then
        return
    end
    self.tableController_:doPlayerTuoGuan(event.data)
end
...
function GameScene:onPlayerChuPai_(event)
    if event.data.isReview then
        ...
        TaskQueue.add(handler(self.tableController_, self.tableController_.doPlayerChuPai), 0, 0.1, event.data)
    end
end
...
function GameScene:onTurnTo_(event)
    ...
    if event.data.isReview then
        ...
        TaskQueue.add(handler(self.tableController_, self.tableController_.doTurnTo), 0, 0, event.data)
    end
end
...
function GameScene:onNotifyLocation_(event)
    ...
    if event.data.code == 0 then
        ...
        if date.config.rules.playerCount == 2 then
            ...
            self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,true,GAME_FHLMZ):addTo(self.window_)
...
function GameScene:onRequestLocation_(event)
    if event.data.code == 0 then
        ...
        self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,false,GAME_FHLMZ):addTo(self.window_)
        self.dingWeiView_:tanChuang(150)
    end
end
...
function GameScene:onJieSan()
    ...
    app:createView("game.DismissRoomView", msg, function (isOK)
        self.isKeyClick_ = false
            if isOK then
                dataCenter:sendOverSocket(COMMANDS.ZMZ_OWNER_DISMISS)
            end
...
function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.ZMZ_REQUEST_DISMISS, {agree = isAgree})
end

function GameScene:sendXuanPaiCMD()
    dataCenter:sendOverSocket(COMMANDS.ZMZ_XUAN_PAI)
end

function GameScene:sendDingWeiCMD()
    dataCenter:sendOverSocket(COMMANDS.ZMZ_REQUEST_LOCATION)
end

function GameScene:sendTuiChuCMD()
    dataCenter:sendOverSocket(COMMANDS.ZMZ_LEAVE_ROOM)
end
