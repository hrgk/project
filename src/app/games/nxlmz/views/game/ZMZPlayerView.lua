...
function PDKPlayerView:setNode(node)
    ...
    params.options = {text= "16", w = 27, h = 40, startChar = "0"}
...
function PDKPlayerView:onPlayerChuHandler_(event)
    ...
    local cards = ZMZAlgorithm.sortOutPokers(clone(event.cards),display.getRunningScene():getRuleDetails())
    view:showPokers(clone(cards))
    self:chuPaiAnimotion_(view)
    self.chuPaiContent_:addChild(view)
    local result = ZMZAlgorithm.getCardType(clone(event.cards),display.getRunningScene():getRuleDetails())
...
function PDKPlayerView:playSound_(result)
    local sound
    if result.cardType == ZMZAlgorithm.DAN_ZHANG then
        sound = result.value .. ".mp3"
    elseif result.cardType == ZMZAlgorithm.DUI_ZI then
        sound = "d"..result.value .. ".mp3"
    elseif result.cardType == ZMZAlgorithm.SI_ZHA  then
        sound = "zhadan.mp3"
    ...
    elseif result.cardType == ZMZAlgorithm.SHUN_ZI then
        sound = "shunzi.mp3"
        local animaData = FaceAnimationsData.getCocosAnimation(31)
        gameAnim.createCocosAnimations(animaData, self.bombLayer_)
    elseif result.cardType == ZMZAlgorithm.SAN_DAI then
        sound = "sandai0.mp3"
    elseif result.cardType == ZMZAlgorithm.SAN_DAI_DAN then
        sound = "sandai1.mp3"
    elseif result.cardType == ZMZAlgorithm.SAN_DAI_DUI then
        sound = "sandai2.mp3"
    elseif result.cardType == ZMZAlgorithm.LIAN_DUI then
        sound = "liandui.mp3"
    elseif result.cardType == ZMZAlgorithm.FEI_JI_DAN or result.cardType == ZMZAlgorithm.FEI_JI_DUI or tresult.cardType == ZMZAlgorithm.FEI_JI then
        sound = "feiji.mp3"
...
function PDKPlayerView:getChuPaiWord_(result)
    local animaData
    if result.cardType == ZMZAlgorithm.LIAN_DUI then
        animaData = FaceAnimationsData.getCocosAnimation(19)
    elseif result.cardType == ZMZAlgorithm.SAN_DAI_DUI then
        animaData = FaceAnimationsData.getCocosAnimation(20)
    elseif result.cardType == ZMZAlgorithm.FEI_JI_DAN or result.cardType == ZMZAlgorithm.FEI_JI_DUI or result.cardType == ZMZAlgorithm.FEI_JI then
