...
local BaseAlgorithm = import("app.games.nxlmz.utils.BaseAlgorithm")
local ZMZAlgorithm = import("app.games.nxlmz.utils.ZMZAlgorithm")
local PokerTable = import("app.games.nxlmz.models.PokerTable")
...
function TableController:init_()
    self.tableView_ = app:createConcreteView("game.ZMZTableView", self.table_)
...
function TableController:doTurnTo(data)
    ...
    if data.seatID == self.hostSeatID_ then
        print("==========self.table_:isTuoGuan()============",self.table_:isTuoGuan())
        if data.yaoDeQi == true then
            self:showPlayController(true, 1)
            if self.table_:isTuoGuan() then
                self:doPlayerTuoGuan({isTuoGuan = true})
            end
        elseif data.yaoDeQi == false then
            self:showPlayController(true, 2)
            if self.table_:isTuoGuan() then
                display.getRunningScene():playerPass()
            end
        end
        if data.canPass == false then
            self:showPlayController(true, 3)
        end
    else
...
function TableController:onChuPaiEvent_(event)
    ...
    --     local hasHei3 = ZMZAlgorithm.hasHeiSan(cards)
    --     local handHei3 = ZMZAlgorithm.hasHeiSan(self.hostPlayer_:getCards())
    --     if handHei3 and not hasHei3 then
    --         app:showTips("第一手牌必须包含红3!")
    --         return
    --     end
    -- end
    local data = {cards = cards}
    -- local config = dataCenter:getPokerTable():getConfigData()
    -- if ZMZAlgorithm.isBigger(cards, self.table_:getCurCards(), config) then
    dataCenter:sendOverSocket(COMMANDS.ZMZ_CHU_PAI, data)
    -- else
...
function TableController:doResumeRoomInfo_(data)
    ...
    for k,v in pairs(self.seats_) do
        ...
        if v:isHost() then
            isTuoGuan = v:isTuoGuan()
            self.table_:showTuoGuanAnima(v:isTuoGuan())
            if v:getCards() and #v:getCards() <=5 then
                self.table_:showTuoGuanBtn(true)
            else
                self.table_:showTuoGuanBtn(false)
            end
        end
    end
    ...
    self.table_:setTableSkin(setData:getCDPHZBgIndex())
end
...
function TableController:resumePlayer_(player, data)
    ...
    player:setTuoGuan(data.isTuoGuan)
...
function TableController:doPlayerChuPai(data)
    ...
    if player:isHost() then
        ...
        if data.warning <= 5 then
            self.table_:showTuoGuanBtn(true)
        end
    end
end

function TableController:doPlayerTuoGuan(data)
    print("===========doPlayerTuoGuan=============")
    if not self.table_:isTuoGuan() then
        return 
    end
    local chuCards = self.table_:getCurrCards()
    if #chuCards == 0 then
        chuCards = {0}
    end
    local player = self:getPlayerBySeatID(self.hostSeatID_)
    local nextSeatID = player:getNextPlayer()
    local nextPlayer = self:getPlayerBySeatID(nextSeatID)
    print("========nextPlayer:getWarningType()============",nextPlayer:getWarningType())
    local list = ZMZAlgorithm.tishi(chuCards, clone(player:getCards()), self.table_:getRuleDetails())
    if #list == 0 then
        display.getRunningScene():playerPass()
        return
    end
    if nextPlayer:getWarningType() == 1 then
        local function callfunc(a, b)
            local resulta = ZMZAlgorithm.getCardType(a, self.table_:getRuleDetails())
            local resultb = ZMZAlgorithm.getCardType(b, self.table_:getRuleDetails())
            if resulta.cardType == resultb.cardType then
                return resulta.value > resultb.value
            else
                return resulta.cardType > resultb.cardType
            end
        end
        table.sort(list, callfunc)
    else
        local function callfunc(a, b)
            local resulta = ZMZAlgorithm.getCardType(a, self.table_:getRuleDetails())
            local resultb = ZMZAlgorithm.getCardType(b, self.table_:getRuleDetails())
...
function TableController:doRoundOver(data)
    ...
    self.table_:showTuoGuanAnima(false)
    self.table_:showTuoGuanBtn(false)
    self.table_:setTuoGuan(false)
end
