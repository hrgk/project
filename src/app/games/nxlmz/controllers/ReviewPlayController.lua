...
function ReviewPlayController:ctor(data, hostPlayer)
    ...
    for i,v in ipairs(data) do
        if (not v.msg.type or v.msg.type ~= "recv") and (v.cmd ~= COMMANDS.MJ_PLAYER_PASS) then
            table.insert(self.gameData_,v)
        end
    end
    self.deleCommondList_ = {
        COMMANDS.ZMZ_PLAYER_ENTER_ROOM,
        COMMANDS.ZMZ_ROUND_FLOW,
        COMMANDS.ZMZ_DEAL_CARD,
        COMMANDS.ZMZ_ROUND_OVER,
        COMMANDS.ZMZ_REQUEST_DISMISS,
        COMMANDS.ZMZ_ROUND_FLOW,
    }
end
...
function ReviewPlayController:filterCmd_()
    ...
    for i,v in ipairs(self.deleCommondList_) do
        for i,v in ipairs(self.gameData_) do
            if v.cmd == COMMANDS.ZMZ_PLAYER_ENTER_ROOM then
                table.insert(self.playerList_, v)
                table.removebyvalue(self.gameData_, v)
                break
            elseif v.cmd == COMMANDS.ZMZ_ROOM_INFO then
                roomInfo = clone(v)
                table.insert(self.commondList_, clone(v))
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.ZMZ_DEAL_CARD then
                table.insert(self.commondList_, clone(v))
                table.removebyvalue(self.gameData_, v)
                break
            end
