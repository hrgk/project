...
function BaseAlgorithm.isBird2(card, startIndex, playerCount)
    ...
    elseif playerCount == 2 then
        return false ~= table.indexof(BIRD_SAN_ER_VALUES2, BaseAlgorithm.getIndex(card, startIndex, playerCount))
    end
end
...
function BaseAlgorithm.isBird3(card, startIndex, playerCount)
    ...
    elseif playerCount == 2 then
        return false ~= table.indexof(BIRD_SAN_ER_VALUES3, BaseAlgorithm.getIndex(card, startIndex, playerCount))
    end
end
...
function BaseAlgorithm.isBird(card, isZiMo, isSortBird, isTongPao, jiePaoIndex, fangPaoIndex, playerCount)
    local fn = {}
    jiePaoIndex = jiePaoIndex or {}
    jiePaoIndex[1] = jiePaoIndex[1] or 1
    fangPaoIndex = fangPaoIndex or 1

    local birdShunXuList = {}
    ...
    if isTongPao then
        ...
        if isSortBird then
            ...
            table.insert(fn, {birdShunXu[1], jiePaoIndex[1]})
        end
    end
    for i=1, #fn do
        if fn[i] then
    ...
    for i = 1, count, 3 do
        ...
        if  1 == table.removebyvalue(list, v, false) and
            ...
            1 == table.removebyvalue(list, v + 2, false) then
                gailun.utils.extends(result, {v, v + 1, v + 2})
        else
            return false, {}
        end
    end
