...
function ActionButtonsController:onEnter()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.FHHZMJ_CHU_PAI, handler(self, self.onPlayerChuPai_)},  -- 某人出牌
...
function ActionButtonsController:onChiClose(chi_path)
    if chi_path then
        ...
        dataCenter:sendOverSocket(COMMANDS.FHHZMJ_USER_CHI, data)
    end 
    self:onActionEnd()
end
...
function ActionButtonsController:onPengClicked_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_USER_PENG, data)
...
function ActionButtonsController:onAnGangClicked_(event)
    ...
    local data = {}
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_USER_GANG, data)
    self.player_:stopZhuanQuanAction()
end
...
function ActionButtonsController:onBuClicked_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_USER_BU_CARD, data)
    self.player_:stopZhuanQuanAction()
end
...
function ActionButtonsController:onZiMouClicked_(event)
    print("onHU8Clicked_:")
    local data = {}
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_USER_HU, data)
    self.player_:stopZhuanQuanAction()
end
...
function ActionButtonsController:sendPlayAction_(action, params)
    ...
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_PLAY_ACTION, data)
    self.player_:stopZhuanQuanAction()
end
...
function ActionButtonsController:onPassClicked_(event)
    ...
    local data = {cards = card}
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_PLAYER_PASS, data)
    self.player_:stopZhuanQuanAction()
