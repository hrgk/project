...
-- require ("app.games.fhhzmj.utils.test")
...
function GameScene:onEnterTransitionFinish()
    ...
    self:onBgLoaded_()
    self:onAllLoaded_()
    local GameSceneTest = require("app.games.fhhzmj.scenes.GameSceneTest")
...
function GameScene:onEnter_()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.FHHZMJ_ROUND_OVER, handler(self, self.onRoundOver_)},
        {COMMANDS.FHHZMJ_GAME_OVER, handler(self, self.onGameOver_)},
        {COMMANDS.LOGIN, handler(self, self.onLoginReturn_)},
        {COMMANDS.FHHZMJ_ENTER_ROOM, handler(self, self.onMJEnterRoom_)},
        {COMMANDS.FHHZMJ_READY, handler(self, self.onReady_)},
        {COMMANDS.FHHZMJ_LEAVE_ROOM, handler(self, self.onLeaveRoom_)},
        {COMMANDS.FHHZMJ_PLAYER_ENTER_ROOM, handler(self, self.onPlayerEnterRoom_)},
        {COMMANDS.FHHZMJ_REQUEST_DISMISS, handler(self, self.onDismissInRequest_)},
        {COMMANDS.FHHZMJ_ROOM_INFO, handler(self, self.onRoomInfo_)},
        {COMMANDS.CHANGE_SERVER, handler(self, self.onChangeServer_)},
        {COMMANDS.FHHZMJ_DEBUG_CONFIG_CARD, handler(self, self.onConfigCard_)},
        {COMMANDS.FHHZMJ_NOTIFY_LOCATION, handler(self, self.onNotifyLocation_)},
        {COMMANDS.FHHZMJ_REQUEST_LOCATION, handler(self, self.onRequestLocation_)},
        --{COMMANDS.FHHZMJ_SHOW_BIRDS, handler(self,self.onShowBirds_)},
    }
...
function GameScene:onStartClicked_()
    self.buttonStart_:hide()
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_READY)
end

function GameScene:onWanFa_(event)
    local date = display.getRunningScene():getTable():getConfigData()
    display.getRunningScene():initWanFa(GAME_FHHZMJ,date)
end
...
function GameScene:onDingWeiClicked_(event)
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_REQUEST_LOCATION)
end
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
             dataCenter:sendOverSocket(COMMANDS.FHHZMJ_LEAVE_ROOM)
        end
    end
...
function GameScene:onNotifyLocation_(event)
    ...
    if event.data.code == 0 then
        ...
        elseif 4 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SiRenDingWei.new(params,event.data.distances,true,GAME_FHHZMJ):addTo(self.layerWindows_)
            self.dingWeiView_:tanChuang(150)
        elseif 3 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,true,GAME_FHHZMJ):addTo(self.layerWindows_)
            self.dingWeiView_:tanChuang(150)
        end
    end
end
...
function GameScene:onRequestLocation_(event)
    ...
    if event.data.code == 0 then
        ...
        if 4 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SiRenDingWei.new(params,event.data.distances,false,GAME_FHHZMJ):addTo(self.layerWindows_)
        elseif 3 == self:getTable():getRuleDetails().totalSeat then
            self.dingWeiView_ = SanRenDingWei.new(params,event.data.distances,false,GAME_FHHZMJ):addTo(self.layerWindows_)
        end
        self.dingWeiView_:tanChuang(150)
    end
end
...
function GameScene:onDismissClicked_(event)
    ...
    local function callback(bool)
        if bool then
            selfData:setNowRoomID(0)
             dataCenter:sendOverSocket(COMMANDS.FHHZMJ_OWNER_DISMISS)
        end
    end
    app:confirm(msg, callback)
end
...
function GameScene:onMJEnterRoom_(event)
    if (display.getRunningScene().name == "GAME_FHHZMJ2D" and setData:getMJHMTYPE()+0  == 2) or (display.getRunningScene().name == "GAME_FHHZMJ3D" and setData:getMJHMTYPE()+0  == 1) then
        dataCenter:pauseSocketMessage()
        local roomID = event.data.roomID
        dataCenter:setRoomID(roomID)
        gailun.EventUtils.clear(self)
...
function GameScene:onConfigClicked_(event)
    display.getRunningScene():showGameSheZhi(true,GAME_FHHZMJ)
end

function GameScene:sendDismissRoomCMD(isAgree)
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_REQUEST_DISMISS, {agree = isAgree})
end
...
function GameScene:onExitClicked_(event)
    ...
    dataCenter:sendOverSocket(COMMANDS.FHHZMJ_QUIT_TABLE)
end
