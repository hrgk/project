...
function PokerListView:fileterSelectedPokers_(pokers)
    ...
    local list = PdkAlgorithm.checkPaiXing(cards, display.getRunningScene():getRuleDetails())
...
function PokerListView:showPokers(cards, isAnima, isBack)
    ...
    -- else
        local tempCards = PdkAlgorithm.sort(cards)
...
function PokerListView:showPokersWithoutAnim_(cards, isAnima, callback, isback)
    ...
    if isAnima then
        ...
        table.insert(actions, cc.CallFunc:create(function ()
            local tempCards = PdkAlgorithm.sort(clone(cards))
...
function PokerListView:faPaiRank_(cards)
    local cards = PdkAlgorithm.sort(cards)
...
function PokerListView:initTishi(isBaoDan)
    if self.isTips_ == false then
        ...
        local list = PdkAlgorithm.tishi(cards, self:getCards_(), display.getRunningScene():getRuleDetails(),isBaoDan)
