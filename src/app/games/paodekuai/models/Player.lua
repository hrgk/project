...
function Player:reShowHandCards(isNeedReset)
    ...
    self:dispatchEvent({name = Player.RESET_HAND_CARDS, cards = self.handCards_,isReConnect = true,isNeedReset = isNeedReset})
end
...
function Player:showRoundOverPoker(cards)
    ...
    if cards and #cards > 0 then 
        tempCards  = PdkAlgorithm.sort(cards)    
    end
