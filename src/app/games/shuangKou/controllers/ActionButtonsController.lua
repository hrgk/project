...
function ActionButtonsController:initEvents_()
    ...
    local handlers = dataCenter:s2cCommandToNames {
        {COMMANDS.SHUANGKOU_CHU_PAI, handler(self, self.onPlayerChuPai_)},  -- 某人出牌
...
function ActionButtonsController:sendPlayAction_(action, params)
    ...
    dataCenter:sendOverSocket(COMMANDS.SHUANGKOU_PLAY_ACTION, data)
end

function ActionButtonsController:onPassClicked_(event)
    print("ActionButtonsController:onPassClicked_")
...
function ActionButtonsController:onChuPaiClicked_(event)
    self:getPlayer():dispatchChuPaiEvent(event)
end
...
function ActionButtonsController:showHanFen(list)
    self.hanFenList_ = list
    self.view_:showHanFen(list)
end

function ActionButtonsController:getHanFenList()
    return self.hanFenList_
end
