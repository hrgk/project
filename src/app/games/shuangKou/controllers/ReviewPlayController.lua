...
function ReviewPlayController:ctor(data)
    ...
    self.deleCommondList_ = {
        COMMANDS.SHUANGKOU_PLAYER_ENTER_ROOM,
        COMMANDS.SHUANGKOU_ROUND_FLOW,
        COMMANDS.SHUANGKOU_DEAL_CARD,
        COMMANDS.SHUANGKOU_ROUND_OVER,
        COMMANDS.SHUANGKOU_REQUEST_DISMISS,
        COMMANDS.SHUANGKOU_ROUND_FLOW,
    }
end
...
function ReviewPlayController:filterCmd_()
    ...
    for i,k in ipairs(self.deleCommondList_) do
        for i,v in ipairs(self.gameData_) do
            if v.cmd == COMMANDS.SHUANGKOU_PLAYER_ENTER_ROOM then
                self.playerList_[v.msg.seatID] = v
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.SHUANGKOU_DEAL_CARD then
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.SHUANGKOU_REQUEST_DISMISS then
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.SHUANGKOU_ROUND_FLOW then
                table.removebyvalue(self.gameData_, v)
            elseif v.cmd == COMMANDS.SHUANGKOU_ROOM_INFO then
                table.removebyvalue(self.gameData_, v)
