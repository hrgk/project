...
local BaseAlgorithm = import("app.games.yzchz.utils.BaseAlgorithm")
local TaskQueue = require("app.controllers.TaskQueue")
local TableModel = import("app.games.yzchz.models.PokerTable")
...
local TABLE_TIME = 
{
    -- delaySeconds, afterSeconds
    onPlayerPass_ = {0, 0},
    onAllPass_ = {0.0618, 0.5},
    onDealCards_ = {0, 2},
    onPlayerChuPai_ = {0, 0.7},
    onNotifyHu_ = {0, 0},
    onEveryonePass_ = {0, 0.5},
    onPlayerMoPai_ = {0, 0.7},
    onUserHu_ = {0, 0},
    onUserPao_ = {0, 1},
    onUserWei_ = {0, 1},
    onUserChi_ = {0, 0},
...
function TableController:onEnter()
    local handlers = dataCenter:s2cCommandToNames { 
        {COMMANDS.YZCHZ_ROUND_START, handler(self, self.onRoundStart_)},
        {COMMANDS.YZCHZ_TURN_TO, handler(self, self.onTurnTo_)},
        {COMMANDS.YZCHZ_CHU_PAI, handler(self, self.onPlayerChuPai_)},
        {COMMANDS.YZCHZ_PLAYER_PASS, handler(self, self.onPlayerPass_)},
        {COMMANDS.YZCHZ_BROADCAST, handler(self, self.onBroadcast_)},  -- 广播消息
        {COMMANDS.YZCHZ_ONLINE_STATE, handler(self, self.onPlayerStateChanged_)},  -- 玩家网络事件广播

        {COMMANDS.YZCHZ_DEAL_CARD, handler(self, self.onDealCards_)},  -- 发牌
        {COMMANDS.YZCHZ_TIAN_HU_START, handler(self, self.onTianHuStart_)},  -- 开始天胡 
        {COMMANDS.YZCHZ_TIAN_HU_END, handler(self, self.onTianHuEnd_)},  -- 开始结束 
        {COMMANDS.YZCHZ_USER_TI, handler(self, self.onUserTi_)},  -- 玩家提 
        {COMMANDS.YZCHZ_USER_PAO, handler(self, self.onUserPao_)},  -- 玩家跑
        {COMMANDS.YZCHZ_USER_WEI, handler(self, self.onUserWei_)},  -- 玩家偎牌 
        {COMMANDS.YZCHZ_USER_HU, handler(self, self.onUserHu_)},  -- 胡牌
        {COMMANDS.YZCHZ_USER_CHI, handler(self, self.onUserChi_)},  -- 吃牌
        {COMMANDS.YZCHZ_USER_PENG, handler(self, self.onUserPeng_)},  -- 碰牌
        {COMMANDS.YZCHZ_USER_SI_SHOU, handler(self, self.onUserSiShou_)},  -- 碰牌
        {COMMANDS.YZCHZ_USER_MO_PAI, handler(self, self.onPlayerMoPai_)},  
        {COMMANDS.YZCHZ_ALL_PASS, handler(self, self.onAllPass_)},
        {COMMANDS.YZCHZ_NOTIFY_HU, handler(self, self.onNotifyHu_)},  -- 通知胡牌请求
        {COMMANDS.YZCHZ_OWNER_DISMISS, handler(self, self.onOwnerDismiss_)},
        {COMMANDS.YZCHZ_FIRST_HAND_TI, handler(self, self.onFirstHandTi_)},
        {COMMANDS.YZCHZ_HU_PASS, handler(self, self.onHuPass_)} 
    }
...
function TableController:onChuPaiEvent_(event)
    ...
    if event.card == BaseAlgorithm.KING then
        return
    end
...
function TableController:showPalyer4View()
    return self.tableView_:showPalyer4View()
end

function TableController:showCurrFlow_(flow,data)
    if data.config.rules.totalSeat == 4 then
        self.tableView_:showPalyer4View()
    end
    self.doFlowList_[flow](flow,data)
end

function TableController:showActions(chi,peng,hu,wc,wd,wz)
    self.actionButtonsController_:showActions(chi,peng,hu,wc,wd,wz)
end
...
function TableController:doResumeRoomInfo_(data)
    ...
    if self.dxOperates then
        self.actionButtonsController_:showCanActions(self.dxOperates)
        self.dxOperates = nil
    end
end
...
function TableController:doPlayerMoPai_(data)
    ...
    if data.isReview then
        if data.card == 301 then
            self.table_:doReviewShouZhang(data.seatID, data.card)
        end
    else
        if self.hostPlayer_ and data.seatID == self.hostPlayer_:getSeatID() and data.card == 301  then
        ...
        if data.card == 301 then
            local seq = transition.sequence({
                ...
                cc.CallFunc:create(function ()
                    ...
                    if self.hostPlayer_ and data.seatID == self.hostPlayer_:getSeatID() then
                        self.hostPlayer_:shouzhang(data.card)
                    end
                    showCardEnd()
                end)
            })
...
function TableController:doHuPai(data)
    ...
    local wAninType = nil
    for i = 1,#huList do
        if huList[i] == 19 or huList[i] == 20 then
            wAninType = 37
        elseif huList[i] == 21 or huList[i] == 22 then
            wAninType = 38
        elseif huList[i] == 23 or huList[i] == 24 then
            wAninType = 39
        end
    end
    local typeid = "100"
    ...
    player:setHuPai(typeid,wAninType)
end
...
function TableController:doUserSiShou_(data)
    if not data or not data.seatID then
        return
    end 
    local player = self:getPlayerBySeatID(data.seatID)
...
-- 玩家死手
function TableController:onUserSiShou_(event)
    if event.data.isReview then
        self:doUserSiShou_(event.data)
    else
        TaskQueue.add(handler(self, self.doUserSiShou_), TABLE_TIME.onUserPeng_[1], TABLE_TIME.onUserPeng_[2], event.data)
    end
end
...
function TableController:doPlayerSitDown(params)
    ...
    if playerData.playerCount then
        self.maxPlayer_ = playerData.playerCount
    else
        playerData.playerCount = self.maxPlayer_
    end
...
function TableController:doTianHuStart_(data)
    ...
    if index == 1 or data.isReview then
        self.table_:setCurCards(data.card, index, true)
    else
        self.table_:setCurCards(data.card, index, true, true)
    end
...
function TableController:checkHu(card, isHide)
    ...
    -- local hostCards = cardInfo[hostSeatID].handCards
    -- if card ~= nil then
    ...
    --     aimTable:doShowHuCard_(huCards) 
    -- end
end
