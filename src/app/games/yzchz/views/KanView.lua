...
local PaoHuZiAlgorithm = import("app.games.yzchz.utils.PaoHuZiAlgorithm")
...
function KanView:getCardPos(index, totalCount)
    local y = self.height - KAN_HEIGHT / 2 - (4-index) * getLineHeight(totalCount)
...
function KanView:clacSelfHuXi(cards)
    ...
    self.huXilabel_ = gailun.uihelper.createLabel({options = {text = huxi .."息", font = DEFAULT_FONT, size = 26, color = cc.c4b(255, 255, 255, 255)}})
    self.huXiBg_:setName("huXiLabel")
    self.huXiBg_:addChild(self.huXilabel_)
    self.huXiBg_:pos(45,125)
