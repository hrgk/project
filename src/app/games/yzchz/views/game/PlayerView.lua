...
local PlayAnim = require("app.games.yzchz.views.PlayAnim")
...
local player_pos = {
    {display.left+player_pos_adjustx, display.bottom + 80},
    {display.width - player_pos_adjustx, display.height - player_pos_adjusty+85},
    {display.left+player_pos_adjustx, display.height - player_pos_adjusty+85},
    {display.left+player_pos_adjustx, display.bottom + 80},
}
...
local cur_card_pos = 
{
    ...
    {display.width - mopaiX, display.height - mopai_pos_adjusty},
    {mopaiX, display.height - mopai_pos_adjusty},
    {display.cx, display.cy + 180},
}
...
local ready_pos = {
    ...
    {display.width - player_pos_adjustx - ready_pos_adjustx, player_pos[2][2]},
...
local THREW_PAI_POS = {  
    {display.width - throw_cards_adjustx, player_pos[1][2] + 280},
    {display.width - throw_cards_adjustx, player_pos[2][2] - throw_cards_adjusty+ 100},
    {throw_cards_adjustx, player_pos[3][2] - throw_cards_adjusty+100},
    {display.width - throw_cards_adjustx, player_pos[1][2] + 280},
}
...
--桌面吃碰偎提的位置
local zuomianpai_posx = 200
local zuomianpai_posy = 570

local ZHUOMIAN_PAI_POS = {  
    {10, display.bottom - 320},
    {display.width - zuomianpai_posx - 51, player_pos[2][2] - zuomianpai_posy},
    {zuomianpai_posx, player_pos[3][2] - zuomianpai_posy},
    {10, display.bottom - 320},
}
...
local HUXI_POS = {
    {player_pos[1][1] - 108, player_pos[1][2] + 80},
    {player_pos[2][1] - 250, player_pos[2][2] - 90},
    {player_pos[3][1] + 65,  player_pos[3][2] - 90},
    {player_pos[1][1] - 108, player_pos[1][2] + 80},
}
...
function PlayerView:changePos()
    player_pos = {
        {display.left+player_pos_adjustx, display.bottom + 80},
        {display.width - player_pos_adjustx, display.height - player_pos_adjusty-30},
        {display.cx, display.height-player_pos_adjusty+85},
        {display.left+player_pos_adjustx, display.height - player_pos_adjusty-30},
    }
    ...
    ready_pos = {
        ...
        {display.width - player_pos_adjustx - ready_pos_adjustx, player_pos[2][2]},
        {player_pos_adjustx + ready_pos_adjustx, player_pos[3][2]},
        {player_pos_adjustx + ready_pos_adjustx, player_pos[4][2]},
    }
    ...
    curCardPos = {
        {display.cx, player_pos[1][2] + cur_card_adjusty},
        {player_pos[2][1], player_pos[2][2] - cur_card_adjusty},
        {display.cx, player_pos[3][2] - cur_card_adjusty},
        {player_pos[4][1], player_pos[4][2] - cur_card_adjusty},
    }

    curCardPos = cur_card_pos

    THREW_PAI_POS = {  
        {display.width - throw_cards_adjustx, player_pos[1][2] + 250},
        {display.width - throw_cards_adjustx, player_pos[2][2] - throw_cards_adjusty+ 100},
        {display.width - throw_cards_adjustx-690, player_pos[3][2]},
        {throw_cards_adjustx, player_pos[4][2] - throw_cards_adjusty+100},
    }

    ZHUOMIAN_PAI_POS = {  
        {10, display.bottom - 320},
        {display.width - zuomianpai_posx - 51, player_pos[2][2] - zuomianpai_posy},
        {display.cx+zuomianpai_posx- 91, player_pos[3][2] - zuomianpai_posy},
        {zuomianpai_posx, player_pos[4][2] - zuomianpai_posy},
    }

    --桌面吃碰偎提 动画移动的位置
    ZHUOMIAN_PAI_ANI_POS = {  
        {150, display.bottom + 150},
        {1000, player_pos[2][2] - 100},
        {player_pos[3][1]+50, player_pos[3][2] - 100},
        {150, player_pos[4][2] - 100},
    }
    ...
    HUXI_POS = {
        ...
        {player_pos[2][1] - 250, player_pos[2][2] - 90},
        {player_pos[3][1] + 65, player_pos[3][2]- 90},
        {player_pos[4][1] + 65,  player_pos[4][2] - 90},
    }
...
function PlayerView:setIndex(index,number)
    print("PlayerView:setIndex",index,number)
    self.index_ = index
    self.number_ = 3
    if number and number == 4 then
        self.number_ = number
        dump(self.number_,"self.number_")
        self:changePos()
    end
    self.nodePlayer_:pos(unpack(OFFSET_OF_SEATS_4[index]))
end
...
function PlayerView:resetPos()
    ...
    -- ZHUOMIAN_PAI_POS = {  
    --     {10, display.bottom - 320},
    --     {display.width - zuomianpai_posx - 51, player_pos[2][2] - zuomianpai_posy - review_adjusty - review_zhuomianpai_adjusty},
    --     {zuomianpai_posx, player_pos[3][2] - zuomianpai_posy - review_adjusty - review_zhuomianpai_adjusty},
    -- }
    ...
    -- HUXI_POS = {
    --     {player_pos[1][1] - 108, player_pos[1][2] + 80},
    --     {player_pos[2][1] - 250, player_pos[2][2] + 80},
    --     {player_pos[3][1] + 65,  player_pos[3][2] + 80},
    -- }
end
...
function PlayerView:initPosByIndex_()
    ...
    local huxiX = HUXI_X
    if self.index_ == TABLE_YZCHZ_DIRECTION.BOTTOM or self.index_ == TABLE_YZCHZ_DIRECTION.LEFT then
    ...
    local huxiX, huxiY = unpack(HUXI_POS[self.index_])
...
function PlayerView:bindPlayer(player)
    ...
    local events = {
        ...
        {cls.SI_SHOU_EVENT, handler(self, self.onSiShou_)},
...
function PlayerView:onHuChanged_(event)
    ...
    self:playHuAnim(event.typeid,event.wAninType)
end
...
function PlayerView:calcWaiPaiPos_(index, offset)
    if TABLE_YZCHZ_DIRECTION.BOTTOM == self.index_ then
        return self:calcWaiPaiPosBottom_(index, offset)
    elseif TABLE_YZCHZ_DIRECTION.RIGHT == self.index_ then
...
function PlayerView:onAllPass_(event)
    ...
    local orgX, orgY = unpack(curCardPos[index])
...
function PlayerView:getThrewPaiPos(totalIndex)
    ...
    local x, y = unpack(THREW_PAI_POS[self.index_])
    local ceng = math.floor(index / 7)
    local row = math.mod(index, 7)
...
function PlayerView:doPaoPai(card,index,isReConnect,inFastMode)
  ...
  if index > 0 and #zkans >= index then
   ...
   --没有找到的话那就
    ...
    local orgX, orgY = unpack(curCardPos[self.index_])
    local py = OP_Y - OP_INTERRVAL * 3
    local aniKan = app:createConcreteView("KanView", cards,nil,false,nil,nil, #self.nodeZhuomianPai_:getChildren()):addTo(self.nodeAni_, showIndex):pos(OP_X,py)
    ...
    local action2 = cc.ScaleTo:create(mtime, 0.5)    
...
function PlayerView:doWeiPai(card,isChou,isReConnect,inFastMode)
    ...
    tmpCard:setScale(1.2)
    local mtime = 0.2
    transition.execute(tmpCard, cc.FadeOut:create(0.1), {
        delay = 0.2,
        easing = "",
        onComplete = function()
            local aniKan = app:createConcreteView("KanView", {card,0,0},nil,false,nil,nil, #self.nodeZhuomianPai_:getChildren()):addTo(self.nodeAni_, showIndex):pos(OP_X,wy)
            aniKan:showHuXi(false) 
            local mtime = 0.3 
            local anix,aniy = self:calcZhuomianPaiAniPos_()
            local actionDelay = cc.DelayTime:create(0.5)
...
function PlayerView:doChiPai(chiPai,biPai,card,isReConnect,inFastMode)
    ...
    local aniKan = app:createConcreteView("KanView", chiPai,nil,false,nil,nil, #self.nodeZhuomianPai_:getChildren()):addTo(self.nodeAni_, showIndex):pos(OP_X,cy)
    aniKan:showHuXi(false)
    --牌从中间运动到目标位置 
    local mtime = 0.3 
    local anix,aniy = self:calcZhuomianPaiAniPos_()
    ...
    for i,v in ipairs(biPai) do
        offset = offset + 100
        local aniKan = app:createConcreteView("KanView", v,nil,false,nil,nil, #self.nodeZhuomianPai_:getChildren()):addTo(self.nodeAni_, showIndex):pos(OP_X+offset, cy)
        aniKan:showHuXi(false)
            --牌从中间运动到目标位置 
            local mtime = 0.3 
            local anix,aniy = self:calcZhuomianPaiAniPos_()
...
function PlayerView:doPengPai(card,isReConnect,inFastMode)
    ...
    local py = OP_Y - OP_INTERRVAL * 3
    self:playPengAnim() 
    local aniKan = app:createConcreteView("KanView", {card,card,card},nil,false,nil,nil, #self.nodeZhuomianPai_:getChildren()):addTo(self.nodeAni_, showIndex):pos(OP_X,py)
    aniKan:showHuXi(false)
    --牌从中间运动到目标位置 
    local mtime = 0.3 
    --牌从中间运动到目标位置 
...
function PlayerView:doTiPai(card,index,isReConnect,inFastMode)
    ...
    transition.execute(tmpCard, cc.FadeOut:create(0.1), {
        ...
        easing = "",
        onComplete = function()
            local aniKan = app:createConcreteView("KanView", {card,0,0,0},nil,false,nil,nil, #self.nodeZhuomianPai_:getChildren()):addTo(self.nodeAni_, showIndex):pos(OP_X,ty)
            aniKan:showHuXi(false)
            --牌从中间运动到目标位置 
            local mtime = 0.3 
            local anix,aniy = self:calcZhuomianPaiAniPos_()
...
local ACTION_ANIM_POS = {
    ...
    {display.width * 600 / DESIGN_WIDTH, display.height * 880 / DESIGN_HEIGHT},
    {display.width * 187.5 / DESIGN_WIDTH, display.height * 880 / DESIGN_HEIGHT},
}
...
function PlayerView:calcChuPaiPosBottom_(total, index)
    ...
    local x = display.right + offset - 100
    local y = 470
...
function PlayerView:calcChuPaiPosLeft_(total, index)
    ...
    local y = 0
    if total > CHU_PAI_LINE_LENGTH then
        x = (display.width - majiang_width * CHU_PAI_LINE_LENGTH) / 2 + (index - 1) % CHU_PAI_LINE_LENGTH * majiang_width - 400
        y = display.height * 180 / DESIGN_HEIGHT - math.floor((index - 1) / CHU_PAI_LINE_LENGTH) * 44 + 480
    else
        local offset = (index - (total - 1) / 2 - 1) * majiang_width
        x = display.left + offset + 100
        y = 950
    end
    return x, y, index, CHU_PAI_SCALE
end
...
function PlayerView:calcZhuomianPaiPosBottom_(total, index)
    ...
    local offset = index * majiang_width * 1.2
    local x,y = unpack(ZHUOMIAN_PAI_POS[self.index_])
    local x = x + offset
    return x, y, index, CHU_PAI_SCALE
end
...
function PlayerView:calcRoundOverShowPokerPos_(total, index)
    if TABLE_YZCHZ_DIRECTION.LEFT == self.index_ then
...
local ani_adjustx = 150
function PlayerView:getAnimPosBySeatid()
    if TABLE_YZCHZ_DIRECTION.BOTTOM == self.index_ then
        return display.cx,display.cy
    elseif TABLE_YZCHZ_DIRECTION.RIGHT == self.index_ then
...
function PlayerView:playChouXiaoAnim()
    self:playGameAnimByType("chouxiao")
end

function PlayerView:playXiaoAnim()
    self:playGameAnimByType("xiao")
end

function PlayerView:playSiShouAnim()
    self:playGameAnimByType("sishou")
end

function PlayerView:playQingAnim()
    self:playGameAnimByType("qing")
end
...
function PlayerView:getHuAnimPos()
    ...
    elseif TABLE_YZCHZ_DIRECTION.RIGHT == self.index_ then
        return display.width - 200,1000
    else 
        return 200,1000
    end
end

function PlayerView:createCocosAnimations(data, parent, callfunc, toX, toY)
...
function PlayerView:playHuAnim(typeid,wAninType)
    local typestr = HUANIFILE[typeid]
    local x, y = self:getAnimPosBySeatid(seatid)
    local function showHuAnim()
        app:createConcreteView("HuAnim"):addTo(self.nodeAnims_):pos(x, y)
    end
    if wAninType then
        local animaData = FaceAnimationsData.getCocosAnimation(wAninType)
        print("XXXXXXX",x, y)
        self:createCocosAnimations(animaData,self.nodeAnims_,showHuAnim,x, y)
    else
        showHuAnim()
    end
    ...
    -- self:performWithDelay(function ()
    --     app:createConcreteView("PlayAnim",typestr):addTo(self.nodeAnims_):pos(x, y)
    -- end, 1)
end
