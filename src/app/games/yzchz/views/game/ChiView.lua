...
function ChiView:onGroupChoosed(index)
    ...
    transition.moveTo(self.selectFaGuang1, {x = self.width/ 10 * index - self.width/ 30 - self.offX[#self.curr_level_data], y = 98, time = 0.2, onComplete = function()
...
function ChiView:onGroupChoosed2(index)
    ...
    transition.moveTo(self.selectFaGuang2, {x = self.width/ 10 * index - self.width/ 30 - self.offX[#self.curr_level_data2], y = 98, time = 0.2, onComplete = function()
...
function ChiView:onGroupChoosed3(index)
    ...
    transition.moveTo(self.selectFaGuang3, {x = self.width/ 10 * index - self.width/ 30 - self.offX[#self.curr_level_data3], y = 98, time = 0.2})	
	self:onChooseEnd(self.p_choose)
end
...
function ChiView:showLevel()
	...
	if not self.haveNext then
		self.chiBg = display.newScale9Sprite('res/images/game/chiview_bg.png', -display.cx+640 , chi_pos_y+100, cc.size(self.width / 9 * #cards,190)):addTo(self) 
		self.chiBg:setAnchorPoint(cc.p(0.5, 0.5))
	else
		self.chiBg = display.newScale9Sprite('res/images/game/chiview_bg.png', aimX , chi_pos_y+100, cc.size(self.width / 9 * #cards,190)):addTo(self) 
		self.chiBg:setAnchorPoint(cc.p(0, 0.5))
	end
	...
	self.selectFaGuang1:setScale(1.2,1)
	for i = 1, #cards do
		local tmp_cards = {}
...
function ChiView:showLevel2()
	...
	local aimX = -display.cx / 2 + self.addChiBgDis + self.chiBg:getContentSize().width + 50
	self.chiBg2 = display.newScale9Sprite('res/images/game/chiview_bg.png', aimX, chi_pos_y+100, cc.size(self.width / 9 * #cards,190)):addTo(self) 
	...
	self.selectFaGuang2:setScale(1.2,1)
...
function ChiView:showLevel3()
	...
	local aimX = -display.cx / 2 + self.addChiBgDis + self.chiBg:getContentSize().width + self.chiBg2:getContentSize().width + 100
	self.chiBg3 = display.newScale9Sprite('res/images/game/chiview_bg.png', aimX, chi_pos_y+100, cc.size(self.width / 9 * #cards,190)):addTo(self) 
	...
	self.selectFaGuang3:setScale(1.2,1)
	for i = 1, #cards do
