...
function GameOverView:clacItemPos_(total, index)
    ...
    local offset = (index - (total - 1) / 2 - 1) * signWith
    local x = offset
    local y = -50
    return x, y, index
end
...
function GameOverView:loaderCsb()
    self.csbNode_ = cc.uiloader:load("views/games/yzchz/GameOver.csb"):addTo(self)
...
function GameOverView:shareHandler_()
    display.getRunningScene():gameShareWeiXin("永州扯胡子","",callback,display.getRunningScene():getTable():getTid(),selfData:getUid(),self.recordId)
end
...
function GameOverView:share2Handler_()
    ...
    display.captureScreen(function (bSuc, filePath)
        ...
        display.getRunningScene():shareImage(2,"永州扯胡子","",0, callback, filePath)
