    ...
    children = {
        {type = TYPES.LAYER, x = display.cx, y = display.top - 15, scale = display.width / DESIGN_WIDTH, ap = {0.5, 0.5}, size = {660, 60},
            children = {
                ...
                {type = gailun.TYPES.SPRITE, var = "btnMenu_", filename = "res/images/game/btnBg.png", x = 482, y = -15,scale9 = true, size = {180, 270},ap = {0.5, 1},
                    children = {
    ...
    children = {
        {type = TYPES.SPRITE, x = display.left, y = display.top, filename = "res/images/shuangKou/progressView/bg.png", scale = 1, ap = {0, 1},
            children = {
                ...
                {type = TYPES.LABEL, var = "labelInfo_", x = 210, y = 30, options = {text="房间号:456287   局数12/16", size=22, font = DEFAULT_FONT, color=cc.c3b(255, 255, 255)}, ap = {0.5, 0.5}},
                {type = TYPES.SPRITE, var = "content_", ppx = 0.5, ppy = 0.5},
                {type = TYPES.LABEL, visible = true, var = "timeLabel_", x = 40, y = 15, options = {text="00:00", size=20, font = DEFAULT_FONT, color=cc.c3b(255, 255, 255)}, ap = {0.5, 0.5}},
                ...
                {type = TYPES.BUTTON, var = "buttonMenu_", normal = "res/images/game/btn_tub.png", autoScale = 0.9, x = display.right - 50, y = 15},
                {type = TYPES.BUTTON, var = "buttonWanFa_", normal = "res/images/game/bt_gz.png", autoScale = 0.9, x = display.right - 130, y = 15},
                {type = gailun.TYPES.SPRITE, var = "btnMenu_", filename = "res/images/game/btnBg.png", x = display.right - 100, y = -15,scale9 = true, size = {180, 270},ap = {0.5, 1},
                    children = {
                        {type = TYPES.BUTTON, var = "buttonConfig_", normal = "res/images/game/btn_sheZhi.png", autoScale = 0.9, ppx =  0.5, ppy = 0.6,},
                        {type = TYPES.BUTTON, var = "buttonDismiss_", normal = "res/images/game/btn_jieSan.png", autoScale = 0.9, ppx = 0.5, ppy = 0.4,},
                        {type = TYPES.BUTTON, var = "buttonQuitRomm_", normal = "res/images/game/btn_tuiChu.png", autoScale = 0.9, ppx = 0.5, ppy = 0.4,},
...
function ProgressView:loadPlayer4Node()
    local roomInfo = self.labelInfo_:getString()
    local disVis = self.buttonDismiss_:isVisible()
    local quiVis = self.buttonQuitRomm_:isVisible()
    local cofVis = self.buttonConfig_:isVisible()
    local zlVis = self.buttonZl_:isVisible()
    self:removeAllChildren()
    self.secondTime = 0
    self.isFirstBattry = true
    self.secondBattery = 0
    gailun.uihelper.render(self, nodes_4Player)
    self.buttonMenu_:onButtonClicked(handler(self, self.onMenuClicked_))
    self.buttonDingWei_:onButtonClicked(handler(self, self.onDingWeiClicked_))
    self.buttonConfig_:onButtonClicked(handler(self, self.onConfigClicked_))
    self.buttonDismiss_:onButtonClicked(handler(self, self.onDismissClicked_))
    ...
    self.buttonWanFa_:onButtonClicked(handler(self, self.onWanFa_))
    self.buttonZl_:onButtonClicked(handler(self, self.onZlClicked_))
    self.labelInfo_:setString(roomInfo)
    self.buttonDismiss_:setVisible(disVis)
    self.buttonQuitRomm_:setVisible(quiVis)
    self.buttonConfig_:setVisible(cofVis)
    self.buttonZl_:setVisible(zlVis)
end

function ProgressView:hideHFNode_()
    self.btnMenu_:hide()
    self.buttonWanFa_:hide()
    self.buttonMenu_:hide()
end
...
function ProgressView:onConfigClicked_(event)
    display.getRunningScene():showGameSheZhi(true, GAME_YZCHZ)
end
