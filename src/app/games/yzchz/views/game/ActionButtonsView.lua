...
ActionButtonsView.ON_WC_CLICKED = "ON_WC_CLICKED"
ActionButtonsView.ON_WD_CLICKED = "ON_WD_CLICKED"
ActionButtonsView.ON_WZ_CLICKED = "ON_WZ_CLICKED"

local images = {"res/images/paohuzi/game/button_chi.png", "res/images/paohuzi/game/button_peng.png", "res/images/paohuzi/game/button_hu.png","res/images/paohuzi/game/button_wc.png","res/images/paohuzi/game/button_wd.png", "res/images/paohuzi/game/button_wz.png", "res/images/paohuzi/game/button_guo.png",}
...
function ActionButtonsView:onEnter()
    ...
    self.buttonWC_ = gailun.JWPushButton.new({normal = "res/images/paohuzi/game/button_wc.png"}, {autoScale = 0.9})
    self.buttonWD_ = gailun.JWPushButton.new({normal = "res/images/paohuzi/game/button_wd.png"}, {autoScale = 0.9})
    self.buttonWZ_ = gailun.JWPushButton.new({normal = "res/images/paohuzi/game/button_wz.png"}, {autoScale = 0.9})
    ...
    local functions = {
                        ...
                        function(sender) self:dispatchEvent({name = ActionButtonsView.ON_WC_CLICKED}) end,
                        function(sender) self:dispatchEvent({name = ActionButtonsView.ON_WD_CLICKED}) end,
                        function(sender) self:dispatchEvent({name = ActionButtonsView.ON_WZ_CLICKED}) end,
                        -- function(sender) self:dispatchEvent({name = ActionButtonsView.ON_GUO_CLICKED}) end,
                        function(sender) dataCenter:sendOverSocket(COMMANDS.YZCHZ_PLAYER_PASS, {}) end,
                      }
    for i=1, #buttons do
        if buttons[i] then
...
function ActionButtonsView:showActions(chi,peng,hu,wc,wd,wz)
    ...
    if data.ruleType == 5 and hu then
        chi,peng,guo = false,false,false
    end
    local isEnabledList = {chi,peng,hu,wd,wc,wz,guo}
    local btnList = {}
    for k, v in ipairs({
        self.buttonChi_, 
        self.buttonPeng_, 
    ...
    dump(btnList,"btnList")
    for k, v in ipairs(btnList) do
        print("position",interval * (k - 1) - width / 2)
        v:setPositionX(interval * (k - 1) - width / 2)
        
        v:show()
    end
   -- self.bg_:addChild(self.buttonGuo_)
end
