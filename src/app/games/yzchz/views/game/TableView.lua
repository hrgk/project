...
local HuangAnim = require("app.games.yzchz.views.HuangAnim")
...
tableNodes.normal = {
    ...
    children = {
        {type = TYPES.NODE, var = "nodeTableInfo_", children = {  -- 桌子信息容器
            ...
            {type = TYPES.LABEL_ATLAS, var = "labelLeftCards_", filename = "fonts/card_back_num.png", options = {text="", w = 33, h = 38, startChar = "0"}, x = display.cx , y = display.height - 110, ap = {0.5, 0.5}},
        }},
        ...
        {type = TYPES.CUSTOM, var = "directorController_", class = "app.games.yzchz.controllers.DirectorController"},
        ...
        {type = TYPES.SPRITE, var = "showHuCard_", filename = "res/images/cdphz/tip/tip.png",ap = {0,1}, x = display.cx-450, y =  display.cy},
        {type = TYPES.SPRITE, var = "showHuCardBg_", filename = "res/images/cdphz/tip/tipBg.png",ap = {0,1}, x = display.cx-550, y =  display.cy-40},
        {type = TYPES.CUSTOM, var = "progressView_", class = "app.games.yzchz.views.game.ProgressView"}, -- 主玩家的手牌容器，需要最高等级的显示
    }
}
...
local countPerLine, perWidth, perHeight = 20, 32, 36
...
local PuPingCardList = import("app.games.yzchz.views.pingPu.PaperCardList")
local ShanXingCardList = import("app.games.yzchz.views.shanXing.PaperCardList")
-- "app.games.yzchz.views.pingPu.PaperCardList"
local paiXingList = {}
...
function TableView:ctor(table)
    ...
    self.roundindx = ""
    -- self.showHuCardBg_:setVisible(setData:getCDPHZTPTS())
    -- self.showHuCard_:setVisible(setData:getCDPHZTPTS())
...
function TableView:doShowHuCard_(event)
    ...
    -- local cards = event.hucard

    -- table.sort(cards, function (lh, rh)
    ...
    -- self.showHuCardBg_:removeAllChildren()
    -- if len > 0 and setData:getCDPHZTPTS() then
    --     self.showHuCardBg_:setVisible(true)
    --     self.showHuCard_:setVisible(true)
    --     for i = 1,len do
    --         local index1 = math.modf(cards[i]/100)
    --         local index2 = cards[i]%100
    --         local path = string.format("res/images/cdphz/tip/%d/%d.png",index1,index2)
    --         display.newSprite(path):addTo(self.showHuCardBg_):pos(0+50+(i-1)*40+80-(len-1)*20 , h*0.5)
    --     end
    -- else
    --     self.showHuCardBg_:setVisible(false)
    --     self.showHuCard_:setVisible(false)
...
function TableView:onCurrCardChange_(event)
    ...
    if not event.card  or event.card == 0 then
        ...
        self:clearNodeMoPaiDaPai_()
        if TABLE_YZCHZ_DIRECTION.BOTTOM == event.index then
            self.currCard  = app:createConcreteView("PaperCardView", card, 1, event.isBack,nil):addTo(self.nodeMoPaiDaPai_):pos(unpack(cur_card_pos[1]))
        elseif TABLE_YZCHZ_DIRECTION.RIGHT == event.index then   
            self.currCard  = app:createConcreteView("PaperCardView", card, 1, event.isBack,nil):addTo(self.nodeMoPaiDaPai_):pos(unpack(cur_card_pos[2]))
        else
            self.currCard  = app:createConcreteView("PaperCardView", card, 1, event.isBack, nil):addTo(self.nodeMoPaiDaPai_):pos(unpack(cur_card_pos[3]))
        end
        if event.isBack then
            return
        end
...
function TableView:calcRoundOverShowPokerPos_(total, i, j, postion, playerX, playerY,perkan)
    if TABLE_YZCHZ_DIRECTION.LEFT == postion then
        return self:calcRoundOverPokerPosLeft_(total, i, j, playerX, playerY,perkan)
    elseif TABLE_YZCHZ_DIRECTION.RIGHT == postion then
...
function TableView:onShowDiPai_(event)
    ...
    if display.getRunningScene():getTable():getConfigData().rules.xingType == 1 then
        xTip =  display.newSprite("res/images/paohuzi/ani/genxing.png"):addTo(self.nodeRoundOver_):pos(display.cx, display.cy + 220 - roundOver_AdjustY)
    else
        xTip =  display.newSprite("res/images/paohuzi/ani/fanxing.png"):addTo(self.nodeRoundOver_):pos(display.cx, display.cy + 220 - roundOver_AdjustY)
    end
    ...
    local xingCard = nil
    if event.xingCard and event.xingCard ~= 0 then
        xingCard = app:createConcreteView("PaperCardView", event.xingCard, 1 , false, nil):addTo(self.nodeRoundOver_):pos(display.cx, display.cy + 420 - roundOver_AdjustY):scale(0.55)
        xingCard:setPositionY(550)
        xingCard:hide()
        xingCard:runAction(seq1)
    end
    ...
    for i=1,#cards do
        ...
        local poker = app:createConcreteView("PaperCardView", cards[i], 3 , false, nil):addTo(self.nodeRoundOver_):pos(display.cx, display.cy + 320 - roundOver_AdjustY)
        poker:fanPai()
        poker:hide()
...
function TableView:calcRoundOverHulistPos_(index,total,hutotal,winner)
    if winner then
        if TABLE_YZCHZ_DIRECTION.LEFT == winner:getIndex() then
            return self:calcRoundOverHulistPosLeft_(index,total,hutotal,winner)
        elseif TABLE_YZCHZ_DIRECTION.RIGHT == winner:getIndex() then
...
function TableView:playBomb_(cardType, index, scale)
    ...
    if index == TABLE_YZCHZ_DIRECTION.RIGHT then
        x, y = display.width * 0.78, 500
    elseif index == TABLE_YZCHZ_DIRECTION.LEFT then
...
function TableView:faceIconFly_(faceID, fromPlayer, toPlayer)
    ...
    if toIndex == TABLE_YZCHZ_DIRECTION.LEFT then
...
function TableView:onFinishRoundChanged_(event)
    ...
    local text = "房间号:"..roomID.."   局数"..event.num.."/"..event.total
    self.progressView_:setRoomInfo(text)
end

function TableView:hideHFNode()
    self.progressView_:hideHFNode_()
end
...
function TableView:makeRuleString(spliter)
    if self.configData_ then
        ...
        local list = {}
        local xInfo = {"跟醒","翻醒"}
        table.insert(list, xInfo[data.rules.xingType])
        if data.rules.wangCount == 0 then
            table.insert(list, "无王")
        else
            table.insert(list, data.rules.wangCount  .. "王")
        end
...
function TableView:showAlarm(seatID, seconds, flow)
    ...
    if player:getIndex() == 1 then
        self.directorController_:setScale(1)
    else
        self.directorController_:setScale(0.9)
    end
    if player then
...
function TableView:onFaPaiAction_(index,seatID)
    ...
    for i=1,count do
        ...
        self:performWithDelay(function ()
            if TABLE_YZCHZ_DIRECTION.BOTTOM == index then
...
function TableView:refreshReviewHandCards_(seatID)
    ...
    local cards = PaperCardGroup.roundOverChaiPai(self.reViewHandCards_[index],true)
    ...
    for i=1, #cards do
        for j=1,#cards[i] do
            ...
            y = y - 173
            local poker = app:createConcreteView("PaperCardView", cards[i][j], cardSize , false, nil):addTo(self.nodeRoundOver_):pos(x, y)
            poker:fanPai()
...
function TableView:showPalyer4View()
    self.progressView_:loadPlayer4Node()
    self.nodeTableInfo_:setPositionY(-160)
    player_pos = {
        {player_pos_adjustx, display.bottom + player_pos_adjusty},
        {display.width - player_pos_adjustx, display.height - player_pos_adjusty},
        {display.cx, display.height-player_pos_adjusty},
        {player_pos_adjustx, display.height - player_pos_adjusty},
    }
    ...
    ALARM_POSITION = {
        {display.cx, display.cy+120},
        {player_pos[2][1] - alarm_pos_adjustx+40, player_pos[2][2] - alarm_pos_adjusty+100},
        {player_pos[3][1] + alarm_pos_adjustx+80, player_pos[3][2] - alarm_pos_adjusty+130},
        {player_pos[4][1] + alarm_pos_adjustx-50, player_pos[4][2] - alarm_pos_adjusty+100},
    }
    cur_card_pos = 
    {
        {display.cx, display.cy+50},
        {player_pos[2][1] - mopai_pos_adjustx, display.height - mopai_pos_adjusty},
        {player_pos[3][1], player_pos[3][2] - mopai_pos_adjusty+120},
        {player_pos[4][1] + mopai_pos_adjustx, display.height - mopai_pos_adjusty}
    }
end
