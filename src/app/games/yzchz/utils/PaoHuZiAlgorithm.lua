...
function PaoHuZiAlgorithm.getGeneralCardSuit(cards)
	local remainCards = {}
	for i = 1,#cards do
		if cards[i] ~= BaseAlgorithm.KING then
			remainCards[1] = cards[i]
			break
		end
	end
	if #remainCards == 0 then
		return SUIT_BIG,true
	else
		return BaseAlgorithm.getSuit(remainCards[1])
	end
end
...
function PaoHuZiAlgorithm.calcHuXiTi(cards)
	...
	local s = PaoHuZiAlgorithm.getGeneralCardSuit(cards)
	if s == SUIT_BIG then
		return HX_TI_DA
	end
	return HX_TI_XIAO
...
function PaoHuZiAlgorithm.calcHuXiShun(cards)
	...
	for i = 1,#cards do
		if cards[i] ~= BaseAlgorithm.KING then
			table.insert(remainCards,cards[i])
		end
	end
	if #remainCards == 3 and not BaseAlgorithm.isShun(cards) then
		return 0
	end
	if #remainCards == 3 then
	...
	elseif #remainCards == 0 then
		return HX_123_DA
	else
		for i = 1,#remainCards do
			if not table.indexof({1,2,3},BaseAlgorithm.getValue(remainCards[i])) then
				return 0
			end
		end
		local s = BaseAlgorithm.getSuit(remainCards[1])
		if s == SUIT_BIG then 
			return HX_123_DA 
		end
		return HX_123_XIAO
...
function PaoHuZiAlgorithm.calcHuXi2710(cards)
	...
	elseif #remainCards == 0 then
		...
		for i = 1,#remainCards do
			if not table.indexof({2,7,10},BaseAlgorithm.getValue(remainCards[i])) then
				return 0
			end
		end
		local s = BaseAlgorithm.getSuit(remainCards[1])
...
function PaoHuZiAlgorithm.calcHuXiByCtypeAndCards(ctype, cards)
	dump(ctype,"calcHuXiByCtypeAndCardsctype")
	dump(cards,"cards")
...
function PaoHuZiAlgorithm.searchChi(cards, curr_card, notWithBiPai)
	...
	if #chiList == 0 and #ret > 0 then
		for i = 1,#ret do
			chiList[i] = {}
			chiList[i][1] = ret[i]
		end
		table.insert(chiList, ret)
	end
	return chiList
end
