...
function PokerTable:setConfigData(data)
    ...
    if data.rules.totalSeat == 2 then
        PaoHuZiAlgorithm.setMinHuXi(21)
    else
        PaoHuZiAlgorithm.setMinHuXi(15)
    end
...
function PokerTable:setCurCards(card,index,isMo,isBack) 
    self.curCards_ = card 
    isBack = isBack and isBack or false
    self:dispatchEvent({name = PokerTable.CURR_CARD_CHANGE, card = card, index = index, isMo = isMo,isBack = isBack})
end
...
function PokerTable:showDiPai(cards, huList, winner, huCard,xingCard) 
    self:dispatchEvent({name = PokerTable.SHOW_DI_PAI, cards = cards, index = index, huList = huList, winner = winner, huCard = huCard, xingCard = xingCard})
end
