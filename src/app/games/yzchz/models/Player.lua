...
Player.SI_SHOU_EVENT = "SI_SHOU_EVENT"  -- 死手改变事件
...
function Player:doTianHuStart(card)
    ...
    gameAudio.playYZCHZHumanSound(card..".mp3", self.sex_)
end
...
function Player:setPaoPai(card, index,isReConnect, inFastMode)
    ...
    if not inFastMode then    
        gameAudio.playYZCHZHumanSound("pao.mp3", self.sex_)
...
function Player:doSiShou()
    self:dispatchEvent({name = Player.SI_SHOU_EVENT})
end
...
function Player:onPengPaiExcecute_(event)
    ...
    if not inFastMode then
        gameAudio.playYZCHZHumanSound("peng.mp3", self.sex_)
    end
...
function Player:onChiPaiExcecute_(event)
    ...
    if not inFastMode then
        gameAudio.playYZCHZHumanSound("chi.mp3", self.sex_)  
        --将吃牌放第一张
...
function Player:setWeiPai(isChou, card,isReConnect, inFastMode)
    --臭偎没有
    if not inFastMode then
        gameAudio.playYZCHZHumanSound("wei.mp3", self.sex_)    
...
function Player:setTiPai(card, index,isReConnect, inFastMode)
     if not inFastMode then
        gameAudio.playYZCHZHumanSound("ti.mp3", self.sex_)
...
-- 设置出牌
function Player:setHuPai(typeid,wAninType)
    if not inFastMode then
        gameAudio.playYZCHZHumanSound("hu.mp3", self.sex_)
    end
    self:dispatchEvent({name = Player.HU_CHANGE_EVENT, typeid = typeid, wAninType = wAninType,inFastMode = inFastMode})
end
